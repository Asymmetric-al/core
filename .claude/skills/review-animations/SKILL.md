...
This is an explicit-only strict motion review. Keep
`disable-model-invocation: true`, and do not auto-route ordinary frontend work
to it in clients that ignore that Claude Code field. Reconcile this overlay
after upstream refreshes before running `bun run skills:sync`.

### Triggers

- The user explicitly invokes `review-animations` or asks for a strict,
...
1. Read `docs/ai/rules/frontend.md`, `emil-design-engineering`, and `anim` before
   applying `STANDARDS.md`.
2. Review only the requested diff or fixed comparison point and cite exact
   files and lines.
3. Core's Base UI variables, shared tokens, route-transition ownership, and
   global reduced-motion policy override generic upstream examples.
4. Return the upstream finding table and explicit verdict; do not modify code.

### Checklist

- [ ] Invocation was explicit and scope is motion-only.
- [ ] Findings use Core tokens and Base UI conventions.
- [ ] Every blocking claim has direct diff evidence.
