...
> **Core:** `apps/*/features/` folders are UI composition only. Do not add `*-queries.ts` or `*-actions.ts` at all. Call `@asym/api` or `@asym/database/hooks` from the feature component.
