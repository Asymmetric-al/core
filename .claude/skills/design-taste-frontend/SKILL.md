...
`disable-model-invocation: true`. It targets marketing/landing/portfolio
taste, not Core product surfaces.
...
- The user explicitly invokes `design-taste-frontend`.
- Do not auto-route Core app UI, dashboards, or data tables through it.

### Workflow

1. Do not restyle Core product apps or replace `base-maia`.
2. Keep frontend.md, Base UI, and Zinc tokens if the request touches this
   monorepo's apps.
3. Prefer `impeccable` and Core frontend skills for in-repo UI critique.

### Checklist

- [ ] Invocation was explicit.
- [ ] `base-maia` and Core product language were left intact.
