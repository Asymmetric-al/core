...
Color recipes stay subordinate to Core's Zinc-oriented CSS
variable tokens. Do not introduce a parallel palette.
...
- Reviewing contrast, token naming, or palette structure.
- Do not generate a new brand palette for existing Core apps.

### Workflow

1. Use existing semantic CSS variables in `packages/ui`.
2. Keep `base-maia` and frontend rulebook contrast/token rules.
3. Do not restyle Core product apps onto an upstream palette.

### Checklist

- [ ] Semantic tokens were reused.
- [ ] No new hardcoded palette replaced Core theme variables.
