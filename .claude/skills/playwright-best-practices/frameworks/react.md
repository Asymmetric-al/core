...
test.describe("auth state", () => {
  test("login updates header across components", async ({ page }) => {
    ...
    await page.getByLabel("Password").fill("secret123"); // pragma: allowlist secret
...
test.describe("signup form", () => {
  ...
  test("validation on empty submit", async ({ page }) => {
    ...
    await expect(page.getByText("Password required")).toBeVisible(); // pragma: allowlist secret
  });
  ...
  test("successful submission redirects", async ({ page }) => {
    ...
    await page.getByLabel("Password", { exact: true }).fill("Secure123!"); // pragma: allowlist secret
