...
const DISABLE_COMMENT_START =
  /^\s*(?:\/\/|\/\*+)\s*eslint-disable(?:-next-line|-line)?\b/;
const PAYLOAD_GENERATED_MARKER =
...
async function verifyDisableCommentFormat() {
  ...
  for (const filePath of files) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      const hasDisableComment = DISABLE_COMMENT_START.test(line);
      if (!hasDisableComment || line.includes("eslint-enable")) {
        continue;
      }

      if (isPayloadGeneratedFile && line.trim() === "/* eslint-disable */") {
        continue;
      }
