...
test("form submission shows server-side validation errors", async ({
  ...
  await page.getByLabel("Password", { exact: true }).fill("Secure1@pass"); // pragma: allowlist secret
...
test("form redirects after successful submission", async ({ page }) => {
  ...
  await page.getByLabel("Password").fill("admin123"); // pragma: allowlist secret
...
test("fill and submit a signup form", async ({ page }) => {
  ...
  await page.getByLabel("Password", { exact: true }).fill("P@ssw0rd!"); // pragma: allowlist secret
  await page.getByLabel("Confirm password").fill("P@ssw0rd!"); // pragma: allowlist secret
