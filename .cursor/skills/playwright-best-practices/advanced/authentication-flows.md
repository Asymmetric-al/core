...
test("completes registration with email verification", async ({ page }) => {
  ...
  await page.getByLabel("Password", { exact: true }).fill("SecurePass!"); // pragma: allowlist secret
  await page.getByLabel("Confirm password").fill("SecurePass!"); // pragma: allowlist secret
...
test("verifies email with mocked endpoints", async ({ page }) => {
  ...
  await page.getByLabel("Password", { exact: true }).fill("Password123!"); // pragma: allowlist secret
...
```typescript
test("resets password through email link", async ({ page }) => { // pragma: allowlist secret
  ...
  await page
    .getByLabel("New password", { exact: true }) // pragma: allowlist secret
    .fill("NewPassword456!"); // pragma: allowlist secret
  await page.getByLabel("Confirm password").fill("NewPassword456!"); // pragma: allowlist secret
  await page.getByRole("button", { name: "Update password" }).click(); // pragma: allowlist secret

  await expect(page.getByText("Password updated")).toBeVisible(); // pragma: allowlist secret
});
```
...
test("shows error for expired reset token", async ({ page }) => {
  await page.goto("/reset-password?token=expired-token"); // pragma: allowlist secret

  await page.getByLabel("New password", { exact: true }).fill("NewPass!"); // pragma: allowlist secret
  await page.getByLabel("Confirm password").fill("NewPass!"); // pragma: allowlist secret
  await page.getByRole("button", { name: "Update password" }).click(); // pragma: allowlist secret
...
test("enforces password requirements on reset", async ({ page }) => { // pragma: allowlist secret
  ...
  await page.getByLabel("New password", { exact: true }).fill("weak"); // pragma: allowlist secret
  await page.getByLabel("Confirm password").fill("weak"); // pragma: allowlist secret
  await page.getByRole("button", { name: "Update password" }).click(); // pragma: allowlist secret
...
test("persists session with remember me enabled", async ({ browser }) => {
  ...
  await page1.getByLabel("Password").fill("Password!"); // pragma: allowlist secret
