...
test("popup login flow", async ({ page }) => {
  ...
  await popup.getByLabel("Password").fill("password123"); // pragma: allowlist secret
...
test("Google OAuth login", async ({ page }) => {
  ...
  await popup.getByLabel("Enter your password").fill("password"); // pragma: allowlist secret
  await popup.getByRole("button", { name: "Next" }).click();
