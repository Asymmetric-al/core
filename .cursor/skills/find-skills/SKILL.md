...
**Example — shadcn:** official shadcn/ui work is covered by `docs/ai/skills/shadcn/SKILL.md`; repo-local Maia/Zinc and shared `@asym/ui` conventions are in `docs/ai/skills/moai-library-shadcn/SKILL.md`. Install or refresh the official shadcn skill with `npx skills add shadcn/ui --skill shadcn -y`, then `bun run skills:refresh-upstream`, `bun run skills:sync`, and `bun run skills:verify`.
