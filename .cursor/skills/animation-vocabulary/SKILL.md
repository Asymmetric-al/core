...
This repo-owned overlay keeps the upstream glossary narrow and compatible with
Core's canonical frontend guidance. Reconcile it after upstream refreshes
...
1. Use the upstream glossary to name and disambiguate the effect.
2. Keep the answer concise and do not turn a naming request into UI work.
3. If implementation or review follows, load `docs/ai/rules/frontend.md` and
   `docs/ai/skills/anim/SKILL.md`; those repo contracts win.

### Checklist

- [ ] The response leads with the most precise glossary term.
- [ ] Close alternatives are distinguished without inventing vocabulary.
- [ ] Any implementation advice is deferred to Core's frontend and motion
      rules.
