...
This is a change-review companion. Load `better-writing`
with it when copy is in scope. Do not treat it as a mandate to restyle Core
apps.
...
- Reviewing a UI change across polish, type, layout, color, writing, and
  accessibility categories.
- Pair with `interface-review` only when that explicit review skill is
  invoked.

### Workflow

1. Stay on the changed surfaces. Do not audit the whole product.
2. Defer Core a11y ownership to `docs/ai/skills/accessibility-review/SKILL.md`.
3. Keep `base-maia`, Base UI, and frontend.md above generic taste notes.
4. Load `docs/ai/skills/better-writing/SKILL.md` when writing findings need
   the dedicated copy skill.

### Checklist

- [ ] Findings stayed on the change, not a redesign brief.
- [ ] `better-writing` was used when copy was in scope.
