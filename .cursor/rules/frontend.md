...
- If you use Nia (MCP) to trace UI code, keep queries scoped to `Asymmetric-al/core` and use the preamble built from `docs/ai/working-set.md` + `docs/ai/stack-registry.md` for search calls (see `.cursor/rules/AGENTS.md#nia-mcp-usage-always-repo-scoped`).
...
- Follow `.cursor/rules/testing.md` for Playwright/a11y/perf expectations.

## Workflow

1. Identify if the change is Server or Client and follow Next.js App Router conventions.
...
5. If shadcn/studio MCP is used, switch to `.cursor/rules/shadcn-studio-mcp.md` and follow it exactly.
