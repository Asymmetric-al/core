...
**Purpose:** Draft or upgrade a single Asymmetric.al GitHub issue so it is implementation-ready (issue lifecycle: draft).
Use this command for one issue at a time.

**When to use:** Drafting or improving a single existing `AL-###` issue before implementation.
...
- If using Nia (MCP) for codebase context, scope queries to `Asymmetric-al/core` (see `.cursor/rules/AGENTS.md#nia-mcp-usage-always-repo-scoped`).
