...
> **Core:** `apps/*/features/` folders are UI composition only. Do not add `*-queries.ts` or `*-actions.ts` at all. Async Server Components call published `@asym/api/*` reads; only `'use client'` leaves use `@asym/database/hooks`. Client mutations cross a supported hook/API route or authenticated Server Action adapter. Server tags and invalidators stay API-owned; feature cache files contain browser keys/options only.

How to organize code under `features/` and `app/`.

## Folder layout

This is the vendor layout. In Core, skip the query/action files and keep server tags in their API owner; `<domain>-cache.ts` contains browser keys only.

```text
features/<domain>/
  <domain>-cache.ts     # Browser query keys/options only (Core); vendor also puts server tags here
  <domain>-queries.ts   # Vendor only — Core: skip; call published @asym/api/* reads
  <domain>-actions.ts   # Vendor only — Core: skip; mutate through packages/api
...
Concrete example: vendor next-beats puts `toggleFavorite` in `features/event/event-actions.ts` because the parent domain owns the concept. In Core the favorite button stays in `features/event/components/`; the privileged mutation lives in `packages/api`. Do not create `event-actions.ts` or a `favorite` feature.
...
Vendor next-beats also keeps `event-queries.ts` / `event-actions.ts` here. **Core: skip those files.**

- `<folder>-queries.ts` — vendor naming; Core does not add this file.
- `<folder>-actions.ts` — vendor naming even when a mutation is about a sub-concept; Core does not add this file.
...
- **`components/ui/`** — upstream primitive location. **Core:** reuse `@asym/ui` / `packages/ui`; do not create an app-local primitive or theme fork.
- **`components/theme/`** — theme provider and toggle, paired.
- **Top-level files** (`site-header.tsx`, `auth-gate.tsx`, `poller.tsx`) — app-shell singletons used once each. No `common/` folder — "common" is not a category. If a component is used everywhere it's a primitive (Core: `packages/ui`, upstream: `ui/`); if it's used once it lives at the top level.
