...
| 7. Queries and mutations live in `packages/api`, not `apps/*/features/` | Vendor next-beats still shows `features/track/track-queries.ts` and `features/playlist/playlist-actions.ts`. In Core those files are **not** created; call `@asym/api` from the feature component. |
