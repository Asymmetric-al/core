...
`@asym/eslint-config` pins `@shadcn/lint@0.2.0` with a Core-maintained
Bun compatibility patch. The
[published tarball](https://registry.npmjs.org/@shadcn/lint/-/lint-0.2.0.tgz)
was released on 2026-09-22; Bun's lockfile records its integrity. Official
documentation and relevant source/tests were reviewed at
[`a89d047`](https://github.com/shadcn-ui/lint/tree/a89d04792f340bcea26af2d539a9eec7285fcc77),
and the installed package was checked separately. The downloaded SHA-512
matches registry integrity. npm's provenance payload names the same source
commit and release run; its signature chain was not independently verified.
Do not overstate that evidence as independent source-to-build attestation.

The root `patchedDependencies` entry applies
`tooling/eslint-config/patches/@shadcn%2Flint@0.2.0.patch` reproducibly during frozen installation.
The patch lives under the tooling owner so a patch-only change selects its
consumer workspaces through the dependency graph. It repairs component-owner
lookup inside `componentFromImport`, preserving upstream node_modules guards,
and validates exact contract classes through the actual Tailwind compiler
when static grammar/theme indexes do not contain plugin-generated classes.
Unknown classes and broken themes still fail. Core configuration
...
covers canonical and local/mixed barrels, aliases, wrappers, unrelated
same-name components and exact typography-plugin contracts. Review both
repairs alongside every package upgrade.

The release supports Node >=20.19, ESLint >=9.30, and Tailwind 4. Core's
verified toolchain is Node 24.15.0, ESLint 9.39.5, Tailwind 4.2.2,
Bun 1.3.14, and Turbo 2.10.0. Core's tooling peer requires ESLint
`^9.39.2` for the flat-config `basePath` API; the root requires `^9.39.5`.
ESLint 9 is API-compatible but reached
[EOL on 2026-08-06](https://eslint.org/version-support/); migrating the
existing Next.js/shared lint stack to ESLint 10 requires compatible framework
plugins. The current published React plugin still calls removed
`context.getFilename()` and the React, JSX-a11y and import plugins exclude
ESLint 10 in their peers. Do not force the major or drop their checks.
...
[rules](https://github.com/shadcn-ui/lint/blob/a89d04792f340bcea26af2d539a9eec7285fcc77/docs/rules.md),
[analysis model](https://github.com/shadcn-ui/lint/blob/a89d04792f340bcea26af2d539a9eec7285fcc77/docs/how-it-works.md),
and [troubleshooting](https://github.com/shadcn-ui/lint/blob/a89d04792f340bcea26af2d539a9eec7285fcc77/docs/troubleshooting.md).

Version 0.2.0 adds Vue/Svelte support and repairs comment handling inside barrel
export lists. Core remains a React/TSX repository and needs no additional
parser. Keep the existing six-rule policy. The commented-barrel regression
covers canonical control identity and the delegated title typography contract;
comments must not turn legal title typography into a consumer override.
...
Dialog, Popover and Sheet content expose width, min-width and max-width through
their actual popup APIs. Those dimensions are caller-owned; height, scrolling,
focus, color, shape and motion remain protected. This does not grant width
overrides on ordinary Buttons or Inputs.

For long dialog content, use the shared `DialogContent scrollable` API. It
keeps the popup within the dynamic viewport using the existing spacing-scale
inset and provides vertical scrolling. The default remains unchanged. Consumer
height or overflow overrides remain rejected; verify keyboard access and
reachable actions at short and narrow viewports when enabling scrolling.

Icons imported directly from `lucide-react` or Core's exact icon registry keep
their SVG presentation APIs. Exclusions use import provenance, not component
names; renaming a Button to Search does not make it an icon. Token, arbitrary
value, inline-style and unknown-class rules still apply.

`PostContent` explicitly accepts rich-text and HTML presentation classes. Its
contract permits typography, the supported typography-plugin prose classes and
`max-w-none`; it does not grant palette, foreground/background, shape, effect
or unreadable-class overrides. Exact plugin classes are validated by real
Tailwind generation rather than silently accepting an unknown-contract warning.
...
Review native anchors and Next `Link` elements that consume `buttonVariants`
separately. In the vetted release, `no-restyle` does not infer Button ownership
from a variant function inside their `className`; a surrounding `cn()` override
can pass that rule even when the same override on `<Button>` is rejected.
Use the helper's existing variant/size options and keep additional classes to
placement. This is an analysis limit, not permission to override appearance.
Do not register `buttonVariants` as a merge helper to manufacture coverage: its
options-object API is different. Verify rendered controls and review their
class expressions as part of the existing complementary UI checks.
...
- The legacy Unlayer adapter permits only `width`, `opacity` and `transition`
  on its `EmailEditor` component. Installed `react-email-editor` exposes a
  `style` API without `className` or DOM prop forwarding. These properties
  size and reveal its internal editor host independently of the sibling loading
  overlay. Redundant flex/display/height inputs were removed after rendered
  verification of the installed DOM and existing adapter CSS. Native elements,
  other properties and sibling files remain checked. Reassess this contract
  when the external adapter API changes; it is not a first-party styling escape.
...
On 2026-09-22 the user explicitly included complete legacy UI cleanup. The
adoption snapshot above is historical migration debt, not the completion
target. The active change now requires a full raw scan with zero findings and
an empty bulk baseline, with behavioral/rendered proof for affected surfaces.
Track remaining work in `openspec/changes/integrate-design-system-lint/`;
do not mark cleanup complete merely because the bounded baseline gate passes.
...
Historical upstream [issue #6](https://github.com/shadcn-ui/lint/issues/6) affected 0.1.0:
...
The comment parser was repaired upstream in 0.1.2. The same shared source directives still sit at the end of
...
Current analysis boundaries and upgrade regressions require explicit review:
...
- Canonical package imports, renamed exports, physical forwarding wrappers,
  ...
  Stock 0.2.0 still loses shared ownership through a local barrel when the app has
...
- [Base UI render composition (#20)](https://github.com/shadcn-ui/lint/issues/20)
  was fixed in 0.1.1. The regression now requires caller classes to be
  attributed to the actual rendered Button and its size API. An opaque render
  target retains the trigger's protection. Prefer the rendered element's
  actual variant/API; do not broadly unlock triggers.
