...
type Fixtures = {
  ...
  member: { email: string; password: string; id: string }; // pragma: allowlist secret
...
export const test = base.extend<Fixtures>({
  ...
  loggedInPage: async ({ page, member }, use) => {
    ...
    await page.getByLabel("Password").fill(member.password); // pragma: allowlist secret
...
export function generateMember(overrides: Partial<Member> = {}): Member {
  return {
    email: generateEmail(),
    password: "SecurePass456!", // pragma: allowlist secret
    name: "Test Member",
    ...overrides,
  };
}
