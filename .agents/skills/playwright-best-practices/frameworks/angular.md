...
test.describe("form validation", () => {
  ...
  test("validates password confirmation", async ({ page }) => { // pragma: allowlist secret
    ...
    await expect(page.getByText("Passwords must match")).toBeVisible(); // pragma: allowlist secret

    await page.getByLabel("Confirm password").fill("Secret123!"); // pragma: allowlist secret
    await expect(page.getByText("Passwords must match")).toBeHidden(); // pragma: allowlist secret
  });
