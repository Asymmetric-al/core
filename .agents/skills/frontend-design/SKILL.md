...
This skill is explicit-only. Keep
`disable-model-invocation: true`. Use it for isolated distinctive-design
exploration, and do not restyle Core product apps.
...
- The user explicitly invokes `frontend-design` for a greenfield artifact
  or an authorized visual exploration.
- Do not auto-route Core admin, donor, or missionary UI through this skill.

### Workflow

1. If the work is in `apps/*` or `packages/ui`, stop and follow
   `docs/ai/rules/frontend.md` plus `base-maia`.
2. Do not restyle Core product apps. Distinctive-aesthetic prompts do not
   override Base UI, Zinc tokens, or existing product language.
3. Animation still starts with `emil-design-engineering` and `anim`.

### Checklist

- [ ] Invocation was explicit.
- [ ] Core product apps were not restyled.
