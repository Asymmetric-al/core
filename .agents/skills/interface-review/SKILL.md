...
`disable-model-invocation: true`. It reviews a change, not the whole
product, and must not restyle Core apps.
...
- The user explicitly invokes `interface-review`.
- Do not auto-route ordinary UI implementation through this skill.

### Workflow

1. Resolve the change scope first. Stay on what the change caused.
2. Hand domain rules to the `better-*` skills; Core a11y stays with
   `accessibility-review`.
3. Keep `base-maia`, Base UI, and frontend.md above generic review taste.
4. Do not treat findings as a mandate to redesign product apps.

### Checklist

- [ ] Invocation was explicit.
- [ ] The review stayed on the change and preserved Core UI ownership.
