...
This is Emil Kowalski's variant-exploration skill, vendored as `emil-prototype`
so it does not collide with Matt Pocock's `prototype` skill. Keep
`disable-model-invocation: true`. Reconcile this overlay after upstream
refreshes before running `bun run skills:sync`.

### Triggers

- The user explicitly invokes `emil-prototype` or asks for multiple live UI
  variants behind a picker.
- Do not confuse it with Matt Pocock `docs/ai/skills/prototype/SKILL.md`.
...
- [ ] Invocation was explicit and did not overwrite Matt Pocock `prototype`.
- [ ] Production code stayed untouched during exploration.
...
- **In a project with a dev server** — an isolated prototype surface outside app routes, one file per variant plus a small harness file. Nothing imports from the prototype surface into production code.
