...
- **Route transitions** belong to `RouteMainViewTransitionBoundary`. Don't compete with it — when a `motion.div` lives inside a route VT layer, suppress its entrance via `useWithinViewTransitionRouteLayer()` (see `PageShell`). The same applies to CSS page-body entrances: gate `animate-in fade-in` page wrappers with the hook (`!withinRouteVt && "animate-in fade-in duration-300"`); tab-panel and content-swap entrances are unaffected.
...
- **Loading spinners**: prefer the shared `<Spinner>` from `@asym/ui/components/shadcn/spinner` (carries `role="status"`/`aria-label` and the `.spinner-essential` utility — CSS, off-main-thread, reduced-motion-exempt essential status feedback). Bare `Loader2` icons should add `spinner-essential`; never wrap them in a `motion.div` rotate loop.
