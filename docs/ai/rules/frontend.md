...
Full reference: `docs/ai/skills/anim/SKILL.md`. Operative rules:

- **Use the motion tokens, not literals.** Prefer `var(--duration-{micro,standard,modal,drawer,route,shared})` and `var(--ease-out-soft)` (or the `EASE_OUT_SOFT` / `DURATION_*` constants from `@asym/lib/motion-presets`). All motion tokens live in `packages/ui/styles/globals.css :root`.
...
- **Don't animate `width` / `height` / layout properties.** Use `transform: scaleX/scaleY/translate` instead. The impact meter in `home-sections.tsx` is the canonical example.
