...
**Matt Pocock `ask-matt`** ([`mattpocock/skills`](https://github.com/mattpocock/skills) `skills/engineering/ask-matt`): `npx skills add mattpocock/skills --skill ask-matt -y`, then `bun run skills:refresh-ask-matt`, `bun run skills:sync`, and `bun run skills:verify`. The focused refresh restores the grill-depth overlay under the main flow and remaps the writing-for-agents successor to the kept `writing-great-skills` snapshot. Do not vendor successor writing-for-agents.
...
- **Explicit UI-library lookup:** `docs/ai/skills/pick-ui-library/SKILL.md`; Core already chose Base UI / `base-maia`. Keep `disable-model-invocation: true`.
- **Emil variant exploration:** `docs/ai/skills/emil-prototype/SKILL.md`; explicit-only, distinct from Matt Pocock `prototype`. Keep `disable-model-invocation: true`.
- **Swift craft only:** `docs/ai/skills/write-swift/SKILL.md`; unused for Core Next.js apps.
...
- **Scaffold a new eve agent from an interview ([ikindacodes/ship-eve](https://github.com/ikindacodes/ship-eve)):** `docs/ai/skills/create-agent/SKILL.md` (pair with **eve**; **kept snapshot** because upstream no longer publishes `skills/`)
- **NestJS architecture reference ([Kadajett/agent-nestjs-skills](https://github.com/Kadajett/agent-nestjs-skills)):** `docs/ai/skills/nestjs-best-practices/SKILL.md`; Core is not a NestJS application — do not introduce NestJS or TypeORM here.
...
| Id                                            | Notes                                                                                                                                                                                                                        |
| --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **ask-matt**                                  | `docs/ai/skills/ask-matt/SKILL.md` — Router over the Matt Pocock skill flows. Core overlay keeps `/grill-for-unknowns` on the main-flow grill-depth step.                                                                    |
...
| **qa**, **request-refactor-plan**             | `docs/ai/skills/qa/SKILL.md`, `docs/ai/skills/request-refactor-plan/SKILL.md` — **Kept snapshots**; removed from `mattpocock/skills` HEAD (previously `skills/deprecated/`). Retain for lockfile identity and compatibility. |
| **ubiquitous-language**                       | `docs/ai/skills/ubiquitous-language/SKILL.md` — **Kept snapshot**; removed from HEAD. Prefer **domain-modeling** for active work.                                                                                            |
| **design-an-interface**                       | `docs/ai/skills/design-an-interface/SKILL.md` — **Kept snapshot**; removed from HEAD. Prefer **codebase-design** / **improve-codebase-architecture**.                                                                        |
| **edit-article**, **obsidian-vault**          | `docs/ai/skills/edit-article/SKILL.md`, `docs/ai/skills/obsidian-vault/SKILL.md` — **Kept snapshots**; removed from HEAD (previously `skills/personal/`).                                                                    |
| **writing-great-skills**                      | `docs/ai/skills/writing-great-skills/SKILL.md` — **Kept snapshot**; removed from HEAD. Successor `skills/productivity/writing-for-agents/` is not vendored and is not lockfile-managed.                                      |
...
**Removed from upstream (kept snapshots):** `design-an-interface`, `edit-article`, `obsidian-vault`, `qa`, `request-refactor-plan`, `ubiquitous-language`, and `writing-great-skills` 404 on `mattpocock/skills` HEAD. Keep the last vendored copies; a GitHub 404 is not a reason to delete them or grow the 128-name lockfile. Do not vendor successor `writing-for-agents`.
