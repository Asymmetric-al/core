...
## 2026-05-08 (AI tooling skills/MCP standardization)

- Date: 2026-05-08
- Repo: Asymmetric-al/core
- Goal: Standardize documented AI-agent skills, MCP server guidance, and CLI guidance around official Supabase and shadcn tooling while preserving the repo's canonical skill/mirror patterns.
- Primary area:
  - `AGENTS.md`, `cursor.md`, `.mcp.json`, `.cursor/mcp.json`
  - `docs/ai/skills/*`, `.agents/skills/*`, `.cursor/skills/*`
  - `docs/ai/rules/{backend,frontend,shadcn-studio-mcp}.md`
  - `docs/mcp-config.example.toml`, `docs/guides/development/mcp-config.example.toml`
  - `scripts/{refresh-upstream-skills,supabase-cli,setup-codex-mcp}.mjs`
  - `README.md`, `openspec/project.md`, `supabase/AGENTS.md`
- Constraints:
  - Documentation/tooling-only; no application code, migrations, secrets, or CI workflow changes.
  - Keep `AGENTS.md` canonical, `cursor.md` concise, `docs/ai/skills` canonical, and `.agents` / `.cursor` skill mirrors generated by `bun run skills:sync`.
  - Keep `.mcp.json` and `.cursor/mcp.json` mirrored; do not add unsafe unscoped Supabase MCP production access.
  - Use official Supabase and shadcn docs as source of truth; preserve Windows/macOS/Codex/Cursor compatibility.
- Evidence sources used:
  - Official Supabase CLI local development and CLI reference docs
  - Official Supabase agent skills, Postgres best-practices blog, agent-skills announcement, and MCP docs
  - Official shadcn CLI v4 changelog, skills docs, and MCP docs
  - Repo scripts: `scripts/sync-agent-skills.mjs`, `scripts/verify-skills-sync.mjs`, `scripts/refresh-upstream-skills.mjs`
