...
This skill is Swift craft guidance. Core's product apps are Next.js and do not
use Swift. Reconcile this overlay after upstream refreshes before running
`bun run skills:sync`.
...
1. Confirm the request is actually Swift before applying this file.
2. For Core Next.js apps, use `docs/ai/rules/frontend.md` and the matching
   TypeScript/React skills instead.

### Checklist

- [ ] The target language is Swift, not Core's TypeScript apps.
