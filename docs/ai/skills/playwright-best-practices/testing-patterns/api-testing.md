...
type SeedFixtures = {
  seedAccount: { id: number; email: string; password: string }; // pragma: allowlist secret
...
export const test = base.extend<SeedFixtures>({
  seedAccount: async ({ request }, use) => {
    ...
    const resp = await request.post("/api/accounts", {
      data: { name: "Test Account", email, password }, // pragma: allowlist secret
    });
    expect(resp.ok()).toBeTruthy();
    const account = await resp.json();

    await use({ id: account.id, email, password }); // pragma: allowlist secret
...
test("user sees workspace on dashboard", async ({
  ...
  await page.getByLabel("Password").fill(seedAccount.password); // pragma: allowlist secret
