...
await page.getByLabel("Password").fill("secretPass123"); // pragma: allowlist secret
...
const response = await context.request.post("/api/auth/login", {
  data: { email: "testuser@example.com", password: "secretPass123" }, // pragma: allowlist secret
});
...
const accounts = [
  ...
  },
  {
    ...
    password: process.env.MEMBER_PASSWORD!, // pragma: allowlist secret
  },
  {
    role: "guest",
    email: "guest@example.com",
    password: process.env.GUEST_PASSWORD!, // pragma: allowlist secret
  },
];
...
test.describe("login page", () => {
  ...
  test("forgot password link navigates correctly", async ({ page }) => { // pragma: allowlist secret
    ...
    await expect(
      page.getByRole("heading", { name: "Reset password" }), // pragma: allowlist secret
    ).toBeVisible();
  });
});
```
...
test.describe("unauthenticated access", () => {
  ...
  test("signup flow creates account", async ({ page }) => {
    ...
    await page.getByLabel("Password", { exact: true }).fill("secretPass123"); // pragma: allowlist secret
    await page.getByLabel("Confirm password").fill("secretPass123"); // pragma: allowlist secret
