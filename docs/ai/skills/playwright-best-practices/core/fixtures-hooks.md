...
```typescript
type Options = {
  defaultUser: { email: string; password: string }; // pragma: allowlist secret
};
...
export const test = base.extend<Options & Fixtures>({
  ...
  defaultUser: [
    { email: "test@example.com", password: "pass123" }, // pragma: allowlist secret
    { option: true },
  ],
  ...
  authenticatedPage: async ({ page, defaultUser }, use) => {
    ...
    await page.getByLabel("Password").fill(defaultUser.password); // pragma: allowlist secret
...
export default defineConfig({
  use: {
    defaultUser: { email: "admin@example.com", password: "admin123" }, // pragma: allowlist secret
  },
});
```
