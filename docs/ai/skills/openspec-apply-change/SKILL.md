...
allowed-tools: Bash(openspec:*), Bash(bun:*)
license: MIT
...
- Numbered commands `/1-start-project` through `/4-close-project` wrap these
  workflows.

### Workflow

1. Read `openspec/config.yaml`, `openspec/project.md`, and the relevant specs
   or active change.
2. Invoke `bun run openspec --` with the explicit change ID for mutating work.
3. Keep Core TDD, UI, data-boundary, and Guardian rules in force.
4. Leave the change active until implementation has merged.

### Checklist

- [ ] Local pinned CLI used via `bun run openspec --`
- [ ] Explicit change ID used for mutating operations
- [ ] Stores were not introduced
- [ ] Incomplete or unmerged work was not archived
- [ ] `openspec update` was not run against the live repo
- [ ] Product Eve was not treated as a coding agent
