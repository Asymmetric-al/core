...
Typography guidance is subordinate to Core's type tokens and
existing `packages/ui` type ramp.
...
- Choosing or tightening type, wrapping, OpenType, or tabular numbers.
- Do not swap Core fonts because this skill names a different family.

### Workflow

1. Inspect current Core type tokens before changing families or sizes.
2. Keep `base-maia` and Zinc-oriented semantic tokens.
3. Pair accessibility text concerns with `better-accessibility` and
   `accessibility-review`.

### Checklist

- [ ] Core type tokens won over generic font recommendations.
- [ ] Product apps were not restyled onto a new type system.
