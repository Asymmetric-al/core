...
This skill is explicit-only mobile web craft for viewport, touch, scroll, and
safe-area tells. Core's product apps are Next.js. Reconcile this overlay after
upstream refreshes before running `bun run skills:sync`.
...
- Keep `disable-model-invocation: true`. Do not load it for ordinary desktop
  UI, motion implementation (`animate`), or Expo (`animate-expo`).

### Workflow

1. Confirm the request is actually mobile-web native-feel before applying this
   file.
2. For Core Next.js apps, `docs/ai/rules/frontend.md`, Base UI / `base-maia`,
   and shared `@asym/ui` tokens still win over illustrative upstream CSS.
3. Never disable zoom. Prefer capability queries over user-agent sniffing.
4. Hardware verification is required for sticky hover, tap delay,
   rubber-banding, safe areas, and the keyboard.

### Checklist

- [ ] Invocation was explicit, or the request is specifically mobile-web
      native-feel rather than ordinary frontend work.
- [ ] Core frontend, Base UI, and accessibility contracts remain authoritative.
