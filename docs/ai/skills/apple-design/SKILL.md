...
Use this upstream skill as a specialized physical-interaction reference. Core's
frontend architecture and motion contract remain authoritative. Reconcile this
overlay after upstream refreshes before running `bun run skills:sync`.

### Triggers

- Gesture-driven UI, momentum, rubber-banding, drag or swipe interactions.
...
1. Read `docs/ai/rules/frontend.md`, then
   ...
   `docs/ai/skills/anim/SKILL.md` as applicable.
2. Reuse Base UI primitives and Core motion tokens; never introduce Radix
   imports, `--radix-*` variables, or parallel duration/easing scales.
3. Keep the global reduced-motion baseline in
   `packages/ui/styles/globals.css`; TS motion must use the repo's
   `useReducedMotion()` contract.
4. Use this skill for physical behavior and feel, then use `motion` only for
   `motion/react` API details.

### Checklist

- [ ] The interaction is direct, interruptible, and velocity-aware where useful.
- [ ] Base UI, shared tokens, accessibility, and reduced-motion rules win over
      illustrative upstream values.
- [ ] Gesture behavior is verified on a real or representative touch device.
