...
Use this skill to implement web motion after Core's design-engineering and
operative motion contracts. Reconcile this overlay after upstream refreshes
before running `bun run skills:sync`.

### Triggers

- Building or rewriting a web animation, transition, or micro-interaction.
- Do not use it for Expo/React Native (`animate-expo`), a codebase audit
  (`improve-animations`), or a strict motion-only review (`review-animations`).

### Workflow

1. Load `docs/ai/skills/emil-design-engineering/SKILL.md` and
   `docs/ai/skills/anim/SKILL.md` first.
...
- [ ] Shared utilities and `@asym/lib/motion-presets` are preferred over new
      timing literals.
