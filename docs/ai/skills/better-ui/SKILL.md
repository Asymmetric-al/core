...
Use this polish reference only after Core's existing UI
system. Do not introduce a second component library or restyle product apps
from generic upstream values.
...
- Polishing existing Core UI details (radius, surfaces, icons, hit areas).
- Do not use this to pick a new design system or restyle an app.

### Workflow

1. Load `docs/ai/rules/frontend.md` and `packages/ui` first.
2. Reuse existing tokens and Base UI primitives. Do not fork radii, shadows,
   or motion scales from illustrative upstream numbers.
3. Keep animation work on `emil-design-engineering` + `anim`.

### Checklist

- [ ] Existing `base-maia` / Base UI ownership is preserved.
- [ ] Upstream literals did not replace Core tokens.
