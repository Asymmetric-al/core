...
| `receipt.number` / `receipt.version`                   | immutable receipt identity and version                                                                                                                                                                                    |
| `receipt.issued_at` / `receipt.gift_date`              | `FrozenInstant` / `FrozenDate` from the receipt version                                                                                                                                                                   |
...
| `receipt.currentness`                                  | closed `current` or `superseded` source value; only a current artifact can supply a new-download action                                                                                                                   |
...
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `receipt.open_current@1`              | Phase 7 opens the exact current receipt artifact after current tenant/Party/artifact authorization reproof; GET never issues, replaces, corrects, or acknowledges anything |
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `surface.donor_official@1`    | System Messages catalog/editor/preview/version history; donor email; donor-safe Phase 6 history; staff body-free history; optional eligible Recent copy; Phase 7 artifact surface    | authoring uses `system_messages.read`, `system_messages.draft.edit`, `system_messages.commit`, and the applicable exact `system_messages.publish.standard`, `system_messages.review.protected`, or `system_messages.publish.protected` capability; Recent copy uses `system_messages.recent_copy.reveal`; donor artifact read uses current Phase 7 Party/artifact authority |
...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `retention.official_receipt@1`      | `official`, at least seven years and preferably permanent under the Phase 3/6 policy version; body-free event plus immutable Phase 7 artifact reference            | tenant chooses 30 days (Recommended/default), 7 days, or Off; 30-day ceiling; protected actions inert | source issuance/supersession, publication/review, preparation, provider crossing/outcome/correction, authorized reveal/denial, suppression decision, retention purge                        |
...
| ----------------------------------- | ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | --------------------- |
| `giving_receipt_issued_v1`          | `profile.receipt_required_email@1` | source `phase7.receipt.issued@1`, fence `{receipt_id, receipt_version}`; required `recipient.safe_display_name`, `receipt.legal_donor_display_name`, `receipt.legal_donor_address`, `receipt.organization_legal_name`, `receipt.organization_tax_identifier`, `receipt.number`, `receipt.version`, `receipt.issued_at`, `receipt.gift_date`, `receipt.gross_amount`, `receipt.deductible_amount`, `receipt.goods_services_statement`, `receipt.currentness`, `receipt.designation_lines`; conditional FMV, religious-benefit text, and in-kind description under their Phase 7 cases; optional none | `receipt.open_current@1`; protect issuing identity, legal donor, receipt identity/version, gift date, gross/deductible money, goods/services result, currentness, and artifact action                                                                                                                      | `forbid.external_sensitive@1`, `forbid.required_message_marketing@1`, `forbid.receipt_cross_party@1` | `P17-KEY-RCPT-ISS@1`  |
| `giving_receipt_replaced_v1`        | `profile.receipt_required_email@1` | source `phase7.receipt.replacement_issued@1`, fence `{receipt_id, receipt_version, superseded_version}`; all issued-receipt required/conditional facts plus `receipt.superseded_number_version` and `receipt.replacement_reason_code`; optional none                                                                                                                                                                                                                                                                                                                                                | `receipt.open_current@1`; issued core plus explicit replacement/supersession identity; prior artifact is never rewritten                                                                                                                                                                                   | same as receipt issued                                                                               | `P17-KEY-RCPT-REPL@1` |
| `contribution_receipt_corrected_v1` | `profile.receipt_required_email@1` | source `contribution.receipt_correction_artifact_issued@1`, fence `{operation_id, operation_outcome_version, receipt_id, receipt_version}`; recipient resolver remains `phase7.legal_donor_delivery_contact@1`; requiredness overrides only to `source_required_with_audited_suppression@1`; all replacement-receipt required/conditional facts plus `correction.operation_reference` and `correction.effective_at`; optional `correction.personal_note`                                                                                                                                            | `receipt.open_current@1`; replacement core plus source operation reference; this key is used only when the `contribution.receipt_correction_artifact_issued@1` source occurrence produced the replacement artifact and cannot duplicate `giving_receipt_replaced_v1` for the same semantic source identity | same as receipt issued plus `forbid.correction_internal@1`                                           | `P17-KEY-RCPT-CORR@1` |
...
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `giving_receipt_issued_v1`                                    | `phase7.receipt.issued@1` → `recipient_email` → `email/service`                                                          | required after Phase 7 eligibility; `phase7.legal_donor_delivery_contact@1`                                                                                       |
| `giving_receipt_replaced_v1`                                  | `phase7.receipt.replacement_issued@1` → `recipient_email` → `email/service`                                              | required after Phase 7 eligibility; same resolver                                                                                                                 |
| `contribution_receipt_corrected_v1`                           | `contribution.receipt_correction_artifact_issued@1` → `recipient_email` → `email/service`                                | source-required/audited suppression; `phase7.legal_donor_delivery_contact@1`                                                                                      |
...
`841934d5910ed8a3a2722742c696b1fd18a22b92` plus the dated Phase 17 planning
package. It contains:
...
Every `ADR-0022` through `ADR-0029` identifier in this Phase 17 companion refers
...
| D4       | canonical structured document/compiler              | ADR-0027      | One Structured Document Produces Accessible Email Artifacts        | `P17-D04-*`    |
...
| D6       | producer-owned protected actions                    | ADR-0022      | Protected Actions Remain Producer-owned And Scanner-safe           | `P17-D06-*`    |
| D7       | bounded Delivery Plans                              | ADR-0023      | Delivery Plans Are Bounded And Compile Through Channel-owned Seams | `P17-D07-*`    |
| D8       | one in-product presentation/engagement model        | ADR-0024      | In-product Notifications Are One Role-safe Attention Projection    | `P17-D08-*`    |
| D9       | transport-dark SMS evidence boundary                | ADR-0025      | SMS Governance Is Evidence-ready While Transport Is Unavailable    | `P17-D09-*`    |
| D10      | tenant-owned Resend connection                      | ADR-0026      | Every Tenant Uses Its Own Proved Resend Connection                 | `P17-D10-*`    |
...
| D13      | Brand Kit, Role Layout and Saved Sections           | ADR-0027      | One Structured Document Produces Accessible Email Artifacts        | `P17-D13-*`    |
| D14      | body-free history and expiring Recent copy          | ADR-0028      | Communication History Is Durable Evidence Without A Body Archive   | `P17-D14-*`    |
| D15      | sealed preparation and whole-message recovery       | ADR-0029      | Preparation And Recovery Preserve One Recipient-specific Identity  | `P17-D15-*`    |
| D16      | census, executable catalog, bindings and activation | none required | One Code-governed System-message Catalog                           | `P17-D16-*`    |
| D17      | bounded reply purposes/destinations                 | ADR-0026      | Every Tenant Uses Its Own Proved Resend Connection                 | `P17-D17-*`    |
| D18      | editable-by-default minimum truth core              | ADR-0027      | Message Contracts Bound Tenant Freedom                             | `P17-D18-*`    |
...
| D20      | default plus bounded Sender Profiles                | ADR-0026      | Every Tenant Uses Its Own Proved Resend Connection                 | `P17-D20-*`    |
