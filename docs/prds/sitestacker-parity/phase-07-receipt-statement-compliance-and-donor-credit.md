...
- `statement_run_items` — per-gift statement line (donation*id XOR gift_credit_id, credit_type, `included_on`, effective/deductible/QPQ amounts, frozen_effective_revision). *(Amended 2026-07-10, Phase 14 (Donor Credit Operations) D1.14: the XOR becomes `contribution_header_id XOR contribution_credit_id` — the Phase 13 (Campaign, Designation, Contribution Ledger & Giving Cart) ledger header replaces `donation_id`, and `gift_credit_id` renames with its table.)\_
...
- `contribution_receipt_snapshots` / frozen legal-donor snapshot — the Phase-4-shipped **interim** frozen-donor record, extended party-kind-aware here; **not a second system of record** (see A3): `{donor_id, party_kind (+ org_type for org-kind legal donors), legal_name, EIN(org), address-of-record, household_id+member_ids}` — the snapshot stores a frozen copy of the resolved `party_kind`/`org_type` at issuance, never live dual truth (renamed from `party_type`/`org_subtype` 2026-07-06 per Phase 9 §C2; the shipped snapshot carries designation lines but no legal-donor identity yet). Sequencing (mirrors Phase 4 §G2): Phase 4's snapshot extension is the interim frozen-donor record (ships with Phase 4, satisfies #507 pre-Phase-7); once `contribution_receipts` ships it embeds the same frozen identity at issuance and becomes the sole authority, the snapshot row demotes to a legacy annotation, and the #506 guard trigger keys on whichever frozen record exists for the donation (both during the overlap).
