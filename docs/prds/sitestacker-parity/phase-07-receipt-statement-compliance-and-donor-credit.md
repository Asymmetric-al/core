...
5. **The tax year is derived wrong — offline gifts and the year boundary break.** `gift_date` is filtered in **UTC**, and `statementYear` is computed with `getUTCFullYear()`, so a Dec-31-11pm-Eastern online gift is mis-bucketed to the next year. There is no offline gift entry at all, no concept of exact-issuer date-of-delivery rules (under the verified U.S. mailed-check policy, the actual mailing date governs and a captured postmark is one evidence source—not the written or received date), and no governed way to record a January-arriving December check in the correct prior year.
...
7. **Correct tax year by stored date-of-delivery.** The exact issuer's versioned Phase 7 jurisdiction-dating contract resolves and stores one delivery date at ingestion/entry in the issuer tax timezone. For a verified U.S. issuer under the U.S. rule set, card/ACH use settlement, USPS-mailed checks use a proved mailing/postmark date, and hand-delivered/cash/in-kind/private-carrier gifts use received. Other issuer packs supply their own closed rules; a missing or unsupported policy fails closed. `gift_date` is never recomputed on read, and `delivery_basis` records which input governed. An **offline gift-entry** flow captures the contract-permitted evidence, shows the computed tax year before save, and warns on a year-boundary straddle. A legitimate prior-year date uses append-only staff attestation by default; tenant or jurisdiction policy may strengthen evidence/review. This fixes the shipped UTC year-bucketing bug without creating universal January ceremony or a hidden U.S. fallback.
...
8. As a **finance operator**, I want to enter an offline gift and see the tax year resolved by the exact issuer's jurisdiction-dating contract before I save, so that a U.S. January-arriving December-postmarked check lands in the right year without imposing the U.S. rule on another jurisdiction.
...
- **A14 — Tax year = one resolved, stored date-of-delivery in the exact issuer's tax timezone; never recomputed on read.** The exact issuer's versioned Phase 7 jurisdiction-dating contract resolves and freezes `legal_issuer_id`, jurisdiction-policy identity/version/digest, the IANA timezone used, `gift_date`, `delivery_basis`, evidence identity, and `tax_year` into an immutable `contribution_dating_facts` revision. The first revision is written in the same transaction as the contribution; an authorized correction appends exactly one successor against the expected current revision. For a verified U.S. issuer under the U.S. rule set, card/ACH use settlement; a USPS-mailed check uses a captured postmark or a distinct staff-attested actual mailing date rather than the written date; and hand-delivered/cash/in-kind/private-carrier gifts use received. `postmark` and `mailing_attestation` are different basis codes and different evidence facts. Other issuer packs supply their own closed rules. A missing jurisdiction contract or issuer timezone produces a typed blocked result; neither currency, donor address, Site, nor locale may infer U.S. behavior. (D6.)

- **A15 — Prior-year dating is append-only, staff-attested by default, and policy-strengthenable.** True prior-year dating (`gift_date` year < entry year) requires an auditable staff attestation with the asserted date, permitted basis/evidence, reason, actor, and time; a postmark or other evidence may be retained when available. A tenant or jurisdiction contract may require stronger evidence, a bounded cutoff, or independent review, but those are not universal defaults. A correction appends one complete `contribution_dating_facts` successor under expected-revision compare-and-set; it never updates the contribution header, an earlier dating fact, or a money posting. The effective fact is the unique leaf of the linear revision chain, and a correction cannot change the frozen legal issuer or tender. Phase 7 emits a pointer-only source event so Phase 19 can stale or route only affected work. If an immutable primary statement release already exists, Phase 19 coordinates the correction through its late-fact lane rather than changing the frozen run or artifact in place. (Phase 19 D5.)
...
- **B4 — Date-of-delivery resolver.** Given exact issuer, versioned jurisdiction-policy identity/digest, method, permitted delivery evidence, and verified issuer timezone, returns `{ status: resolved, gift_date, delivery_basis, tax_year, source_pins } | { status: blocked, reason_code }`. The function is pure and total over its typed result; callers may persist money only after a resolved dating fact exists. Initial resolution and every authorized correction append an immutable `contribution_dating_facts` revision. Expected-revision compare-and-set and a unique successor constraint keep one linear chain and one effective leaf. The A14 core.
...
- `contribution_dating_facts` — the Phase 7-owned immutable occurrence-grain linear revision authority. Required fields: `id`, `tenant_id`, `contribution_header_id`, immutable `legal_issuer_id`, immutable `gift_method`, `revision`, nullable `supersedes_dating_fact_id`, jurisdiction-policy identity/version/digest, resolved IANA timezone, distinct permitted evidence/attestation identity and `delivery_basis`, `gift_date`, `tax_year`, `resolved_at`, source-fact digest, actor/reason for corrections, and semantic idempotency key. Enforce `UNIQUE (tenant_id, contribution_header_id, revision)`, `UNIQUE (tenant_id, supersedes_dating_fact_id)`, and same-tenant/exact-issuer composite foreign keys. `UPDATE`/`DELETE` are forbidden. Concurrent corrections use expected-revision compare-and-set; one occurrence cannot fork into two current dating facts. The first revision is created by the locked contribution commit seam, and every consumer pins a fact identity/revision/digest rather than recomputing from mutable policy.
- `contribution_receipt_plan_facts` — the Phase 7-owned, immutable occurrence-grain authority for an exact issuer with an active Canadian pack. Exactly one row is created for each eligible cash occurrence under that pack, unique by `(tenant_id, contribution_header_id)`, with `id`, `legal_issuer_id`, exact pack-publication identity/digest, `plan_code ∈ {individual_cash, annual_cumulative_cash}`, activation epoch, policy revision, and `resolved_at`. A composite same-scope FK `(tenant_id, contribution_header_id, legal_issuer_id)` references the immutable issuer binding on the contribution header, so one occurrence cannot acquire a second issuer. The Phase 7 resolver derives the issuer from that committed occurrence and creates the row in the same transaction as source intake/posting; `UPDATE`/`DELETE` and caller/UI-supplied issuer or plan values are forbidden. It is structurally absent only when the exact issuer has never activated a Canadian pack. A repairable issuance pause preserves the active plan epoch and continues freezing source plan facts while generation/issuance is held. A confirmed legal lock or ended pack closes the epoch: later occurrences receive no issuable plan fact and are never queued for retroactive receipts. Reactivation requires fresh proof and starts a new epoch. Every transition preserves all immutable historical plan facts, receipts, coverage, serials, and audit evidence read-only. The plan fact is not itself a receipt, coverage record, document, or communication occurrence, and later plan changes create only future rows.
...
- contribution occurrence/header (Phase 13 replaces legacy `donations`): immutable `legal_issuer_id` and `gift_method` (TEXT+CHECK), plus `UNIQUE (tenant_id, id, legal_issuer_id)` for same-scope dependent facts. The locked commit seam creates the required initial `contribution_dating_facts` row in the same transaction. Delivery evidence, policy pins, resolved timezone, `gift_date`, and `tax_year` live only in that versioned authority; `entry_date = created_at::date` remains non-authoritative.
...
- exact issuer compliance profile: `tax_timezone` (IANA; no database or application default) and optional prior-year policy. Onboarding or jurisdiction-pack activation must verify it; date resolution fails closed while it is absent.
...
- **E3 — Single-gift issuance path.** Wire source-confirmed tender finality plus prospective plan/policy-admitted issuance and void/replace-on-negative into the Stripe event processor (durable, not inline in the webhook), keyed on contribution identity + frozen source facts so the staged-gift delivery path calls **into** the shared issuance, and manual/imported/historical gifts without a staged-gift record can be receipted.
...
- **F7** — Gift tax year = one date-of-delivery resolved and stored by the exact issuer's versioned jurisdiction contract in its verified tax timezone; the U.S. contract uses proved USPS mailing/postmark evidence for mailed checks, while other jurisdictions never inherit that rule. Backdating is append-only and policy-strengthenable; no fiscal-close engine.
...
- A partial refund **net-reduces** (never drops); a full refund/void appends the Phase 7 validity/correction effect; an ACH `charge.dispute.created` after issuance emits exactly one source correction/coverage effect. Phase 18 alone creates any successor/current artifact and Phase 19 coordinates any affected statement operation.
- Phase 17 re-notifies a source-authorized successor **exactly once per version**; a same-version retry dedupes; a late `sent` after `bounced` is a no-op.
- Date-of-delivery: under a verified U.S. issuer contract, a check postmarked 2025-12-30 / received 2026-01-03 → **tax year 2025** and a check dated 2025-12-31 / postmarked 2026-01-02 → **2026**; an online gift resolves in the exact issuer's verified tax timezone; missing or non-U.S. issuer policy never falls back to these U.S. rules.
...
- **A fiscal-close / accounting-period engine** — calendar year plus the exact issuer's verified tax timezone only; no per-tenant fiscal-year-start, period-locking state machine, or GL close/reopen (a cutoff date plus governed high-risk action covers "closed").
...
- **{{T7}}** — Date-of-delivery resolver (B4); immutable `contribution_dating_facts` linear revision schema and constraints; immutable header legal-issuer/tender binding; distinct postmark, mailing-attestation, received, or settlement evidence and closed basis; jurisdiction-policy identity/version/digest; verified IANA issuer timezone with no default; `gift_date`; `tax_year`; resolution time; source digest; expected-revision correction CAS; pointer-only correction event; UTC-boundary fix; and exhaustive jurisdiction/tender/concurrency tests. _(foundation-ish)_
- **{{T8}}** — Offline gift-entry flow + prior-year dating governance (auditable staff attestation by default, optional policy-strengthened evidence/review, and append-only dating-fact successor). _(blocked on T7)_
- **{{T9}}** — Fund deductibility policy + lean per-gift/per-line tax facts + resolver (B8). _(foundation-ish)_
- **{{T10}}** — Receipt eligibility evaluator (B1) + exact-issuer prospective receipt-plan resolver and immutable occurrence-grain `contribution_receipt_plan_facts` seam + source-finality/plan-admitted issuance and void/replace state machine + `receipt_status` formalization. _(blocked on T7, T9)_
