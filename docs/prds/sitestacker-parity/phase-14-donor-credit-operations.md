...
15. As a **finance staffer**, I want acknowledgment purpose/readiness/coverage in separate Phase 14 owner records joined to the gift — `{not_applicable, held, ready, released, canceled}` — with an "Unacknowledged" work view joined to exact Phase 6 outcomes, so that no thank-you is fire-and-forget without Phase 14 claiming communication outcomes. `[D2.g3]`
...
34. As a **notify party watching two tributes** ("Mom and Dad"), I want coverage keyed on (canonical recipient Party, contribution header) so I'm told about each gift exactly once, so that overlapping tributes and merges never double-mail me. `[D3, D3.1]`
...
51. As a **donor-care staffer**, I want every deceased notify recipient blocked at compose and send, and honor-honoree death to pause honor and route its memorial successor without treating a deceased memorial subject as the recipient, so that notifications reach only living, otherwise qualified people. `[D3.8, D3.12]`
...
53. As a **donor-care staffer** flipping honor→memorial, I want close-and-guided-successor (notify preferences re-confirmed, no copying old letters/items or automatic gift-link inheritance, existing canonical recipient Party/header claims preserved), type immutable after the first letter, and a deceased honoree auto-pausing the honor stream with a conversion task, so that the saddest transition is handled with care. `[D3.12]`
...
- **One locked function, one lock.** All credit mutations go through **one locked `SECURITY DEFINER` function taking the SAME per-contribution advisory lock as the current P13 owner command** (FORWARD: Phase 13 PRD §G source serialization; epic #690). Credits serialize with money movements on the same header; there is no second lock namespace to deadlock against.
...
3. **Acknowledgment purpose/readiness/coverage is Phase 14-owned operational state joined to the gift** — `{not_applicable, held, ready, released, canceled}` (TEXT + CHECK) — plus an **"Unacknowledged" work view joined to exact Phase 6 outcomes**. `released` means Phase 14 froze and handed off the exact owner request; it never means sent or delivered. Natural mappings: identified → `ready` after the hold window → `released`; ambiguity, staff hold, new-party hold, or "Not Provided" → `held`; Anonymous tier → `not_applicable`; reattribution or withdrawal can move unstarted work to `canceled`. Phase 6 outcomes never rewrite this axis. The purpose/request record and its exact same-Tenant source relations carry state, hold timing and revision; acknowledgment transitions never update the frozen contribution header or settlement source facts (C-02, 2026-09-23).
...
Every gift whose **current qualified Phase 14 attribution** is "Not Provided" lands in a **finite, owned worklist** — the Attribution Inbox (the QBO For-Review/month-end pattern). The tenant dashboard shows an **attribution completeness %** (the LinkedIn completeness-bar effect, ~+50% completion in the cited research) — **no per-record nag emails, ever**. Late identification → the authorized Phase 14 command updates separate current-attribution state and invokes the existing qualified credit mutation path, preserving frozen header capture and historical attribution → the system offers **ONE consolidated thank-you covering all unthanked grants in the window** (never a burst of N back-dated letters). The Inbox is the same worklist idiom Sections G and H reuse (matching-gift aging, tributes awaiting setup) — one pattern, three consumers.
...
- **Email:** Phase 14 submits its exact purpose, source occurrence, audience, readiness and frozen facts to the current Phase 17 message contract and Delivery Plan; Phase 6 owns consent and dispatch. The old `email_template_system_bindings` table and `tribute.digest` / `tribute.honor_immediate` names are prototype/cutover evidence, not current stable-key or template authority. The Phase 17 census keeps tribute, advisor and matching acknowledgment meanings deferred until their exact producer/recipient/facts contract is admitted. Qualify the owning catalog entry and Live activation generation before sending; an absent or Reserved key creates no Phase 6 state or later historical catch-up. This consumes the same source-admission boundary as F.7 and introduces no Phase 14 template store.
...
- **`tribute_notification_items`** — the **coverage ledger** (new; no Phase 7 ancestor). `recipient_party_id` identifies the actual canonical recipient Party; `notify_party_id` separately references the exact `tribute_notify_parties` stream-preference row. Keep `tenant_id, letter_id, tribute_id, header_id` and frozen `rendered_donor_as` / `letter_total_minor` facts. Coverage is **`UNIQUE (tenant_id, recipient_party_id, header_id)`**, never the per-tribute preference-row identity. Same-Tenant composite relationships must prove that the claimed Party is the authorized recipient of the referenced preference row and letter; an authorized recipient override resolves that exact recipient/provenance before the claim.
...
**Adjudication — the grain is `(canonical recipient Party, header)`, not `×tribute` or a stream-row ID:** one Party may have different `tribute_notify_parties` rows for Mom and Dad. Those rows retain independent preferences and provenance but share the same Party/header claim. Atomic claim/letter composition and database uniqueness prevent duplicate coverage under either concurrent commit order. Tombstone/re-add and Party/tribute merge must preserve earlier coverage, immutable presentation history and the applicable strictest suppression; none resets the claim into a new send. `tribute_id` remains framing, not coverage identity. The letter period anchor still includes the exact stream-preference row, as D3.2 requires.

An admitted cancellation of a prepared/held request **before source release** cancels the purpose and reopens its items in the same expected-state/revision transaction [D3.1]. It races source release with one durable winner. Released, submitted or uncertain outcomes require source/Phase 6 receipt-backed recovery; a cancel control, timeout or replay never resets already-released coverage or creates an automatic retraction letter.
...
- **Pre-release cancellation only:** the source command may atomically cancel a proved unstarted prepared/held purpose and reopen its coverage under the shared release/cancel guard. Released/submitted/uncertain custody follows current owner recovery, not coverage reopening or a claimed recall (F.1 and the ownership matrix).
...
- **Deceased checks are type-aware and run at compose and send.** Every deceased notify recipient is blocked. A deceased honor honoree pauses the honor stream and routes F.12’s guided memorial successor. A deceased memorial subject is not itself a recipient and does not alone block a memorial notification to living, otherwise admitted recipients. Re-prove the current facts because days can pass between composition and fulfillment.
...
- **Type flip honor→memorial** (the honoree died) = **close + guided successor**: the flow creates the memorial tribute, **carries notify parties across with re-confirmation** (each must be re-affirmed — grief changes who wants mail), and **does not copy old letters/items or automatically inherit gift links** (the memorial stream starts fresh). Existing canonical Party/header coverage claims remain historical facts: a new stream is not a deduplication exemption, and deliberately linking an already-covered gift cannot manufacture another notification. `tribute_type` is **immutable after the first letter**; a deceased-marked honoree **auto-pauses** an honor stream and routes a conversion task — the platform never mails a celebratory letter to a dead man's address.
...
**The D3 poison-fixture set (9+, consolidated with the phase-wide inventory in the Testing & Poison Fixtures section):** forbidden amount/deductibility facts refuse at the Phase 17 and Phase 18 public seams; `never` = zero sends forever (including composed-then-flipped); deceased-recipient blocked at compose and at send; restricted-party invisibility end-to-end (and restricted-donor crowd-blend rendering); import → zero sends even after tenant enable; crash-retry creates exactly one covered letter occurrence; two tributes + one watcher + one gift ⇒ told once (the canonical `(recipient_party_id, header_id)` grain); only admitted pre-release cancellation reopens items; fold-below-floor ⇒ total omitted + task.
...
- Duplicate resolution = **supersede-merge**: the loser moves to `superseded` with `merged_into_id`; settlements re-point to the survivor; nothing is deleted. This qualified audited re-point remains allowed under C-02; it is not an acknowledgment transition or financial-row rewrite. Preserve settlement identity, original request/source/audience history and prior acknowledgment coverage; merge cannot reopen coverage or send another thank-you.
...
Phase 13 already owns `is_daf_grant`, `no_quid_pro_quo`, and `daf_pledge_no_sponsor_reference` on the header (Phase 13 PRD §Data Model, DAF-shape bullet). Phase 14 **extends the header with entry-time capture facts**. They freeze with posting, including the initial attribution outcome/reason; they are not the current operational attribution or Inbox state. Late identification and qualified reattribution use the separate Phase 14 current-attribution records below, plus the existing credit mutation/supersede rules:
...
- daf_attribution_state — text — CHECK ∈ {identified, not_provided, anonymous}: the required-combobox
  INITIAL entry outcome only. An identified entry references its daf_advisor credit; not_provided
  seeds the current owner attribution state. The Inbox uses current qualified attribution, not this
  frozen historical value after late identification or reattribution — [D2, C-02]
- daf_attribution_reason — text NULL — the one-tap reason chip; required when
  the INITIAL daf_attribution_state = 'not_provided'; later reasons stay with the separate current
  attribution owner record/history, never overwrite this posted capture — [D2, C-02]
...
### Current DAF attribution — separate owner state (C-02)

Initial `daf_attribution_state`/reason remain frozen entry evidence on the posted
header. Current identification, primary attribution-party selection and reason
are Phase 14-owned operational facts in separate records related to the exact
same-Tenant contribution and existing qualified credit references. They carry
current revision and audited source provenance; they do not duplicate money or
replace the credit spine. No physical table-count cap may omit this ownership
split. Initial entry seeds this state in its accepted transaction; late
identification and reattribution use current authorization, expected revision
and existing credit freeze/supersede/Party-merge rules, preserving prior capture
and evidence. Accepted state/credit/audit and required outbox facts commit
atomically; retry reuses the existing semantic operation identity.

The Attribution Inbox and completeness tile derive from that current qualified
attribution, with the same existing identified/not-provided/anonymous semantics.
A historic not-provided header does not strand a resolved gift in the Inbox.
Missing or stale owner state is a proof failure, not permission to fall back to
an old header value, infer identification, bypass Anonymous handling or send.
D.12's primary attribution party remains the acknowledgment audience; additional
co-advisor credits do not create extra automatic letters. Current-state changes
preserve the existing D.6 consolidated-offer and D2 hold/import/consent gates.

### Acknowledgment purpose/request records — the D2 guardrail-3 axis

**Founder ruling (C-02, ratified 2026-09-23): separate operational records;
keep posted rows immutable.** Phase 14 owns acknowledgment purpose/request
records and exact source relations in Asym Postgres. These are separate from
Phase 13 contribution headers/lines/postings and matching-settlement source
facts. The earlier mutable acknowledgment/hold columns and work-view index on
`contribution_headers` are superseded; there is no whitelisted posted-header
update or mutable-state condition that can reopen a posted-fact guard. See
[ADR-0206](../../adr/0206-immutable-posted-financial-facts-and-owner-operational-state.md)
and the [decision record](../../ai/audits/2026-09-23-contract-congruence.md#c-02-posted-state).
This is the target storage contract, not a claim that the records have shipped.

Required logical record and relation shapes (concrete SQL names may follow the
implementation's house conventions; neither a fixed table count nor one
header per request may erase the already-admitted D.6 consolidated request):

```
Phase 14 acknowledgment purpose/request record
- id / tenant_id — house identity and composite same-Tenant relationships
- exact purpose/semantic occurrence identity, audience and covered source set —
  retry reuses the accepted request; no duplicate acknowledgment from worker replay
- acknowledgment_request_state — TEXT + CHECK:
  {not_applicable, held, ready, released, canceled}
- acknowledgment_hold_until — timestamptz NULL — existing ~10 minute default,
  tenant configuration and longer first-send hold remain unchanged [D2 g1/g6]
- current_state_revision — advances under the owner command's expected-state/revision guard
- stable purpose request identity and durable transition/command receipt — link downstream
  owner outcomes without copying their statuses or reopening frozen source facts

Phase 14 exact acknowledgment-source relations
- tenant_id / request identity / contribution header identity — composite same-Tenant FKs
- exact matching settlement identity when applicable, proving its corresponding header/line
  through the existing composite same-Tenant settlement/source constraints [D4.10]
- required coverage and uniqueness constraints — preserve one attribution-party acknowledgment
  per grant, per-settlement employee meaning and D.6's one consolidated thank-you covering
  multiple unthanked grants; never collapse a batch employer check's employees into one status
- exact admitted source set and audience frozen at release; retained relations preserve
...
source set/audience/facts, advances the request revision/state, records the
accepted command receipt and writes the durable source outbox. Cancellation of
unstarted work races that release under the same guard. Replays converge on
the durable receipt; submitted/released/uncertain outcomes use exact owner
recovery rather than clearing coverage or pretending a recall. No mutation of
a frozen contribution or settlement fact is part of any acknowledgment
transition. The record/relation constraints and fail-closed census entries
ship before a dependent generator, including privileged cross-Tenant denial.

`released` means that Phase 14 froze and handed off the exact owner request;
it does not mean sent or delivered. The **Unacknowledged** work view joins the
Phase 14 records and source coverage to exact Phase 6 event/outcome relations
and Phase 18 artifacts where applicable. No communication or artifact outcome
is copied into Phase 14. This remains a derived work view over owner records,
not a separate proofreading/review queue.

The Phase 15 NF3 gate remains unchanged: a batch/import-origin acknowledgment
request starts `held`, with `batch_gate_pending` derived from the gift's
existing frozen entry-origin metadata. The explicit per-batch
**Send-acknowledgments** command acts on the separate request records under
their current revisions; it never patches the posted gift's origin or state.
Imports remain held and never pass that gate. No new origin column is added.
...
- recipient_party_id — uuid NOT NULL — composite same-Tenant FK → parties; the actual recipient
  used by the Party/header coverage key, including an authorized override — [D3.1]
- notify_party_id — uuid NOT NULL — exact tribute_notify_parties stream-preference row provenance,
  not coverage identity. Composite same-Tenant relationships bind this row to its recipient Party
  and to the referenced letter; a mismatched Party/stream/letter is rejected under privileged writes.
  Required matching composite UNIQUE/FK targets are schema constraints, not browser assertions.
...
- status — text NOT NULL — CHECK ∈ {covered, skipped_reversed, reopened}: an admitted PRE-RELEASE
  cancellation reopens its items atomically under the source guard (never a delete); later qualified
  recomposition retains recipient identity, records exact stream/letter provenance and freezes its facts;
...
COVERAGE GRAIN: UNIQUE (tenant_id, recipient_party_id, header_id) — a "Mom and Dad" watcher following
...
- Employee acknowledgment state/hold timing are NOT columns on this settlement fact: separate
  Phase 14 purpose/request records reference the exact same-Tenant settlement/header/line through
  the acknowledgment-source relations above; operational transitions preserve the settlement — [D4.10, C-02]
...
No new audit table. `contribution_operation_audit_events` (REAL: `supabase/migrations/20260526132000_contribution_operations_core.sql`) gains a **nullable polymorphic subject**: `subject_type` TEXT + CHECK over the Phase 14 record types (`contribution_credit`, `matching_gift_expectancy`, `matching_gift_settlement`, `tribute`, `contribution_tribute`, `tribute_notify_party`, `tribute_letter`, `affiliated_party_credit_rule`, `daf_sponsor`, `party_payer_alias`, `credit_generation_run`, plus explicitly registered C-02 current-attribution and acknowledgment-request/source-relation subjects) plus `subject_id uuid`, with the pair validated by that migration's **existing tenant-refs guard-trigger pattern** (the composite-tenant-FK posture, extended — the trigger already exists on this table). Pre-`received` expectancy transitions, credit supersedes, preference changes, and `never` unsets all get an audit home without a second spine [D1.5, D4.12]. Before any new C-02 owner command writes, extend the closed governed subject registry, CHECK and existing guard with its exact record identity and same-Tenant source-reference validation; implementation spellings must map unambiguously to the qualified record shapes. Atomic attribution/request transition receipts and audits must be representable without a generic unchecked subject or second audit table. Existing credit freeze/supersede and qualified Party/expectancy merge exceptions remain unchanged.
...
| DAF grant capture facts (identity tier, grant id/fund/purpose, attribution state) | Asym Postgres (frozen P13 entry capture; separate P14 current-attribution owner records/credit references)               | Gift entry freezes capture; qualified P14 current-revision attribution command handles later changes | Asym; sponsor paperwork = evidence                                                                                                                    | Qualified reattribution/credit rules preserve initial capture and historical evidence (+ rule update/delete + owner-qualified correction offer) |
...
| Acknowledgment purpose/request records + exact source relations                   | Asym Postgres (separate Phase 14-owned operational records)                                                              | Authorized current-revision source command; atomic release receipt + outbox                          | Phase 14 purpose/readiness; posted P13 facts stay immutable; acknowledgment commands do not mutate settlement facts; Phase 6/18 retain their outcomes | Idempotent receipt-backed recovery; no posted-source mutation or silent coverage reset                                                          |
...
11. **One locked mutation function, one lock.** All credit mutations go through one locked SECURITY DEFINER function taking the **SAME per-contribution advisory lock as the current P13 owner command**; batch match entry is an ordinary P13 entry transaction with expectancies `FOR UPDATE ORDER BY id` + `lock_timeout` [D1.8, D4.2]. _Fixture: two concurrent clerks on overlapping expectancies — no deadlock, no double-settle._
...
14. **Coverage grain.** `UNIQUE (tenant_id, recipient_party_id, header_id)` on `tribute_notification_items` uses canonical Party identity, while `notify_party_id` retains the exact stream-preference provenance. Same-Tenant/recipient/letter composite constraints and atomic claim/letter composition prevent two different watcher rows for one Party from notifying the same gift twice. Pre-release cancellation alone may reopen coverage under the shared release guard; later correction stays append-only. _Fixtures: both cross-tribute concurrent commit orders; re-add/merge preserves coverage and strictest suppression; authorized override uses the actual recipient; forged provenance fails._
...
17. **`tribute_type` immutability.** BEFORE UPDATE trigger rejects a type change after the first letter; honor→memorial is close + guided successor (notify preferences re-confirmed; no copied letters/items or automatic gift-link inheritance; existing canonical recipient Party/header claims preserved) [D3.12]. _Fixture: post-letter type flip RAISEs._
...
- **Ack work view:** index the separate Phase 14 request records by `(tenant_id, acknowledgment_request_state)` for held/ready/released work, with exact source-relation lookup indexes and qualified identity constraints. No acknowledgment-state index belongs on `contribution_headers`. The derived **Unacknowledged** view joins those records to exact Phase 6 outcome relations and any Phase 18 artifacts; it stores no copied outcome and Phase 14 has no `failed` communication status.
...
14. **Type-aware deceased safety:** every deceased notify recipient blocks at compose/send; an honor honoree’s death pauses honor/routes its memorial successor; a deceased memorial subject alone does not block living qualified recipients [D3.8, D3.12, D3.14].
...
16. **Two tributes, one watcher = told once:** canonical `(recipient_party_id, header_id)` coverage prevents duplicate notification despite different stream-preference IDs. Exercise both concurrent commit orders, tombstone/re-add, owner-qualified merge and authorized recipient override; preserve the exact per-stream letter anchor and historical snapshots [D3.1, D3.11, D3.14].
...
18. **Pre-release cancel reopens items:** prepared/held cancellation and release share one expected-state guard. Only the proved pre-release winner reopens coverage atomically. Released/submitted/unknown outcomes never reopen merely from cancel, timeout or replay and use the owner’s recovery/correction path [D3.1].
...
- **Acknowledgment storage/transition proof (C-02).** Exercise grant, multi-grant consolidated and per-settlement employee requests through separate owner records. Assert exact same-Tenant header/settlement/line references, stale-revision rejection, one release-versus-cancel winner and replay reuse of the accepted receipt/outbox. Confirm request transitions leave every posted contribution/header/line and settlement source fact unchanged; a forged privileged reference fails structurally. Batch-origin hold/release preserves frozen origin metadata, imports never pass the gate, and no Phase 6/18 outcome is copied into Phase 14.
- **Census rows fail closed.** All Phase 14 record types, including the separate current-attribution, acknowledgment purpose/request and exact source-relation shapes (§Data Model), ship Phase 3 (Minimum Permission & Role-Scoped Projection Foundation) census rows **in the same migration** as their tables (FORWARD — the P3 field census is groomed, not built); a test asserts every new table has census coverage, and a missing row makes reads fail, not fall open [D1.10, D3.11].
...
3. **S3 — DAF operations + the advisor stream [D2].** `daf_sponsors` + the shared `party_payer_aliases` matcher + one-click triage [D4.5]; the required 4-state attribution combobox + fund-name memory with provenance chips; the Attribution Inbox; the entry-gated auto-send advisor thank-you with the FULL guardrail set (hold-then-send window, ack states, disclosure line, outbox feed, ambiguity holds, amount-omitted default, re-attribute flow); sponsor eligibility and official facts admitted by Phase 7, any exact artifact owned by Phase 18, and independent Phase 6 delivery suppression by default. _Blocked by:_ S2; Phase 6 (Shared Communication Event Model) send seam + consent gate. _Kill/rollback:_ the hold/readiness guard may cancel only unstarted pre-release requests; released/submitted/uncertain work follows owner recovery without recall or duplicate effects; tenant stream config stops unstarted work while attribution capture keeps working; **imports never auto-send** [D2 guardrail 8].
