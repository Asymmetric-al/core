...
> **Hard dependencies:** Phase 13 (Campaign, Designation, Contribution Ledger & Giving Cart) — the header/lines/postings ledger every credit row keys to; Phase 7 (Receipt & Statement Compliance Rules + Donor Identity/Credit Model) — the donor-credit truth model (A8–A11) this phase operationalizes; Phase 9 (Full CRM Depth & Relationship Graph) — the party spine, Giving tab, and merge machinery. **Enhanced by:** Phase 17 (System Messages & Template Management) — tenant editing of the templates Phase 14 ships fixed.
...
- **Tribute and memorial, done with care [D3] — Sections E–F.** A second letter stream structurally incapable of the classic blunders: setup-gated **automated digest letters** on an age-anchored cadence (weekly while the loss is fresh, monthly after, never stopping silently); an explicit coverage ledger guaranteeing a "Mom and Dad" watcher is told about each gift exactly once; per-gift amounts **structurally absent** from the notification class (render-refusal, both engines); an opt-in aggregate total with a monotonic floor so a family never sees the memorial shrink; a first-class **mail channel** with governed print custody; restricted and anonymous donors rendered identically as "an anonymous friend." Two fixed house templates ship v1; Phase 17 (System Messages & Template Management) adds tenant editing on the same registry.
...
- **Close the Phase 7 A10 hole** — document-class merge-field allowlists on both render engines (acknowledgment: no deductibility fields; notification: no per-gift amounts + controlled donor identity), render-refusal shipping before the first letter (Section E).
...
- **No template editor.** Zero new template tables; two fixed house tribute templates on the shipped registries; tenant editing is Phase 17 (System Messages & Template Management) [D3.6].
...
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Phase 7 §A A8**                       | "One hard-credit legal donor per gift; soft credit is recognition-only and structurally non-receiptable." Credits carry `is_receiptable = FALSE` "enforced by DB CHECK **and** service layer; a soft credit can never mint a receipt or enter any money/receipt/cash total. Do not add a second donor FK."                                                                                                               | Every credit row, role, and generator inherits this. The CHECK ships as tripwire; the primary wall is structural — the receipt-mint path takes no `contribution_credits` input (Section E, [D1.11]).                                                                                                                                                                                                                                                                                      |
...
| **Phase 7 §A A10**                      | "The three-document wall is structural." Acknowledgment → "**no** deductibility merge-fields available to the template"; notification → "**amount hidden**, never a tax document"; "DAF advisor = acknowledgment with $0 deductible + 'this is not a tax receipt.'"                                                                                                                                                      | Phase 14 _implements_ the wall as document-class allowlists (repo recon: Statement Studio governance is permission-based today — a compliance hole) and carries the single dated A10 amendment, the governed `tribute_aggregate_total` field (Sections E, F, [D1.11, D3.4]).                                                                                                                                                                                                              |
| **Phase 7 §A A11**                      | "Matching = two donations; tribute = annotation; recurring = per-installment." The match is "a **separate** donation with `donor_id` = the company (its own receipt); the employee gets a soft credit only."                                                                                                                                                                                                             | Preserved, with one dated amendment: the spawned/linked contribution's legal donor = **payer-of-record** (defaults to employer; intermediaries are real payers) (Section G, [D4.4]).                                                                                                                                                                                                                                                                                                      |
...
13. As a **finance staffer**, I want a confidently identified attribution to auto-send the non-receipt advisor thank-you with no letter-review queue, so that the identity decision at entry IS the review. `[D2]`
14. As a **finance staffer**, I want a hold-then-send window (~10 min default, tenant-config) cancelable from the toast and the gift record, so that I can catch a mistake after save — undo beats confirm. `[D2.g1]`
15. As a **finance staffer**, I want acknowledgment as a STATE on the gift — `{pending_send, sent, held, suppressed, failed, not_applicable}` — with an "Unacknowledged" work view, so that no thank-you is fire-and-forget. `[D2.g3]`
16. As a **finance staffer**, I want suppressed/failed sends (Phase 6 consent gate + bounce/complaint suppression) surfaced as visible events, never silence, so that a blocked letter is a known fact. `[D2.g4]`
...
22. As a **finance staffer**, I want the sponsor's receipt minted per Phase 7 (Receipt & Statement Compliance Rules + Donor Identity/Credit Model) as the compliance record but its DELIVERY suppressed by default, so that we stay compliant without mailing sponsors who ask not to be mailed. `[D2]`
...
109. As a **donor**, I want acknowledgments and notifications rendered only through document-class merge-field allowlists — acknowledgment: no deductibility fields; notification: no amounts + controlled donor identity — with render-REFUSAL on violation on both email and print paths, so that the wrong field can never appear in the wrong document. `[D1.11, D3.6]`
...
- **Editable-with-audit until the FIRST external reference** — a `statement_run_items` row pointing at the credit (C.6), or a sent document linked via `communication_event_relations` (FORWARD: Phase 6 (Shared Communication Event Model) PRD, epic #550) — **then frozen, supersede-only**: corrections after freeze create a new row with `supersedes_id` pointing at the old one, and the old row leaves the `active` predicate (which is what scopes the A.1 identity key and every index in C.7).
...
#### C.6 `statement_run_items` — the amended contract restated [D1.14 amendment of Phase 7]
...
- **`anonymous`** — the donor requested anonymity via the sponsor → combobox state ④; no attribution is ever solicited; the acknowledgment axis is `not_applicable`.
...
#### D.4 The eleven auto-send guardrails (ratified set — none reintroduce a queue) [D2]

1. **Hold-then-send window** — default **~10 minutes, tenant-configurable**; cancelable from the save toast **and** from the gift record. A save-trigger has no proofreading moment, so the window is minutes, not Gmail-seconds; undo-beats-confirm (NN/g: confirmation dialogs habituate).
...
3. **Acknowledgment is a STATE on the gift** — `{pending_send, sent, held, suppressed, failed, not_applicable}` (TEXT + CHECK) — plus an **"Unacknowledged" work view**. Never fire-and-forget (Bloomerang precedent; parallels the Phase 13 receipt axis). Natural mappings: identified → `pending_send` (hold window) → `sent`/`failed`; ambiguity, staff hold, or new-party hold → `held`; consent/bounce block → `suppressed`; Anonymous tier → `not_applicable`; "Not Provided" stays `held` while the gift rides the Attribution Inbox.
4. **The Phase 6 (Shared Communication Event Model) consent gate + bounce/complaint suppression apply at send** — the shipped gate in `packages/api/src/email/consent.ts` (REAL: message-type-aware, fail-closed; transactional/relational classes bypass marketing opt-outs but never `do_not_contact`/bounce/complaint). **Suppressed/failed are visible events, never silence.**
...
10. **Every auto-send = a timeline event on the party + a tenant-wide "Sent automatically" outbox feed** (HubSpot pattern) — staff can always answer "what did the system send on my behalf yesterday?"
...
#### D.8 Sponsor mail-suppression and the minted-but-not-mailed receipt [D2]

The sponsor's tax receipt is **minted per Phase 7 (Receipt & Statement Compliance Rules + Donor Identity/Credit Model)** — it is the compliance record and exists unconditionally — but **DELIVERY to the sponsor is suppressed by default**: sponsors explicitly ask charities not to mail them grant receipts (documented in sponsor "grow your giving" FAQs). The gift record shows a **"receipt minted — delivery suppressed"** note so staff never read the absence of an outbound receipt as a missing receipt. A tenant can flip delivery on per sponsor. This is delivery suppression only — it never touches the mint path or the receipt's existence.
...
**The compliance hole [D1.11]:** Statement Studio's variable governance today is **permission-based, not document-class-based**. The shipped design doc (`docs/guides/features/statement-studio/variables.md`, REAL) gives every source-map record "Required scope" and "Permission and sensitivity rules" — _who_ may use a variable — but has **no dimension for _which document class_ a variable may render into**. Under that model, a staff member with giving-variable permission could bind a deductible-amount field into an acknowledgment template, and nothing structural would stop the render. Phase 14 closes this before its first letter ships.

#### E.1 Document-class allowlists on the variable registry [D1.11]

Every render path — Email Studio template bindings and PDF Studio — declares its **document class** (`receipt | acknowledgment | notification`, TEXT + CHECK where persisted). The variable source map gains a **per-class allowlist**: a variable is renderable into a class only if the allowlist says so. Class rules:
...
#### E.3 Render-REFUSAL, never silent field-drop — enforced on BOTH render paths [D3.6]
...
- **Publish/bind time:** validation refuses the binding (Statement Studio's validation step — "unsafe" per the shipped checklist in `variables.md` — extended with the class dimension).
- **Render time:** the renderer **REFUSES** — the render fails loudly, the letter enters its `held`/`attention` state, and a routed task is created. Defense in depth: bind-time checks catch authoring; render-time refusal catches drift (a variable re-classified after publish, an import, a code path that skipped validation).
- **CI: one refusal test per render path** — Email Studio bindings AND PDF Studio — asserting that an amount field in a notification render input and a deductibility field in an acknowledgment render input each **fail the render** rather than render partially. This is a **build-order gate: the allowlist + refusal tests ship before the first letter** (Section on Build Order sequences it; per D3.6 "ships before the first letter").

#### E.4 The receipt wall — primary enforcement plus the schema tripwire [D1.11]

Two layers, deliberately redundant:

- **PRIMARY wall: the receipt-mint path structurally takes no `contribution_credits` input.** The minting service's input type has no credit parameter; a **lint** (import/call-graph check on the mint module) and a **test** (a receipt minted for a header with N credit rows is byte-identical to one minted with zero) enforce it. A soft credit cannot influence a receipt because the code path _cannot see_ credits — impossibility by construction, the same posture as Phase 13's "suppression is the identity."
...
- Composition freezes the input (Section F's cutoff-frozen item set); render is a pure function of the struct — re-rendering a letter later reproduces it byte-for-byte, satisfying the Phase 6 (Shared Communication Event Model) frozen-artifact retention rule.
...
- **Print:** the **shipped PDF Studio foundation** — REAL: `packages/api/src/pdf-templates/store.ts`, `native-adapter.ts`, `docraptor.ts`, configured via `packages/config/pdf-studio.ts`.
- **Phase 17 (System Messages & Template Management)** adds tenant _editing_ on the same registry — same rows, new authoring surface, allowlists already in force. **Phase 18 (Receipt & PDF Template System)** consolidates PDF templating. Neither phase inherits a migration burden from Phase 14, because Phase 14 added no tables to migrate — that is the point of the ruling.
...
**Adjudication — why an explicit coverage table (5:1 among reviewers) instead of deriving coverage from Phase 6 (Shared Communication Event Model) relations:** backdated gifts land inside already-covered periods; the print channel has **no send event until physical fulfillment**, so "what has been communicated" and "what has been sent" diverge by days; retries must recompose from a stable record; the anti-join ("gifts not yet in any letter") is unindexable against a generic relations table; and P6's person-link is donor-XOR-missionary, so notify parties structurally don't fit it. The letter itself still **is** a communication event (F.14) — the items table is the tribute-domain coverage truth beside it, written **in the same transaction** as the letter row [D3.1].
...
6. **Render reads the frozen item set only.** The letter body is derived exclusively from its `tribute_notification_items` rows (with their `rendered_donor_as` / `letter_total_minor` snapshots) — never from a live re-query, so retry, preview, and the archived artifact (F.13) are all byte-identical in content.
...
- **Channel-aware consent at print ENQUEUE:** the consent gate evaluates message-type **notification** for the mail channel — **`do_not_contact` + deceased + address-present, fail-closed; marketing opt-outs do not apply** (a notification is not marketing). The **consent snapshot is frozen onto the communication event exactly like email** — the shipped consent gate (`packages/api/src/email/consent.ts` — REAL: the message-type-aware, fail-closed outbound gate) is the pattern being extended, with mail-channel evaluation being FORWARD work this phase owns.
- A mail-channel notify party **with no postal address = HOLD + routed task, never a silent skip** — silence is how a family concludes nobody gave.
...
- Batches are delivered via **short-lived signed URLs**; every download writes an **identifiers-only download audit** record (who, when, which batch — never letter contents in the audit row, per the P3 field-keys-not-values audit rule).
...
- **Email** letters bind through the **shipped Email Studio system-bindings registry** — REAL anchor: the `email_template_system_bindings` table consumed by `packages/api/src/email/template-store.ts` (the constant and reads are in that file today). Phase 14 adds two binding keys: **`tribute.digest`** and **`tribute.honor_immediate`**.
- **Print** letters render on the **shipped PDF Studio foundation** — REAL anchor: `vendor/react-pdf-packages/asym-pdf-studio-adapter/` plus the receipt-PDF path in `packages/api/src/admin/contribution-operations/receipt-pdf.ts`. Phase 18 (Receipt & PDF Template System) consolidates the document-template system later; Phase 14 rides what exists.
...
- The **D1.11 notification allowlist is enforced on BOTH render paths with render-REFUSAL** (a template referencing a non-allowlisted field fails the render, loudly) and **ships before the first letter** — this is a build-order gate (F.14). The allowlist system itself is Implementation Decision E.
- **Phase 17 (System Messages & Template Management)** later adds tenant editing **on the same registry** — no rework, no migration.
...
2. **Setup UI + MANUAL single letters via both engines** — the tribute page, the 2-field stub, and staff-composed one-off letters through the email binding and the PDF path (proves both render walls with a human in the loop).
...
**The D3 poison-fixture set (9+, consolidated with the phase-wide inventory in the Testing & Poison Fixtures section):** amount-merge-field render-refusal on **both** render paths; `never` = zero sends forever (including composed-then-flipped); deceased-recipient blocked at compose and at send; restricted-party invisibility end-to-end (and restricted-donor crowd-blend rendering); import → zero sends even after tenant enable; crash-retry mints exactly one letter (the `period_key` unique); two tributes + one watcher + one gift ⇒ told once (the `(notify_party, header)` grain); cancel-reopens-items; fold-below-floor ⇒ total omitted + task.

**Real-vs-forward evidence (as of authoring).** REAL today: the Email Studio system-bindings registry (`email_template_system_bindings` via `packages/api/src/email/template-store.ts`); the PDF Studio foundation (`vendor/react-pdf-packages/asym-pdf-studio-adapter/`, `packages/api/src/admin/contribution-operations/receipt-pdf.ts`); the message-type-aware fail-closed consent gate (`packages/api/src/email/consent.ts`); the audit spine this phase rides (`contribution_operation_audit_events`, `supabase/migrations/20260526132000_contribution_operations_core.sql`); and the P6/P7 PRD texts being amended. FORWARD (groomed-not-built, named owner): `contribution_headers`/lines and the `effective_seq` cursor (Phase 13 PRD); `communication_events`/relations, the monotonic guard, and the mail channel (Phase 6 PRD + the D3.5 amendment); `resolveProjection` census rows and governed exports (Phase 3 PRD); the data-health catalog (Phase 8 PRD); tenant template editing (Phase 17); document-template consolidation (Phase 18); import tooling (Phase 30). Every tribute table, the coverage ledger, the digest engine, and both house templates are **net-new Phase 14 builds** — the repo has zero tribute product code today.
...
- **REFUSES statement-referenced headers**, verbatim from the ruling: a header whose credits are frozen by a `statement_run_item` reference (A.4) is skipped and reported, never silently restated — a backfill that touched issued statements would be a silent restatement of documents the tenant already sent. The refusal is a listed outcome ("skipped: statement-referenced"), so finance can see exactly what the backfill declined to do and decide whether a supersede workflow (P7's lane) is warranted per case.
...
- frozen_at — timestamptz NULL — stamped in the same txn by the FIRST external reference (a
  statement_run_items row, or a sent doc via communication_event_relations); BEFORE UPDATE OR DELETE
...
- receipt_delivery_suppressed — boolean NOT NULL DEFAULT TRUE — the sponsor receipt is MINTED per
  P7 (the compliance record) but DELIVERY is suppressed by default; per-sponsor flip — [D2]
...
### Acknowledgment-state columns — the D2 guardrail-3 axis

Acknowledgment is **"a STATE on the gift … never fire-and-forget"** [D2 guardrail 3], paralleling the P13 receipt axis. Two placements, one vocabulary:

```
acknowledgment state (TEXT + CHECK, shared vocabulary):
  {pending_send, sent, held, suppressed, failed, not_applicable}

contribution_headers — extension columns (audience = the ONE attribution party per grant [D2/D.12])
- acknowledgment_status — text NOT NULL DEFAULT 'not_applicable' — CHECK ∈ set above — [D2 g3]
- acknowledgment_hold_until — timestamptz NULL — the hold-then-send window (~10 min default,
...
- acknowledgment_status — text NOT NULL DEFAULT 'pending_send' — CHECK ∈ same vocabulary
...
Final names per D3.14: Phase 7's `tribute_notifications` is **RETIRED** (ambiguous with sent letters); its notify-once flag is generalized into frequency `once`. Sent letters **ARE** Phase 6 `communication_events` — the letter row below is the tribute-domain exactly-once anchor that _emits_ one event; the items table is the coverage truth beside it. (`tribute_letters` is this PRD's naming consolidation for the letter row D3.1/D3.2 specify — the natural plural-snake completion of the D3.14 set.)
...
tribute_letters (the exactly-once anchor: one composed consolidated letter per stream per period)
...
- channel — text NOT NULL — CHECK ∈ {email, mail} — [D3.5]
- status — text NOT NULL — CHECK ∈ {composed, held, sent, canceled}; mail custody
  (queued→printed→mailed/returned) lives on the P6 event, not here — [D3.2, D3.5]
...
- cutoff_at — timestamptz NOT NULL — composition is cutoff-frozen; a late gift rides the next
  period — [D3.2]
- communication_event_id — uuid NULL — the ONE emitted P6 event (+ relations for the timeline) — [D3.1]
- rendered_artifact_ref — text NULL — the frozen rendered artifact (P6 operational retention);
  render reads the frozen item set only — [D3.13]
...
- acknowledgment_status / acknowledgment_hold_until — per-settlement ack columns (block above) — [D4.10]
...
| Tribute letters + coverage ledger                                                 | Asym Postgres (`tribute_letters`, `tribute_notification_items`); sends ARE Phase 6 `communication_events` | The exactly-once composer transaction; staff compose-now                                             | Asym; the frozen `rendered_donor_as`/artifact is the truth of what was said | Cancel-reopens-items in-txn; recompose next period; NO retraction letters                           |
...
6. **`is_receiptable = FALSE` tripwire.** Column + DB CHECK kept per P7 A8 even though the primary wall is the credit-blind mint path; a **schema lint asserts the CHECK exists** [D1.11]. _Fixture: a DAF advisor can never mint a receipt or enter a deductible total._
...
- **Ack work view:** partial index on `contribution_headers (tenant_id) WHERE acknowledgment_status IN ('pending_send','held','failed')` — the "Unacknowledged" view is a query, not a table.
...
| `finance:fulfill_tribute_letters`                                                     | Print custody of composed tribute letters — a **governed export** per Phase 3: short-lived signed URLs, identifiers-only download audit, a retention class with auto-purge post-fulfillment, and the manual `queued → printed → mailed / returned` status transitions [D3.5].                                                                                                                                                  | Finance/admin or the staffer who runs the print queue.  | No SoD. Every download is audited; a no-address mail party is a HOLD + task, never a silent skip [D3.5].                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- **Every auto-send = a timeline event on the party + a row in the tenant-level "Sent automatically" outbox feed** — the D2 guardrail (10), reused **verbatim** by the tribute stream [D2, D3.9]. Nothing this phase sends is fire-and-forget: acknowledgment is a state on the gift (`{pending_send, sent, held, suppressed, failed, not_applicable}`) with an "Unacknowledged" work view [D2 guardrail 3], and per-stream tribute letter states (`{active, held, attention, ended}`) surface the same way [D3.9]. Suppressed/failed sends are **visible events, never silence** [D2 guardrail 4].
...
4. **Frozen-after-reference:** any `UPDATE`/`DELETE` on a credit referenced by a `statement_run_items` row or a sent document RAISEs — including raw SQL as `service_role`; supersede (`supersedes_id`) is the only path [D1.5].
...
9. **Allowlist render-refusal, both engines:** a deductibility merge field in an acknowledgment-class render, or a per-gift amount field in a notification-class render, **refuses to render** on both the email and print paths [D1.11, D3.6, D3.14].
...
- **Render golden snapshots.** The two fixed house tribute templates (memorial condolence / honor celebratory) and the advisor thank-you render from the TYPED render struct only [D3.6]; golden snapshots on both engines (email + print) pin the rendered artifacts so template drift is a visible diff, never a silent change.
...
3. **Receipt-mint-no-credit-input lint:** the receipt-mint path imports/receives no `contribution_credits` symbol or input (lint + test — the primary A10 wall) [D1.11].
...
1. **S1 — Schema + census + constraints + locked functions + the allowlist wall.** All Phase 14 tables (`contribution_credits`, `tributes`, `contribution_tributes`, `tribute_notify_parties`, `tribute_notification_items`, `matching_gift_expectancies`, `matching_gift_settlements`, `daf_sponsors` + `party_payer_aliases`, standing rules, `credit_generation_runs`) with **ALL identity/scope/bound constraints in the FIRST migration, before any generator merges** [D1.1] — the D1.1 identity key + `external_ref` partial UNIQUEs, D1.2 exclusivity trigger, D1.3 bounds, D1.5 freeze trigger, D4.1 settlement uniqueness, D4.3 state-guard trigger, composite tenant FKs + FORCE RLS everywhere [D1.9], census rows fail-closed [D1.10]. The one locked SECURITY DEFINER credit-mutation function on the P13 per-contribution advisory lock [D1.8]; the `contribution_operation_audit_events` polymorphic-subject extension [D4.12]. The **document-class allowlist wall on both render engines with render-refusal ships here — before the first letter** (the D3.6 build-order gate), plus the full CI lint list and the D1.4 recognition fold + `effective_seq` cursor read model. Implement the P13 amendments this slice depends on (lines `UNIQUE(tenant_id, header_id, id)`; the correction service's `credit_recheck` outbox event; remittance tender attributions demoted to capture-input-only) [D1.14]. _Blocked by:_ Phase 13 (Campaign, Designation, Contribution Ledger & Giving Cart) ledger; Phase 3 (Minimum Permission & Role-Scoped Projection Foundation) census machinery. _Kill/rollback:_ inert — nothing composes, sends, or credits yet.
...
4. **S4 — Tribute setup + manual single letters.** The 2-field inline tribute create that never gates gift posting; the "Tributes awaiting setup" worklist; the tribute record ops page (dual-pane donor list, states, previews); MANUAL single letters via both engines (compose-now advances coverage) [D3.12, D3.3]. _Blocked by:_ S1 (the allowlist wall must precede the first letter); Phase 6 (Shared Communication Event Model). _Kill/rollback:_ manual-only — a human triggers every letter; no scheduler exists yet.
...
- **Tenant-editable tribute templates** — Phase 17 (System Messages & Template Management) adds editing on the same system-bindings registry; Phase 18 consolidates print templating. Phase 14 ships exactly two fixed house templates [D3.6].
...
_Vertical-slice tickets mapped to the Build Order, one line each. **Unpublished — no issue numbers**; the epic + children are minted at `/to-tickets` time and remain `status:blocked` on the predecessor spine (Phase 3, 7, 9, 10, 12, 13 builds) per program convention._
...
3. **T3 (S1)** — Document-class merge-field allowlists on both render engines with render-refusal + the five CI lints (non-money additions, sole-aggregator, receipt-mint-no-credit-input, single-field assertion, `is_receiptable` schema lint). _(substrate; gates every letter)_
...
11. **T11 (S4)** — Tribute setup gate (2-field inline create, awaiting-setup worklist, tribute ops page) + manual single letters via both engines. _(tribute)_
...
14. **T14 (S5)** — P6 `mail` channel + print custody as a governed export (`finance:fulfill_tribute_letters`, signed URLs, retention + purge). _(tribute)_
