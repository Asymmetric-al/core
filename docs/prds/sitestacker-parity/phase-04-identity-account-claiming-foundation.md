...
- A **frozen receipted legal-donor snapshot** (name + address + donor-id-at-issue) on the receipt/donation record at issuance — immutable; merge/redaction touch only the mutable CRM projection.
...
- The **populated** person/constituent spine and cross-entity (donor+missionary) dedupe — only the inert typed anchor ships now.
...
- `gift_attributions` / **soft credit** / tribute / matching / DAF — with `donations.donor_id` documented as the **legal donor (hard credit)** invariant, kept separate from the person spine (the mistake that forced Salesforce's NPSP→Nonprofit-Cloud re-platform).
