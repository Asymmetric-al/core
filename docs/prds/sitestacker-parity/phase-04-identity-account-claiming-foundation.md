...
- **A2 — Anchor: profiles-as-anchor now; typed person spine reserved.** The claim binding is `donors.profile_id` (existing FK, authoritative) + a donor `authz.memberships` row + an audit event. "CRM person" remains a **projection** over donors/missionaries into Twenty via `crm_record_links` — never an Asym table; money-truth stays in Asym. A minimal **inert typed `persons` anchor** + nullable `person_id` FKs are shipped now (not populated/read) so the Option-C spine is a cheap future retrofit.
...
- A **frozen receipted legal-donor snapshot** (name + address + donor-id-at-issue) on the receipt/donation record at issuance — immutable; merge/redaction touch only the mutable CRM projection. This snapshot is reserved as **party-type-aware**: the fields **Phase 7** populates freeze `donor_id` + `party_type` + `legal_name`/name + address-of-record + (for org/DAF/foundation legal donors) EIN + (when a household is the legal giving unit) household-member-ids. Phase 4 ships only the reservation and the individual subset; Phase 7 **extends** the snapshot columns for org/DAF/household legal donors, so appending those columns does not break the immutable-snapshot contract.
- **Inert reserved:** empty `persons` (id, tenant_id, created_at) + nullable `person_id` FK on donors/missionaries/profiles — not populated or read in Phase 4.
- **Replayable:** a `merge_operations` record (survivor, loser, per-child re-pointed FKs, golden-record choices, pre-merge consent).

**Reuse:** `authz.memberships` (roles/multi-hat); `crm_record_links` (provider links — Stripe now, Twenty, Mailchimp later; the second Stripe customer from a merge lands here); `crm_merge_candidates` (dedupe/review, extended to reference two same-tenant Asym donor ids); consent (`do_not_email` / `do_not_contact` / `email_suppressions`); Email Studio `email_templates`; the Phase-3 audit spine.

**Reserved seam (entity-link types):** the `crm_link_entity_type` DB enum and its TS mirror `CrmIdentityConceptId` are **extended in Phase 7** to add `household` / `organization` / `daf_sponsor` (and `person` / `gift_credit`) entity-link types; Phase 4 ships only the current entity-link set (baseline: `supabase/migrations/20260508000413_crm_identity_mapping.sql` and `packages/api/src/crm/identity/concepts.ts`).
...
- **Inngest:** scheduled self-healing dedupe scan (Phase-1 reconcile pattern).
...
- **Prior art:** Phase-3 resolver/projection golden-snapshot tests; the Phase-1 Inngest reconcile job (pattern for the dedupe scan); existing `packages/api` service unit tests.
...
- **Twenty projection** as a hardened cross-system isolation boundary (tenant-guarded sync reader, per-tenant Twenty workspace) — reserved to when the projection is actually wired.
...
- [ ] The inert typed `persons` anchor + `person_id` FKs exist; the receipted legal-donor snapshot is frozen at issuance.
...
- **T3** — Data-model migrations: the 2 tables, columns, inert `persons` anchor, receipt snapshot, unique keys, and the canonical email-normalizer. _(foundation)_
