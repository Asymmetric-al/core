...
- **A2 — Anchor: profiles-as-anchor now; typed person spine reserved.** The claim binding is `donors.profile_id` (existing FK, authoritative) + a donor `authz.memberships` row + an audit event. "CRM person" is **Asym-owned operational truth** over donors/missionaries — today via the donor/missionary records themselves, later via the Phase-7-populated `persons` spine (the earlier Twenty-projection clause is retired — [ADR-0001](../../adr/0001-asym-postgres-owns-crm-truth-twenty-retired.md)); money-truth stays in Asym. A minimal **inert typed `persons` anchor** + nullable `person_id` FKs are shipped now (not populated/read) so the Option-C spine is a cheap future retrofit.
