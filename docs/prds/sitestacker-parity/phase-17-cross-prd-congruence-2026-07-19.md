...
| 16    | **BOUNDED GAP, AMENDED.** Message obligations were exact, but catalog/sender/recovery structures and atomic multi-step compilation were not yet ratified.                | Map them to Phase 17 contracts; preserve Phase 16 meaning/candidate/source-fence ownership and Phase 7 plan/finality-admitted receipt authorization. Phase 18 owns the exact artifact and Phase 17 owns delivery. Submit one bounded plan occurrence through Phase 6, never independently committed child calls. |
...
9. OpenSpec strict validation and Markdown/Prettier/link checks pass. Every
   ...
   scenarios.
10. Every document reports the published epic #873 and children #874–#905
    without claiming Phase 17 is built, live, dispatched, or
    `ready-for-agent`; every child remains open with `status:blocked`.
