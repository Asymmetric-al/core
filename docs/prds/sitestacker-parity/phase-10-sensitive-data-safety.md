...
- **`party_publishing_consent`** — per-person: `party_id`, `publish_name`/`publish_photo`/`publish_story` (bool + consent evidence), `do_not_publish` (hard flag), consent timestamps. Consumed by the firewall + the send seam. Under Phase 22 D26, absence of the three granular affirmative records alone does not create a Page/Update checklist or deny its whole-candidate path; any recorded objection, hard flag, restriction, or stricter current safety result remains subtractive and non-overridable.
