...
- As Ana, a paused gift remains visible with the exact safe pause wording. A terminal miss produces one grouped in-product notice, no email and no automatic outreach task.
...
The successful command also submits one recipient-specific `recurring_schedule_changed_v1` typed intent through the Phase 6 communication-intent seam after current recipient/purpose proof. Phase 17 owns rendering. Delivery failure never reverses a valid change, but the durable confirmation shows only evidenced Phase 6 truth such as **Confirmation requested**, **Sent**, or **Could not deliver** and exposes the safe contact-repair path. Skip, pause, cancel, and payment-recovery actions retain their own truthful action confirmations rather than being mislabeled as a schedule-change message.
...
They see no card/bank, provider, decline reason, credential, safety cap, exact internal retry dates, sibling-line data, or recovery controls. A terminal-miss notice is in-product only, grouped in the UI, informational, and creates no default donor-outreach task.

### D.9 Meaningful-transition communication contract

A provider attempt is not automatically a human message. A new human meaning is. Phase 16 records the durable domain transition and recipient candidate, re-proves its current business eligibility, and submits eligible meaning through Phase 6's guarded service. Phase 6 creates and owns the durable `communication_intent`, consent/contactability, channel dispatch, resulting `communication_event`, delivery outcomes, and suppression; Phase 17 owns governed templates. The vendor remains replaceable.

The baseline state transitions are:

| Transition                  | Donor treatment                                                                                                                        | Missionary treatment                      |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| D7 plan durably opened      | One recovery-start message with exact occurrence, next candidate dates, next ordinary date, and safe self-service                      | No per-attempt notice                     |
| Same-state retry fails      | No discretionary Asym message                                                                                                          | None                                      |
| New action required         | One immediate action message, deduped until meaning changes                                                                            | High-level status only if allowed         |
| Provider-confirmed success  | Normal receipt; no extra “recovered” email                                                                                             | Projection updates                        |
| Occurrence closes Missed    | One terminal message: exact amount/date, no debt/backcharge, schedule continues, next amount/date, update/give-once/pause/cancel paths | One durable in-product privacy-safe event |
| ACH processing              | Initiation/processing confirmation only                                                                                                | Processing projection                     |
| ACH final success           | Normal receipt                                                                                                                         | Projection updates                        |
| Applied schedule amendment  | Durable in-product arrangement confirmation plus one deduped `recurring_schedule_changed_v1` intent for the proven recipient           | Projection updates; no missionary message |
| Corrected or reversed truth | Append a corrective/superseding message                                                                                                | Projection updates; old history remains   |

A mandatory network, payment-rail, jurisdiction, provider, authorization, or authentication notice overrides the discretionary quiet period. An applicable Visa merchant-initiated stored-credential decline receives written notice and at least seven calendar days of access to an alternative payment method unless current written guidance for the exact acquirer/network program expressly permits a different compliant treatment. Semiannual and annual recurring donors receive a seven-day upcoming-charge reminder as the v1 baseline; stricter live requirements win.

One billing-cohort occurrence is the communication unit, not each line, allocation, attempt, or webhook. Every intent has one owner and a permanent semantic key across tenant, occurrence, meaning/version, audience, and channel. Repeated/out-of-order events are no-ops. Templates may vary bounded branding and tone but cannot alter eligibility, dates, action effect, mandatory content, no-debt language, safety logic, or transaction truth. Transactional messages contain no marketing, guilt, or upsell.

Recipient resolution is purpose-specific and never inferred from the Commitment Party alone. Phase 7's legal donor receives the official receipt; the current financial authorizer receives authorization, payment-action, recovery, and applicable rail/network notices; a separately proven D14 service contact receives only the stewardship purpose for which that contact is effective. An authorization challenge freezes the exact recipient Party/contact point used for delivery, but apply re-proves the current authorizer and terms. If one cohort contains lines with different permitted recipients, Phase 16 submits one recipient-specific meaning per resolved recipient through Phase 6; Phase 6 creates the corresponding durable intent. Each submission includes only that recipient's authorized line facts plus the truthful cohort-level payment effect needed to act and dedupes by recipient as well as meaning. Missing, ambiguous, stale, or unauthorized recipient evidence suppresses discretionary submission and opens the exact repair path; it never falls back to a Party email, payer email, or another line's contact.
...
Key columns: `id`, `tenant_id`, `fixed_total_pledge_id`, `enrollment_id`, `plan_version_id`, `expectation_id`, `stage` (`upcoming|source_aware_followup`), `candidate_at`, `semantic_key`, `state`, `claim_token`, `claim_expires_at`, `suppression_reason`, nullable `communication_intent_id`, `last_evaluated_at`, `source_cursor`.

`fixed_total_pledge_id` has a composite same-tenant FK and must match the enrollment/plan/expectation chain. `communication_intent_id`, when present, is a composite same-tenant FK to Phase 6 `communication_intents` and must carry the same permanent semantic key/recipient/channel meaning. Permanent unique `(tenant_id, fixed_total_pledge_id, plan_version_id, expectation_id, stage)` via stored key components. State: `candidate`, `claimed`, `submitted`, `suppressed`, `expired`, `canceled`, `indeterminate`; provider delivery state remains Phase 6. Candidate evaluation cannot force submission.
...
| `reminders`                  | D19 policy/enrollment/candidates                                                                                                       | Consent/delivery/rendering                                                                     |
| `communications`             | D9 transition meaning, purpose-specific recipient projection, permanent semantic keys, and Phase 6 intent submission                   | Consent/suppression, templates, dispatch, provider delivery, or communication history          |
...
| `applyRecurringChange`                                                            | Valid preview/authorization → append command/epoch/membership/terms, frozen confirmation snapshot and provider saga; domain facts atomic, provider result explicit; submit one deduped schedule-change intent when applicable                                                     |
...
| `claimReminderCandidates` / `proveAndSubmitReminderIntent`                        | Bounded due shard → proof-at-send result to Phase 6 or explicit suppression                                                                                                                                                                                                       |
| `proveAndSubmitRecurringCommunicationIntent`                                      | Named D9 transition, including applied schedule amendment, + exact occurrence/command/recipient/purpose/channel facts → current eligibility proof and idempotent Phase 6 `communication_intents` row or explicit suppression; no direct send                                      |
...
Domain transactions append outbox rows in the same commit. Dispatch may repeat. For D9/D19, the consumer calls Phase 6's idempotent intent-submission service and records its returned `communication_intent_id`; it never calls an email provider or creates a `communication_event` itself. Phase 6 performs send-time consent/suppression and creates the event only at the actual send seam. Consumers dedupe by permanent semantic key and record monotonic outcome. Dead letters never roll back money/intent; they create a repairable exception with owner and SLO.
...
- reminder due/claimed/suppressed/submitted by reason, semantic duplicate prevented, contact/consent/template/source failure, stop-token result, Phase 6 delivery state; and
...
| REM-05 | Permanent semantic key defeats duplicate jobs/webhooks/provider 24h expiry. Delivery evidence monotonic; never claims read/aware.                                                                                                                                                                                                                                                                                          |
...
Phase 16 pins typed meanings and merge facts; Phase 17 owns governed localized rendering. Canonical template/event identifiers are:
...
Normal success uses the Phase 7 governed receipt template, not a Phase 16 duplicate. Phase 16 missionary pause status and terminal missed-occurrence alerts are in-product only. A future missionary pause email is a typed future domain meaning under D5 only: Phase 16 submits neither a `communication_intent` nor a `communication_event` for it. A future owning phase may submit an eligible intent through Phase 6, and Phase 6 may create the event only at dispatch/publication; this phase selects no delivery platform or template identifier. Failed-occurrence missionary alerts remain non-email. Localization must preserve amounts/currencies, civil dates/timezone, action effects, no-debt language, mandatory terms, and terminology bans. Missing/unpublished/incompatible template suppresses optional submission and creates a repair exception; it never falls back to arbitrary English/provider content.
