...
| Communication intent, delivery-profile, event, consent, delivery and outcome spine                                     | Phase 6             | Consume exclusively. Phase 16 owns recurring/fixed domain transition meaning, recipient projection and candidate policy, then submits idempotently to Phase 6; never a parallel mail queue or sender.                |
| Receipt, statement, legal donor and tax facts                                                                          | Phase 7             | Consume. Phase 16 records processing-ACH initiation-confirmation meaning and submits it through Phase 6; an official successful-payment receipt waits for processor-confirmed success.                               |
...
| Phase 7  | **Concern: material conflict.** Phase 7 A6 allows an ACH receipt at `payment_intent.processing`; D4/D10 require official receipt only after confirmed success.                            | For Phase 16 recurring ACH occurrences, `processing` produces a truthful initiation/submission confirmation only. The official successful-payment receipt is emitted after processor-confirmed success. A later return creates the exact inverse/supersession required by Phase 7; history is not deleted. This is a dated narrow supersession, not a rewrite of card or settled-tender rules.                                                                                                                                              |
...
| Phase 13 | **Concern: major dated supersession required.** Its ledger remains authoritative; its recurring design does not, and its provisional ACH rule conflicts with Phase 16 recurring finality. | Keep append-only contribution/designation/posting truth, Connect direct-charge execution, provider event ingestion, designation eligibility and refund/return corrections. Replace the recurring object, executor mapping, lifecycle, retry, portal and cutover claims. For Phase 16 recurring ACH, `processing` persists evidence/projection only; success posts/receipts once and a late return appends the exact inverse/supersession.                                                                                                   |
...
**Amendment:** a recurring line may have an inclusive final eligible date and become ended after its final occurrence; that does not make it a fixed-total pledge. A fixed-total pledge has its own promise/progress/end/release model. No automatic continuation or conversion occurs. Any fixed-pledge reminder follows D19 explicit enrollment and the Phase 6/17 communication boundary. Do not reuse stale continuation fields as an all-purpose commitment mechanism.
...
### A9 — Phase 7 and Phase 13 recurring ACH finality
...
**Winner:** D4 and D10 for Phase 16 recurring ACH.
**Amendment:** for Phase 16 recurring ACH, processing persists agreement/occurrence/attempt evidence and an initiation confirmation/projection only. It is neither received money nor an official receipt. Provider-confirmed success posts and receipts once; a later return appends the exact inverse and receipt/statement supersession. One-time ACH keeps its separately governed Phase 13 policy.
**Required proof:** a processing→success→late-return fixture must show zero received/posting/official-receipt facts at processing, exactly one posting and receipt at success, and exactly one inverse plus receipt/statement supersession after return, including duplicate and out-of-order event replay.
...
**Amendment:** Phase 16 derives typed domain meaning, purpose-specific recipient projections, and deterministic candidate keys, then calls the Phase 6 idempotent intent-submission seam. Phase 6 owns the durable pre-dispatch intent, governed delivery-profile version, consent/contactability, dispatch, communication event, and outcome; Phase 17 owns governed templates and rendering. The exact email provider remains replaceable. A UI may state only the evidenced submission/delivery state and never “donor aware.”
...
8. Meaningful transition communication is idempotent and uses the Phase 6 spine; fixed-pledge reminders require explicit enrollment and tenant narrowing only.
...
| D5       | Skip one named occurrence; bounded or indefinite pause; unchanged normal grid; direct cancel; fresh authorization/new epoch on restart; no catch-up; cohort default with line split; missionary read-only pause visibility.                                                                                                                                                                                                 | Partially supersedes #708/#709; future notification delivery remains Phase 6/17.                                                                              |
...
9. **Communication candidates and seams:** D9/D19 record typed domain meanings and candidates; eligible current communications submit immutable intents to Phase 6 after reproof and semantic idempotency. Phase 6 creates events only at dispatch or in-product publication, and Phase 17 owns rendering. Future missionary pause email meaning has no Phase 16 runtime intent/event.
...
| Are communication policy, rendering and delivery ownership separated?                  | **Yes.** Phase 16 domain meaning/candidates; Phase 6 durable intent, consent, dispatch and delivery; Phase 17 rendering. |
