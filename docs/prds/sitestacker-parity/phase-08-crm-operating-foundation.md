...
> **Scope amendment (2026-07-06 — ADR-0001).** Twenty CRM has been retired as a product dependency ([ADR-0001](../../adr/0001-asym-postgres-owns-crm-truth-twenty-retired.md); [Phase 1 ownership matrix](./phase-01-source-of-truth-ownership-matrix.md)). The Twenty-facing scope of this PRD — Tranche 2 (Notes write-enable, T12), the provider write gates and provider-idempotency machinery as Twenty-write enablers, the "Twenty conformance" section, and every live-Twenty round-trip evidence requirement — is **withdrawn**. The surviving concerns (staff operations visibility, CRM data health, alert routing, env-flag hygiene) will be **re-groomed against Asym-internal subjects** in a dedicated session before any Phase 8 build starts. Do not build from this document until the re-groomed PRD lands; issues #587–#601 carry proposed edits pending founder approval.
...
- **`communication_events`** (Phase-6 table) — add **`recipient_email` (TEXT NULL)**, the first no-person writer, with an **exclusive-arc CHECK** (`donor_id XOR missionary_id XOR recipient_email` present). Phase 8 back-fills this Phase-6 schema detail; add `field_policies` census rows for any `communication_events.*` fields reaching export/Mission-Control surfaces.
...
**Hard prerequisites (Phase 8 cannot ship until both are merged):**
...
- **Phase 6** — supplies `communication_events` + `recordCommunication` capture at the single `sendEmail` seam (the vehicle for every emailed alert). Phase 6 itself hard-depends on Phase 4 **and** the Phase-3 consent gate, so Phase 4 is transitively doubly required.

**Inherited seams (consumed as designed):** Phase 3 (consent gate — **PR #502, currently OPEN/unmerged**; `field_policies`/export governance; role-scoped projection chokepoint; audit lane), Phase 7 (redaction wall + version-scoped idempotency), Phase 2 (tenant-scoped only, no `site_id`), Phase 0 (Built/Live/Confirmed).
