...
- **Status:** Groomed specification; issue set pending; not dispatched
...
| `legal_issuer_requirement` | `none`, `verified_us_issuer`, or `active_ca_registered_charity_issuer`.                                                                         |
...
| `us.contribution_acknowledgment.annual@1` | `official_tax`; Phase 19 selects population, Phases 7/13 own item facts | Legal contributor; exact frozen annual item set                        | Itemized acknowledgment; each gift preserves its own date, amount/property, and case | `accessible-archive-v1` | Dark until Phase 19 seam and U.S. gates pass              |
...
| `ca.official_receipt.individual_cash@1`   | `official_tax`; Phase 7 issuance + Phase 13 money                       | True donor for one eligible cash gift under one active verified issuer | CRA ordinary cash case, `ca_r_v1` serial, current signer                             | `accessible-archive-v1` | Absent unless issuer deliberately activates Canadian pack |
| `ca.official_receipt.cumulative_cash@1`   | `official_tax`; Phase 19 population + Phase 7 issuance                  | True donor; nonoverlapping source-frozen cash coverage                 | CRA cumulative cash case, one `ca_r_v1` serial                                       | `accessible-archive-v1` | Absent unless pack active and coverage proof passes       |
| `ca.official_receipt.non_cash@1`          | `official_tax`; Phases 7/13                                             | True donor for one eligible noncash gift                               | Property, FMV/deemed-value and eligible-amount facts required by Canadian contract   | `accessible-archive-v1` | Absent unless pack active and case proof passes           |
| `ca.official_receipt.advantage_split@1`   | `official_tax`; Phases 7/13                                             | True donor for one eligible split-receipting event                     | Payment/value, advantage description/FMV, eligible amount, protected wording         | `accessible-archive-v1` | Absent unless pack active and case proof passes           |
| `giving.summary.informational@1`          | `governed_business`; Phases 7/13/19                                     | Authorized donor for exact source-selected period                      | Plain informational summary; explicitly not an official tax acknowledgment           | `accessible-v1`         | Supported after core D3/D4 gates                          |
...
| `ca.cash.cumulative@1`   | Exact nonoverlapping item set and total plus every required item/date fact                          | One new serial; current signer                        | Phase 19 owns population/run; coverage cannot overlap prior official receipts                                 |
...
| Statement population/run         | Phase 19                                                       | Phase 18 accepts item-authoritative generation requests only                                                                                           |
...
| `pause_ca_issuance`         | active epoch            | Authorized reason/effective instant/impact                                                                                                                                                                         | New paused epoch; existing artifacts unchanged     | Deleting history or blocking gifts                                          |
| `lock_ca_authority`         | active/paused           | Confirmed suspension/revocation evidence                                                                                                                                                                           | Legal lock at effective instant                    | Queuing suspension-period gifts for later receipts                          |
| `end_or_reactivate_ca_pack` | active/paused/ended     | Impact/handoff or fresh full proof and continuity                                                                                                                                                                  | Closed interval or new active interval             | Toggle reversal or reused authority epoch                                   |
