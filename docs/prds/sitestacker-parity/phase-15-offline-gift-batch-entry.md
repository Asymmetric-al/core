...
**Deposit grouping.** Bank-bound tenders (check, cash, offline ACH, wire) carry a **deposit-state** — a separate operational dimension (undeposited → assigned → deposited, with Phase 20 clearance shown separately) decoupled from both entry and posting — and can be grouped into first-class **deposit groups** through a separate Phase 15 current-membership relation, with at most one current group per gift; posted financial rows carry no mutable deposit pointer. The relationship is fully flexible: deposit-before-entry, entry-before-deposit, 1:1, many-batches-to-one-deposit, or an inconsistent mix week to week all work, because nothing forces a rigid batch=deposit binding. Phase 15 owns the deposit group, exact membership revision, slip, and operational deposit-state. Phase 20 derives the balanced accounting handoff and bounded source-labelled Bank Match from that frozen source evidence; QBO/Xero owns the general-ledger account and final bank reconciliation. Stripe-settled gifts are excluded from manual deposits — they reconcile through exact Stripe payout evidence.
...
**D4 — Every tender first-class.** Phase 13 remains the single persisted tender vocabulary — `check, cash, ach, wire, securities, in_kind, church_remittance` first-class. Phase 15 may present `crypto`, `vehicle`, and `real_estate` as reserved **intake workflow labels**, but commit maps each to the actual arrival rail plus Phase 13's canonical noncash line subtype/asset identity; those labels never become a competing tender truth. Non-cash gifts share **one** `noncash_gift_details` extension (not five lanes); **describe-never-value** is a schema invariant across all channels (the org never attests value — Form 8283 is the donor's duty; the 8282 disposition clock ships with proceeds). Securities remain one gift with an append-only liquidation/proceeds lifecycle. Phase 15 is the sole canonical asset-lot/disposition/proceeds/finality source projection; each raw fact stays `non_contribution` and cannot directly create a Contribution, Field Account occurrence, or Accounting Release. Phase 21 D21 alone may derive an exact source-final Realized Support Basis, while Phase 20 remains separately capability-certified for any nonduplicate accounting delivery. In-kind is $0 recognized with walled valuation. The **phone lane** is a native embedded Stripe Payment Element keyed by staff (SAQ-A) with a server-confirm MOTO flag (support-gated per connected account; gate+detect+degrade), plus an ACH lane in both variants (mid-call Financial Connections as primary, a bounded secondary staff-keyed TEL lane behind its own capability and consent guardrails). Phone gifts are **online** gifts written by the Stripe webhook, dispatched and tracked from the workbench, never offline money rows. A distinct `take_phone_payment` capability gates the lane. This targets the **Phase 13 Stripe Connect connected-account substrate from day one** (Connect is already ratified in Phase 13); the phone lane never stores, logs, or processes raw card or bank details. _(Amended 2026-08-01 by Phase 21 D21 without reopening D4's one-gift, one-generic-extension, or describe-never-value rulings.)_
...
**D6 — Deposit grouping via the undeposited-funds spine.** First-class `deposit_groups` + a separate Phase 15 **gift-grain current-membership relation**, with at most one current group per gift and no mutable pointer on posted financial rows, + a **derived** deposit-state (a separate operational dimension formalizing recorded→deposited with Phase 20 clearance shown independently and retiring the flat `deposit_reference` TEXT), decoupled from both entry and posting. A `settlement_rail` discriminator (bank-direct vs Stripe-rail) governs eligibility: bank-direct check/cash/ACH/wire are depositable; Stripe-rail card/ACH are forced `settles_via_payout` and the DB rejects any deposit link (prevents double-count vs payout). Maximum flexibility: temporal ordering is unfixed (deposit-before-entry, entry-before-deposit, simultaneous), batch-to-deposit cardinality is unfixed (1:1, N:1, 1:N); each gift still has at most one current membership. Assignment is optional and changeable after posting through the separate owner relation. A printed slip is a retained **immutable snapshot**; membership stays free-to-edit until a Phase 20 Source Coverage Manifest freezes that exact revision, then changes require append-only successor/correction evidence. Phase 15 owns grouping, slip/report, and operational state. **Phase 20 owns the downstream balanced Accounting Effect, Expected Bank Arrival, and bounded Bank Match; QBO/Xero owns the native undeposited-funds/clearing account and final bank reconciliation.** Deposit membership moves no source money, so it never creates a Phase 15 compensating posting; an append-only `deposit_assignment_events` log preserves every reassignment.
...
- **Phase 20 (Accounting Exports & Reconciliation)** — FORWARD; **out of scope for Phase 15.** Owns immutable downstream Accounting Releases, Expected Bank Arrivals, and bounded source-labelled Bank Match. A noncash disposition or D21 realization remains accounting-dark until Phase 20 separately certifies one non-overlapping canonical economic source; Phase 15 cannot emit accounting work merely because proceeds are source-final. Phase 15 owns the deposit group/membership, slip, and operational deposit-state. QBO/Xero owns native GL accounts, books, and final bank reconciliation; no Phase 15 or Phase 20 status column may pretend otherwise.

- **Phase 21 D21 (Noncash Support Realization)** — FORWARD and downstream. It consumes only the exact source-final, non-overlapping canonical projection Phase 15 owns, derives at most one Realized Support Basis under its pinned cost treatment, and hands that basis to D2/D11. It never changes the original Contribution or turns a raw Phase 15 row into a Field Account occurrence. Missing source lineage, finality, quantity coverage, or proceeds evidence remains a Phase 15 source-owned exception.
...
60. As a **batch-entry operator** recording a securities (stock) gift, I want it modeled as **one gift with a lifecycle** — the original gift plus source-mode-labelled, append-only liquidation/proceeds facts — so that donated stock and the cash it later becomes remain one honest record, not two counted gifts. A raw disposition row creates no Contribution, Field Account occurrence, or accounting posting; Phase 21 D21 and Phase 20 consume only their separately certified projections. `[AG, D4, P21 D21]`
...
64. As a **batch-entry operator** recording a vehicle, real-estate, or crypto gift, I want **one generic noncash shape** with a reserved draft intake label and duty flags that commit to the actual Phase 13 tender plus line subtype (no bespoke per-asset engine in v1), so that unusual noncash gifts can be recorded correctly without over-building five specialized lanes or creating a second tender vocabulary. `[AI, D4, P21 D21]`

64a. As a **finance lead** tracking a partial sale, pooled lot, installment,
provider liquidation, or terminal nonmonetary outcome, I want one exact
Tenant-, Legal-Entity-, source-role-, asset-lot-, purpose-, and currency-scoped
source projection with understandable status and one source-owned repair task,
so that Phase 21 can use proved proceeds without staff estimates, duplicate
gifts, or hidden quantity gaps. `[P21 D21]`
...
| `crypto` / `vehicle` / `real_estate` | noncash-reserved  | n/a                            | non-money tally            | intake labels map to P13 line subtype + actual rail; no engine v1   |

**`moto` is not a `gift_method`.** A phone card gift is `card`; MOTO is a _capture-channel attribute_ expressed as a server-confirm flag (5.8.3), never an enum value. Likewise, the reserved `crypto` / `vehicle` / `real_estate` draft labels select the noncash intake experience; commit records the actual Phase 13 tender/arrival rail and canonical line subtype rather than persisting a second tender vocabulary.
...
The workbench's `gift_method` field remains the D4 draft discriminator, not a
second Phase 13 tender/domain authority. For ordinary money it maps directly to
the Phase 13 tender. For reserved noncash labels, commit preserves the actual
arrival rail separately and maps the property to Phase 13's canonical noncash
line subtype/asset identity. A later disposition never changes that original
rail or reclassifies the gift as cash. The mapping is one closed, versioned
server descriptor shared by validation and commit; no UI or import caller may
choose a different mapping.

Liquidation/disposition outcomes live in append-only
`gift_disposition_facts` plus exact lot/purpose coverage facts (mirrors P13
posting immutability / ADR-CD-004). Phase 15 composes those rows into the **one
canonical source projection** for asset identity, source role, disposition,
proceeds, source-specific finality, evidence, and corrections. Every raw row is
typed `non_contribution` and **CI-gated out of every contribution money
aggregate**, exactly as Phase 14 (Donor Credit Operations) gates soft credits —
a test asserts no raw row can itself emit a Contribution, Field Account
occurrence, or accounting posting. Phase 21 D21 may consume only exact
source-final, non-overlapping projection coverage to derive a separate Noncash
Support Realization; Phase 20 may consume it only after a separately certified
nonduplicate accounting-source decision. P15 migrates nothing.
...
- **Liquidation proceeds** (`sale_date`, gross, exact costs, net, or an exact
  net-only provider shape) are append-only source and lot/purpose coverage facts,
  `non_contribution`, CI-gated out of every contribution money aggregate. The
  "record liquidation" surface is reachable **only from an existing noncash
  gift**, never from "new gift"; the workbench soft-blocks an ordinary cash row
  that duplicates an open tenant-held/tenant-agent asset's expected proceeds.
  An intermediary or DAF sponsor that was the legal donee and later sends the
  tenant cash follows the ordinary cash/grant source lane instead of this bridge.
- P15 does **no** gain/loss math, deposit matching, Field Account posting, or GL
  posting. Phase 21 D21 may derive only an exact source-final Realized Support
  Basis; Phase 20 remains separately gated.

A securities contribution posts when its Phase 15 source and Phase 13 ledger requirements pass, independently of Phase 7 receipt facts. Liquidation, transfer confirmation, proceeds, valuation true-up, and evidence are append-only post-posting facts worked in **one detached follow-up worklist that reuses the Mission Control follow-up-task infra** (D1.a-permitted). That follow-up never delays posting, reopens the posted batch, or creates a second contribution. If the UI exposes `liquidation_status ∈ {not_applicable, pending, partial, settled}` plus an aging view (`open → resolved / abandoned / written_off`), both are derived projections over those append-only facts; each open item has an explicit finance owner-role + SLA. `settled` is a plain-language staff projection meaning the recorded disposition appears complete for its current source version; it is **not** the D21 admission predicate. The canonical projection emits `source_final=true` only when the versioned source-family rule proves the exact disposition/tranche, amounts or terminal outcome, evidence, and completeness watermark. A staff status choice cannot set it.

#### 5.5.1 Phase 21 D21 canonical disposition projection amendment

Phase 15 is authoritative for the sole asset-lot/disposition/proceeds/finality
source projection that D21 consumes. It preserves complete Tenant and Legal
Entity scope; exact original Contribution and accepted-purpose line; immutable
Phase 13 asset identity plus normalized source lot, quantity, scale, tranche,
and non-overlapping covered quantity; source family, provider organization,
environment, account, legal-recipient/source role, and immutable source
object/event/version; canonical economic root; disposition kind; source-
effective, sale/trade, source-final, discovery, and recorded timestamps; ISO
currency; exact gross, actual cost, and net components or an explicitly
certified exact-net-only shape; per-lot/per-purpose allocated proceeds and
rounding residual; source-specific finality rule/version, finality state,
completeness watermark, and private evidence reference/digest; terminal
nonmonetary outcome where applicable; semantic idempotency; and append-only
correction/supersession lineage.

`source_mode` is closed and evidence-derived:

- `tenant_held_asset` — the tenant/Legal Entity was the legal recipient and held
  the property; exact final proceeds may be D21-eligible;
- `tenant_agent_liquidation` — a proved provider acted as that tenant/Legal
  Entity's agent and retained exact asset-to-proceeds lineage; exact final
  proceeds may be D21-eligible;
- `intermediary_cash_grant` or `daf_cash_grant` — another legal donee liquidated
  its own asset and sent cash; this is an ordinary cash/grant source and is
  never a D21 asset realization for the tenant; and
- `terminal_nonmonetary` — retained, used, donated onward, abandoned, or
  worthless property; it closes the source obligation but creates no monetary
  support.

This is one versioned read projection over Phase 15 source facts, not a second
gift, mutable sale row, generic money aggregate, Field Account entry, bank
deposit, or Accounting Release. Partial sales, pooled lots, installments,
corporate actions, canceled trades, retained/used/donated-onward/abandoned/
worthless property, delayed costs, and adverse corrections remain append-only,
tranche-explicit, and quantity/proceeds-covered. `source_final` is derived by
the pinned source-family rule; the UI term `settled`, a provider's generic
`complete`, or a staff attestation is never a universal substitute. If exact
lineage, scope, coverage, component truth, evidence, or finality is missing or
ambiguous, the source worklist owns one cause-labelled exception; D21 cannot
fill it with staff-estimated proceeds, inferred zero fees, an appraisal, or a
convenient valuation.

The original noncash `recognized_value`, internal valuation, FMV, appraisal,
and provider estimate are structurally excluded from Phase 21 D2 candidate
generation. Only an exact source-final D21 Realized Support Basis may later be
admitted through D2/D11. The Phase 15 contribution lifecycle remains one gift,
and its donor, receipt, campaign, supporter, and fundraising facts do not run a
second time.
...
Crypto, vehicles, and real estate are enterable in v1 **through the single
`noncash_gift_details` shape**. Each has a reserved draft intake discriminator
(never silently flattened to `in_kind`) that commit maps to the actual Phase 13
arrival rail plus canonical noncash line subtype, and each carries a per-asset
duty flag that raises a signal:
...
- P14 acknowledgment machinery — `contribution_credits` + separate acknowledgment request/source records + the NF3 "Send acknowledgments" gate (P14 epic #719): the receipt/acknowledgment wall is enforced against it.
...
Deposit grouping answers a physical-world fact: staff carry checks and cash to a bank as deposits, operationally distinct from gift-entry batches. Phase 15 owns grouping, source-labelled deposit-state and the immutable slip source snapshot/content; Phase 18 owns qualified generated-document rendering, artifacts and current object access. Phase 20 consumes exact deposit revision/coverage to own the canonical Accounting Effect and bounded Bank Match; QBO/Xero owns the final general ledger and bank reconciliation. Phase 15 builds none of those downstream engines.

The model is the **undeposited-funds spine** (the QuickBooks / Aplos / MinistryPlatform / Virtuous canonical): a first-class `deposit_groups` entity, a separate Phase 15 **gift-grain current-membership relation** keyed by exact Tenant/Legal Entity/contribution identity and allowing at most one current deposit group per gift, and a **derived** deposit-state that is decoupled from both the gift-entry-batch lifecycle (D2/D5) and the posting lifecycle (P13). The membership relation is outside the immutable contribution header, designation lines and postings. One simple shape, exercised three ways, covers all six founder-named workflow variants (V1–V6) with no configuration switch and no special cases. This is the leanest compliant shape; the cut list at the end is binding "do not build."
...
Temporal ordering of entry-vs-deposit is UNFIXED (before, after, simultaneous all valid). **Batch-to-deposit** cardinality is UNFIXED (1:1, N:1, and the founder's "1:N" — different gifts from one entry batch assigned to different deposits). Each gift has at most one current membership in the separate Phase 15 owner relation; neither 1:N batch grouping nor reassignment permits simultaneous multiple current deposits for a gift or a mutable pointer on a posted financial row. This one current-membership shape supports every variant.

### D6.1 — The separate deposit-state dimension [HD-2, HD-3]

Deposit-state is a **separate Phase 15 operational dimension**, joined to the immutable Phase 13 contribution rather than stored on its header. It formalizes the accepted recorded/banked waypoints without duplicating payment, posting, receipt or accounting authority. Phase 20 owns Bank Match/clearance evidence; Phase 15 displays that owner result separately and cannot author it. The retired flat `deposit_reference` field is not a mutable financial-row carrier (C-02 / ADR-0206).
...
Current gift-grain membership and admitted return/NSF evidence live in separate Phase 15 operational records and the append-only assignment/event history. Deposit-state derives from that exact current membership, physical-banking/slip evidence and the immutable tender/settlement-rail facts. No returned flag, membership pointer or clearance cache is written onto a posted financial row.
...
<a id="d62--the-gift-grain-scalar-link-hd-1"></a>

### D6.2 — The separate gift-grain current-membership relation [HD-1]

**Accepted C-02 storage ruling (2026-09-23):** use a separate Phase 15-owned **gift-grain current membership relation**, keyed by exact Tenant, Legal Entity and contribution identity. Its one nullable `deposit_group_id`/current-assignment pointer is outside `contribution_headers`, lines and postings. The [accepted ADR-0206](../../adr/0206-immutable-posted-financial-facts-and-owner-operational-state.md) preserves the same at-most-one current deposit and append-only move semantics; it changes physical placement, not deposit policy. Constraints remain structural:

- **≤ 1 current deposit per gift, many gifts per deposit.** A unique same-Tenant/same-Entity contribution key in the separate membership relation and one nullable exact-scope group reference enforce this cardinality. Historical assignment events do not create a free-form N:M current-membership model; the original double-deposit prohibition remains binding.
...
- **Separate operational membership, never a financial-row update.** The entire posted header, every designation line and every posting are immutable, including future columns. Attach/detach/reassign changes only the Phase 15 membership head/revision and appends its exact move event; grouping moves no money and creates no compensating contribution posting. An operational reset cannot clear the durable financial freeze.

Every accepted assignment, reassignment and detachment appends one `deposit_assignment_events` occurrence with exact source identity, expected prior assignment sequence, actor, reason and from/to group. The current membership head, affected group membership revisions and event/audit/receipt facts advance atomically under the existing owner CAS/lock protocol. The pointer is a coherent operational projection of that history, never a second independent assignment authority or a header column.
...
**Separate Phase 15 gift-grain membership and owner evidence** (C-02 / ADR-0206; created or qualified alongside the minimal source substrate, never a posted-header extension):

- One current membership per `(tenant_id, legal_entity_id, header_id)`, enforced structurally; its nullable `deposit_group_id` references the exact same-scope group with no CASCADE or SET NULL. The current pointer carries the exact accepted assignment revision/sequence and expected-prior CAS relation; append-only move history and currentness commit together.
- `settlement_rail` remains an immutable qualified source fact consumed from the Phase 13 contribution/tender. Membership commands re-prove bank-direct eligibility, exact Legal Entity, currency and bank-account scope; the separate relation cannot override the rail to admit a Stripe payout item.
- Returned/NSF operational evidence references the exact admitted return occurrence and retains historical membership. Any money reversal remains an append-only Phase 13 effect; changing a Phase 15 indicator cannot create, edit or reverse money.
- Clearance/date and Bank Match are Phase 20 owner results in qualified joined projections, never operator-maintained fields or a clearance cache on a posted financial row. Tax dating remains the separate Phase 7 authority.

**Index plan:** qualify an indexed, bounded undeposited query over immutable source eligibility and the current Phase 15 membership relation (join/anti-join by exact same-Tenant/Entity contribution identity). Index the operational current/unassigned slice and source lookup keys to make aging/workbench reads bounded and self-pruning; prove the plan with the existing large-corpus fixtures. Do not add a mutable membership predicate/column to `contribution_headers` or a full-table `(tenant_id, deposit_state)` low-cardinality btree. Tax `gift_date` remains exclusively in Phase 7’s versioned facts [HD-7, HD-14].
...
The deposit `expected_total` is **INDEPENDENT** of the D2 batch control-total and legitimately unequal under batch-to-deposit N:1 / 1:N grouping (each gift retains at most one current membership). The two mismatch signals are never conflated. The deposit total is a **soft reconciliation aid** — a live selected-vs-expected tally and a **non-blocking** mismatch warning — NEVER a second commit/post gate. Deposit grouping is a post-commit operational activity; D2's blocking control-total already lives on the entry batch (CB-F confirmed).
...
Phase 15 defines and freezes the exact deposit revision/member manifest and slip source snapshot/content below. The printable **tagged-PDF + accessible-HTML** artifact is admitted/rendered through the qualified Phase 18 GeneratedDocument contract; Phase 15 supplies no independent renderer:
...
The slip is aggregated donor-PII egress: generation/view/export requires `finance:manage_deposits` plus the exact purpose, recipient, source-scope and current object authority required by Phase 18, with audited access. Default to a **PII-minimized** source variant (tender list + amounts; donor identity opt-in). No deposit refs, donor data or check numbers appear in URLs. Every full/range/print access re-proves current object authority; a Phase 29/Supabase/provider signed URL is not product access authority. Phase 29 remains the separate scanned-file/image custody seam.

**Purpose qualification is required before generated slip support.** Resolve and pin the exact existing Phase 18 purpose/version and registered Approved Data View under the [executable purpose registry](./phase-18-document-purpose-authority-manifest.md). `custom.business_document@1` is usable only if its already-governed safe-view and purpose contract admits this exact source/content; its presence is not proof that deposit slips are registered or supported. If no current contract qualifies, generated-slip execution stays blocked on the owning purpose/view/renderer qualification. Do not invent a deposit-purpose key, widen the eleven-purpose launch catalog or bypass it with a local renderer, template query or signed URL.

**The seam is explicit:** Phase 15 owns the operational grouping workflow,
current deposit revision and immutable SLIP source snapshot/content. Phase 18
owns qualified generated-document rendering, retained artifacts and access. Phase 20 atomically consumes an
...
same-day} × {batch-to-deposit 1:1, N:1, 1:N reassignment; at most one current membership per gift}, plus: a two-staff compare-and-set
...
FORWARD (groomed dependencies, not fresh execution proof): Phase 13’s immutable contribution headers/lines/postings and qualified source rail/identity are supplied by the minimal posting substrate (#690/#762). Phase 15 current membership/return evidence lives in separate operational records with append-only assignment history; no deposit/status/returned column is updated on a posted financial row. The former flat `deposit_reference` is superseded by D6. Phase 18’s exact purpose/view/renderer qualification is required for generated slip artifacts and current object access; no existing deposit-purpose support is presumed. Phase 20 owns immutable Accounting Release/delivery and bounded Bank Match; QBO/Xero owns final books. Phase 29 is the separate scanned-file/image custody seam, with RDC/scanner integration reserved. The predecessor substrate owns `deposit_groups`, gift-grain current membership, `tenant_bank_accounts` and `deposit_assignment_events` once.
...
> Batch-origin acknowledgment purposes land in separate Phase 14 request/source records held for `batch_gate_pending`. The explicit per-batch **"Send acknowledgments"** action asks Phase 14 to release only the exact ready, currently authorized consolidated scope; Phase 17 preparation and Phase 6 dispatch/delivery remain separate owner facts. NF3 adds one human release edge, no second sender or copied outcome state, and never creates Phase 7 tax-receipt authorization or a Phase 18 official generated-document request.
...
- **Block C — held & won't-send, each with a plain reason — ALWAYS VISIBLE, never collapsed**: _Held (requires current owner readiness and its admitted batch release)_ — waiting on credit generation, ambiguous attribution or the explicit human gate (deep-link to the Phase 14 Attribution Inbox where appropriate); an unready or new late-delta occurrence never auto-sends merely because it later becomes ready; _Scheduled (not sent from here)_ — tribute notify-parties on a monthly/decay cadence (the gate un-gates only the immediate first notice, never overrides donor-set cadence); _Won't send_ — `do_not_contact` / tribute `never` / no email on file.
...
**Recipient correction/new occurrence (escape valve, not a re-blast).** Results may invoke the current responsible owner command for a source-authorized correction or distinct admitted occurrence for one recipient, preserving the original request, coverage and communication history. Reuse the exact accepted semantic identity for retries/recovery; no fresh salt, new key or “not sent” inference may bypass source coverage or release guards. A bulk re-press converges on the existing release. **No bulk "re-send whole batch."**

---

### S8.15 — NF3 state model, the P14 amendment, the 7th P8 signal

**State model — reuse separate Phase 14 request/source records; derive the rollup.** NF3 adds no P15 per-recipient queue, header acknowledgment column or copied communication state. Batch-origin source purposes retain `held` / `batch_gate_pending` until their exact ready scope is deliberately released through Phase 14. Preserve consolidated requests and exact source relations; do not fabricate one request per header. The batch rollup joins Phase 14 purpose/readiness/release, Phase 17 preparation and Phase 6 outcome records by exact authorized source identity/revision, without duplicated join counts or a stored batch enum. Template/time evidence comes from the owning occurrence/command, never a bare “sent” Boolean.

**Late-arriving delta (ADJ-6).** New gifts still use a new/follow-on batch. A later credit or source-authorized correction meaning may create a distinct exact Phase 14 purpose occurrence held for the existing batch gate; show only that newly eligible scope, not the already-released set. The button derives from current qualified pre-release request/readiness and source coverage, with owner semantic idempotency. A missing, failed or uncertain Phase 6 outcome does not re-arm already-released source coverage or authorize a fresh send identity.

**The Phase 14 boundary (NF3; C-02 storage synchronization).** Batch-origin purposes are held for `batch_gate_pending` and the explicit per-batch human gate; import-origin purposes remain held and never pass this batch gate. Imports never auto-send; any separately admitted import/tribute activation remains with its existing owner contract. Auto-send suppression remains intact. Reuse the separate Phase 14 request/state/source relations and existing consolidated cardinality, not legacy `acknowledgment_status` / hold columns on the financial header. Phase 15 requests the owning release command and never writes its sender/outcome state; the tax-receipt owner chain is unchanged.

**The 7th Phase 8 data-health signal — `acknowledgments-pending-past-N-days`** — derives from exact owner-labelled held/readiness and preparation/communication failure projections, not a shared header status. Preserve the tenant-configurable age threshold (default about seven days), default-on/tunable/snooze/disable behavior and batch/source scope. Missing owner data is unknown, not zero or permission to retry under a new identity.
...
**What NF3 deliberately does NOT build (binding cut list):** no P15 send pipeline/queue/state machine or financial-row acknowledgment fields; no mandatory per-type toggle or per-recipient review; no “re-send whole batch” or cross-batch send-all; no new acknowledgment approval/second-approver policy; no unsend/void machine; no template authoring (Phase 17 owns it), scheduler/drip/journey, import-origin UI, acknowledgment analytics/open tracking or printable/CSV manifest v1. Preserve the existing source/Phase 17/6 commands, tribute source cadence, current authorization and owner-qualified pre-release cancellation/recovery. C-01’s money-correction rule remains separate.

**Buildability checklist:** preserve the one AcknowledgmentsPanel/Card with summary/rail/quick-entry mounts; exact pre-release manifest with per-viewer projection and qualified variables; source/revision-coherent batch rollup over separate owner records; current-capability-checked `sendBatchAcknowledgments` calling Phase 14’s exact consolidated request/source release command; readiness/drift integration; existing Phase 17/6 owner handoff; owner-labelled Results and permitted recovery; pending-age data-health filter; and accessible status tokens. There is no NF3 outbox drainer or state flip bypass. Cancellation names the owner boundary: Phase 14 pre-source-release cancellation may withdraw source work; after that release only Phase 6/17-qualified definitely-unsubmitted custody cancellation may stop the communication, using its exact state-version guard without reopening Phase 14 coverage. Dispatching/submitted/indeterminate work cannot claim recall or obtain a fresh identity.

**Test grid:** idempotent double-press; exact owner-scoped cancellation races (Phase 14 pre-source-release, or qualified Phase 6/17 definitely-unsubmitted custody after source release without reopening Phase 14 coverage); no recall of dispatching/submitted/indeterminate work; late-credit new-occurrence delta send-once; consent-suppressed visibility; tribute `never` + cadence "Scheduled"; restricted-projection manifest correctness + count reconciliation + cleared-actor gating; readiness-partial send-and-sweep; dead-letter surfacing; quick-entry inline line present/absent; the **three-document-wall assertion (the acknowledgment gate never creates tax-receipt authorization or an official generated-document request).**
...
One shape serves securities, in-kind, crypto, vehicle, and real estate (the
reserved draft labels map to Phase 13 line subtypes plus the actual arrival
rail) — **not five intake lanes**. Source facts are append-only. Phase 21 D21
and Phase 20 consume only separately certified projections; neither migrates or
re-owns these facts.
...
- liquidation_status — derived read projection over append-only
  ...
  settled}; it is not mutable batch state, never gates or reopens posting, and
  is not the D21 `source_final` predicate — [D4, P21 D21]
```

### `gift_disposition_facts` — proceeds, non-contribution [D4 A4]

Append-only liquidation facts for securities/noncash. **CI-gated out of every
contribution money aggregate** exactly as Phase 14 gates credits — a disposition
row can never itself emit a Contribution, Field Account, or accounting posting.
The source projection described in 5.5.1 may enrich these facts for exact D21
consumption without changing their `non_contribution` meaning.

```
gift_disposition_facts (FORWARD, append-only event/version root)
- id / tenant_id / legal_entity_id — uuid — explicit same-scope identity; FORCE RLS
- header_id / designation_line_id — uuid NOT NULL — composite same-Tenant/same-Entity FK to the
  committed Phase 13 noncash Contribution and accepted-purpose line; reachable only from that
  existing gift, never from "new gift" — [D4, P21 D21]
- fact_kind — text NOT NULL — CHECK = 'non_contribution' — the CI tripwire; a schema lint asserts it — [D4]
- source_family / source_mode — text NOT NULL — closed source adapter and CHECK ∈
  {tenant_held_asset, tenant_agent_liquidation, intermediary_cash_grant, daf_cash_grant,
  terminal_nonmonetary}; source mode derives from legal-recipient evidence, never staff convenience
- provider_organization_id / provider_environment / provider_account_ref — text NULL — exact
  provider scope where applicable; not tenant-selecting authority
- legal_recipient_party_id / source_role — uuid / text — exact legal recipient and role evidence;
  tenant-held/agent eligibility fails closed unless it resolves to this Legal Entity
- source_object_id / source_event_id / source_version — text NOT NULL — immutable external or manual
  evidence identity/version; UNIQUE with tenant, entity, source family, environment
- canonical_economic_root_id / disposition_tranche_key — text NOT NULL — semantic idempotency and
  partial/installment/corporate-action grouping
- disposition_kind — text NOT NULL — sale, redemption, liquidation, retained, used,
  donated_onward, abandoned, worthless, canceled, or source-specific mapped kind
- source_effective_at / sale_or_trade_at / discovered_at / recorded_at — timestamptz — separate exact
  clocks; gift date remains Phase 13 and is never overwritten
- amount_shape — text NOT NULL — CHECK ∈ {gross_cost_net, exact_net_only, nonmonetary}
- currency — text NULL — validated ISO-4217; required for monetary shapes, NULL for nonmonetary
- gross_proceeds_minor / actual_cost_minor / net_proceeds_minor — bigint NULL — checked integer minor
  units; gross − cost = net for `gross_cost_net`; gross/cost MUST be NULL for `exact_net_only`
  (never infer a zero fee); all three MUST be NULL for `nonmonetary`
- source_finality_state / source_finality_rule_version — text NOT NULL — adapter-native state plus the
  exact versioned rule used to derive source_final; staff cannot set source_final
- source_final_at / completeness_watermark — timestamptz / text NULL — required when source_final;
  a generic UI `settled` label is insufficient
- evidence_ref / evidence_digest — text NOT NULL — private retained source proof and integrity digest;
  no public URL or binary in this table
- corrects_fact_id / supersedes_fact_id — uuid NULL — composite same-scope self-FKs; corrections append
- created_by / created_at — uuid / timestamptz — append-only; no UPDATE/DELETE path — [D4]

gift_disposition_coverage_facts (FORWARD, append-only exact coverage)
- id / tenant_id / legal_entity_id / disposition_fact_id — uuid — composite same-scope FK; FORCE RLS
- asset_lot_id / phase13_asset_identity_ref / designation_line_id — text / text / uuid NOT NULL — exact
  original asset and accepted-purpose lineage; no fuzzy donor/date/amount match
- quantity_minor / quantity_scale — bigint / smallint NOT NULL — exact signed source quantity;
  fractional units never use floating point
- covered_proceeds_minor / allocation_residual_minor / currency — bigint / bigint / ISO-4217 NULL —
  exact per-lot/per-purpose allocation and deterministic residual for monetary outcomes
- coverage_seq / corrects_coverage_id — bigint / uuid NULL — monotonic, append-only correction lineage;
  overlapping positive coverage for one source version/lot/tranche is DB-rejected
```

`noncash_disposition_source_projection` is a tenant-filtered `SECURITY INVOKER`
read projection over the two append-only tables plus Phase 13's immutable
Contribution/accepted-purpose/asset references. It emits one exact row per
source event × asset lot × purpose × tranche × currency with the fields in
5.5.1, including derived `source_final`, total/covered/uncovered quantity and
proceeds, and correction lineage. It has **no write path** and is the only D21
source contract. Existing simple manual capture maps into it only when exact
evidence, scope, component truth, finality, and coverage are sufficient;
otherwise it remains a source-owned exception and creates no positive
realization. A raw row, a UI `settled` label, or the original noncash valuation
can never bypass this projection.
...
gift-grain current deposit membership — separate Phase 15 operational relation
- tenant_id / legal_entity_id / header_id — exact source key, UNIQUE for one current membership
- current_deposit_group_id — nullable, composite same-scope group FK; no CASCADE / SET NULL
- current_assignment_seq / revision — CAS-bound to the append-only assignment event fold
- a membership change, affected group revision and exact event/receipt/audit facts commit atomically
- no unrestricted N:M current memberships; no financial-row pointer, counter or return-status write
...
### Phase 14 acknowledgment request reuse — no financial-row or sender-state copy [NF3]

Batch-origin acknowledgments (DAF advisor thank-you, tribute notifications, soft-credit acks) reuse the **separate Phase 14 request and exact source-relation records**. Preserve the owning consolidated-request cardinality; a request may cover its admitted contribution/settlement set, not one new request per header. NF3 retains the source hold reason `batch_gate_pending` and the explicit per-batch human release of the exact ready, currently authorized scope. Phase 14 owns purpose/audience/readiness/coverage/release; Phase 17 owns preparation and Phase 6 owns dispatch/delivery outcomes. The batch rollup joins those exact owner records and source relations without header acknowledgment columns, copied outcome state, duplicate queues or source-count multiplication. Phase 14 pre-source-release cancellation uses its shared release/cancel guard. After Phase 14 source release, only an exact Phase 6/17-qualified definitely-unsubmitted cancellation may stop that communication under its current owner state-version guard; it never resets the released Phase 14 coverage or frozen request. Dispatching/submitted/indeterminate work cannot claim recall and stays on its same-identity recovery path. No acknowledgment path creates Phase 7 official receipt authorization or a Phase 18 official generated-document request.

---

**C-02 storage proof.** Run the existing D6 V1–V6 / tender / timing / cardinality
matrix against the separate membership relation. Prove both concurrent CAS orders,
wrong Tenant/Legal Entity/currency/account/rail, stale assignment and consumed
revision, rollback/replay, retained NSF membership and coherent joined reads.
Every legitimate attach/detach/reassign/return projection leaves posted financial
rows byte-for-byte unchanged; raw/privileged header changes fail even after an
operational reset. Historical move/Source Coverage evidence remains immutable;
only exact successor revisions may advance. These tests do not change C-01.
...
3. **Stripe-not-depositable.** `settlement_rail='stripe_rail'` rejects every group-targeting membership command/event at the DB/owner boundary, including privileged writes. Eligibility comes from immutable source rail and exact Tenant/Entity/currency/account proof, never from a caller override. [D6 HD-5; C-02]
4. **One-deposit-only + conservation.** A unique exact source key and one nullable group reference in the separate membership relation permit at most one current deposit per gift. Expected total remains a frozen soft aid with live actual/persisted variance, never a hard gate or silently rewritten value. [D6 HD-6/HD-7; C-02]
...
6. **Separate operational membership, append-only movement.** Attach/detach/reassign updates only the Phase 15 current membership/revision and appends its exact assignment event; the posted header/lines/postings remain byte-for-byte unchanged. Grouping moves no money and adds no compensating posting. Durable financial freeze survives every operational status/membership reset. [D6 HD-2; C-02 / ADR-0206]
7. **Deposit-state derived, not stored.** Derive current grouping from the exact assignment fold/current membership and physical-banking evidence; admitted return/NSF evidence remains operational and preserves membership. Phase 20 clearance/Bank Match remains a separate owner projection. No deposit-state enum or return/clearance cache lives on a financial row. [D6 HD-3; C-02]
...
26. **One source projection; no valuation shortcut.** Phase 15's
    `noncash_disposition_source_projection` is the sole canonical
    asset-lot/disposition/proceeds/finality contract. Its raw facts are always
    `non_contribution`; only exact source-final, same-scope, non-overlapping
    lot/purpose/tranche/currency coverage may be read by D21. Original recognized
    value, FMV, appraisal, claimed value, provider estimate, UI `settled`, and
    staff attestation can never create a D2 candidate. [P21 D21]
27. **Exact component and quantity conservation.** Monetary source facts are
    either proved `gross_cost_net` with `gross − actual_cost = net` or proved
    `exact_net_only` with gross/cost absent—not zero; terminal nonmonetary facts
    carry no amounts. Covered plus uncovered quantity equals the exact source
    lot/tranche quantity at one scale, and per-purpose proceeds plus the explicit
    deterministic residual equals the exact source proceeds. Replay or a
    correction cannot overlap existing positive coverage. [P21 D21]
...
**2. Deposit service (create / attach / detach / slip) — one service, two homes.** `createDepositGroup`, `attachGift`/`detachGift`/`reassignGift` compare the exact expected current membership/assignment revision under the cooperative owner lock. A bounded synchronous command updates only separate membership/group operational records and appends one immutable move event per accepted change, with command receipt/audit facts atomically; it is never an UPDATE of a posted header or the `contribution_operation_batches` async money saga. Preserve at-most-one current membership, exact Tenant/Entity/currency/bank-account/rail constraints and NSF history. Phase 20 freezes only each consumed membership revision as Source Coverage; later changes append a successor revision and any required cause-linked accounting correction, never rewrite consumed evidence. The service freezes the P15 slip source snapshot and invokes only the exact qualified Phase 18 purpose/Approved Data View request seam; Phase 18 owns tagged-PDF/accessible-HTML rendering, retained artifacts and current object authorization. Preserve capability/audit/PII controls; unsupported purpose/view/renderer qualification blocks generated-slip execution, never falls back to a local renderer or raw signed URL. Phase 29 scanned-file custody and Phase 20 clearance remain separate.

**3. `sendBatchAcknowledgments` service (NF3).** Resolve the exact selected batch/source scope through current access, source readiness and consolidated Phase 14 request relations; invoke that owner’s expected-revision release command under its permanent semantic identity. Recheck restricted-source authority and retain unready/unauthorized purposes held. Join Phase 17 preparation and Phase 6 outcomes for the response, never flip header/request/event state directly or enqueue a new P15 sender operation. A repeated action converges on the same source release. Phase 14 pre-source-release cancellation and Phase 6/17 definitely-unsubmitted custody cancellation are distinct owner commands. The latter may stop a qualified communication after source release only with its exact owner state-version guard, preserving the frozen Phase 14 request and released coverage. Dispatching/submitted/indeterminate work uses same-identity reconciliation and cannot become a fresh send from “not sent yet.”

**4. `projectNoncashDispositionSource` — the sole D21 source contract.** Given
an exact `(tenantId, legalEntityId, sourceFamily, canonicalEconomicRootId)`, it
reads the real append-only source and coverage rows through the same-scope
`SECURITY INVOKER` projection and returns immutable source versions with exact
lot/purpose/tranche/currency coverage, component shape, evidence digest,
source-finality rule/version, completeness watermark, and correction lineage.
It never accepts a caller-supplied `source_final`, never emits an original
valuation as proceeds, and never writes a Contribution, Field Account
occurrence, bank fact, or Accounting Release. Missing or ambiguous proof returns
one cause-labelled Phase 15 repair requirement; it does not guess.

**5. Resolver-backed donor search.** ONE shared endpoint the picker, CSV-paste matcher, and new-donor dedupe all call, routed through the Phase 3/10 subtract-only resolver (`resolveDonorMatch`): a minimal projection, restricted parties never appear and never change the response shape, tenant-scoped pg_trgm/FTS index, ≤300 ms e2e, read-your-writes for inline creates (a partial unique index `(tenant_id, normalized_email)` + insert-on-conflict-return-existing; email-less creates flagged dupe-suspect at validate, never auto-merged).

**6. Receipt-authorization / acknowledgment intent contract.** Both use durable domain intents, never an inline render, email, or provider call. The commit transaction emits source receipt-authorization facts only for Phase 7 plan-admitted individual receipts; `annual_cumulative_cash` emits no per-gift authorization, generation request, or delivery occurrence. The receipt chain progresses asynchronously through Phase 7 source authorization → Phase 18 exact generated document → Phase 17/6 governed communication, with tenant-fair claims, semantic idempotency, retries, and owner-specific dead-letter/recovery. The source intent carries `eligible_at` (release time) + `gated_on` (`settlement | none`): "immediately" is the default value with a short tenant-tunable recall delay. A settlement-gated intent cannot exist before ACH `succeeded` creates the contribution, and no worker may claim it before that source state. Acknowledgment intents remain Phase 14 meaning handed to Phase 17/6; the three streams classify as **transactional-relational** and bypass marketing opt-out while always respecting `do_not_contact`, bounce/complaint, and tribute `never`.
...
| `gift_disposition_facts`, `gift_disposition_coverage_facts`, and `noncash_disposition_source_projection` | Phase 15 sole canonical asset/disposition/proceeds/finality source; Phase 21 D21 may consume exact source-final coverage; Phase 20 remains separately capability-certified | Append-only source event and exact lot/purpose/tranche coverage from an existing noncash gift; read-only projection composes exact scope, source role, amounts, finality, evidence, and corrections | Exact source version + semantic idempotency + append-only correction lineage; raw rows never win as Contribution, Field Account, bank, or accounting truth | Re-ingest or append the exact source correction and remaining coverage; never delete, estimate proceeds, infer fees/finality, create a second gift, or post downstream truth directly          |
| `deposit_groups` / gift-grain current membership / `deposit_assignment_events`                           | Separate Phase 15 operational/source records; Phase 20 consumes exact frozen membership revisions                                                                          | Owner-qualified deposit command with expected-revision CAS, atomic current-head/event/receipt/audit and append-only move history                                                                    | Exact current assignment/group revision and immutable consumed Source Coverage                                                                             | Append a successor membership revision and any required cause-linked accounting correction; never update financial rows or rewrite consumed history                                            |
| `settlement_rail` (tender)                                                                               | Phase 13 vocab / Phase 15 stamp                                                                                                                                            | Set at entry; drives deposit-eligibility                                                                                                                                                            | Immutable per gift                                                                                                                                         | Source-owned correction evidence; never a posted-header re-stamp via the correction spine                                                                                                      |
...
| Acknowledgment purpose/readiness/release and joined outcomes                                             | Separate Phase 14 request/source records; Phase 17 preparation; Phase 6 delivery; Phase 15 derives authorized batch rollup                                                 | `sendBatchAcknowledgments` invokes exact Phase 14 release command; no direct state flip                                                                                                             | Exact consolidated source scope + owner revision + permanent semantic identity                                                                             | Phase 14 pre-source-release cancel; separately qualified Phase 6/17 definitely-unsubmitted custody cancellation never resets released coverage; same-identity recovery; P8 anti-stranding view |

### Build-order note (where the substrate lands)

The **first P15 build ticket lands the minimal Phase 13 posting substrate** (headers / designation_lines / postings + `effective_seq` + the BEFORE-UPDATE immutability trigger + the `credit_recheck` outbox) — it is FORWARD (Phase 13 epic #690, zero SQL today) and every money-final invariant above rides on it; draft-stage editing ships first (needs no ledger), and a flat-`donations` in-place-edit interim is FORBIDDEN. The D21 source-projection tables, same-scope constraints, amount-shape checks, non-overlap exclusion/locked insert path, and `SECURITY INVOKER` projection land with the noncash source lifecycle before any D21 consumer is enabled. The D6 deposit layer and the D7 template layer are post-commit / config layers that land AFTER the substrate + the D5 commit service; the settlement_rail discriminator (CB-A) must be resolved before the HD-5 deposit-eligibility predicate is written. All P15 tables ship their tenant-FK + RLS + uniqueness constraints in the first migration (fresh-build, no users).
...
The phase has five distinct test surfaces; each behavior is proven at exactly one primary seam, not smeared across all five.
...
5. **Noncash source seam — the real append-only projector.** Contract and
   Postgres integration tests append real source/coverage facts and read
   `projectNoncashDispositionSource`; they never fabricate a D21 payload around
   the projector. This is the oracle for source-mode/legal-recipient resolution,
   source-family finality, exact component shapes, lot/purpose/tranche coverage,
   idempotency/corrections, and the permanent absence of original valuation or
   direct downstream writes.
...
- **D6 deposit grid — V1–V6 × tenders × timing × cardinality.** Every founder-named workflow variant (V1 same-day RDC, V2 deposit-before-entry, V3 entry-then-deposit-days-later, V4 N batches → 1 deposit, V5 1:1, V6 inconsistent week-to-week) × {check, cash, bank-direct ACH/wire, Stripe-rail (must reject the link), securities/in-kind (no-deposit state)} × {deposit-before-entry / after / simultaneous} × {1:1, N:1, 1:N}. Asserts coherent derived deposit-state, append-only `deposit_assignment_events`, current-membership expected-revision CAS, exact Phase 20 consumed-revision immutability with audited successor/cause-linked correction, immutable printed-slip snapshots and NSF-retained membership. Cardinality describes batch-to-deposit grouping; each gift has at most one current deposit. Posted financial rows remain unchanged.
...
- **D21 noncash-source grid — mode × finality × disposition × component shape ×
  correction.** Cover `{tenant-held, proved tenant-agent, intermediary legal
donee, DAF sponsor, terminal nonmonetary}` × `{pending, partial, UI-settled but
unproved, exact source-final, ambiguous}` × `{single lot, partial lot, pooled
lots, installment, corporate action, canceled trade, retained/used/donated
onward/abandoned/worthless}` × `{gross-cost-net, exact-net-only,
nonmonetary}` × `{first event, replay, late cost, adverse correction,
out-of-order successor}`. Property tests prove exact quantity and minor-unit
  conservation, deterministic residuals, no overlapping positive coverage, and
  same Tenant/Legal Entity/purpose/currency isolation. Poison fixtures prove an
  appraisal/FMV/recognized value, staff-selected `settled`, inferred zero fee,
  fuzzy lot match, cross-scope row, raw disposition row, and duplicate economic
  root all create **zero** D21 candidates, Contributions, Field Account
  occurrences, bank facts, or Accounting Releases. Intermediary/DAF cash grants
  route through the ordinary cash/grant source exactly once; terminal outcomes
  produce no monetary support.
- **NF3 acknowledgment-gate grid.** Prove repeated/concurrent release of the same exact consolidated request/source scope converges once; the pre-release/cancel race has one winner; any permitted later Phase 6 custody cancellation leaves Phase 14 coverage intact; submitted/unknown outcomes never re-arm or create another identity. Preserve late-source-delta re-arm for only the new admitted occurrence, restricted per-viewer manifests/counts with no hidden-row disclosure, held behavior for non-cleared actors, and the Phase 7/18 receipt-versus-acknowledgment wall. Rollup tests join current owner revisions without copying outcomes or multiplying multi-source counts.
...
1. **The minimal Phase 13 posting substrate FIRST + retire the Track-B 501 bridge.** The Phase 13 ledger is groomed-not-built (zero SQL today; its epic #690 children are blocked), so P15's first non-spike ticket lands the minimal `contribution_headers` / `contribution_designation_lines` / `contribution_postings` shape it posts through: frozen legal-donor + designation snapshots, signed-delta postings with a monotonic `seq` under `FOR UPDATE`, the whole-row UPDATE/DELETE freeze keyed to the durable initial posting fact (including privileged writes and operational-reset bypass tests), the `effective_seq` cursor, separate Phase 13-owned payment/review operational records, and the one locked SECURITY DEFINER contribution-mutation function on the per-contribution advisory lock. Initial Phase 7 dating/receipt-plan admission facts use their required owner seam in the same qualified source commit; required local source effects, operation receipt/audit and `credit_recheck` outbox commit atomically. A harmless header lock may serialize append-only `seq`; no mutable counter/status/cache is written onto a posted financial row. **Delete the Track-B offline bridge** (the `OfflineEntryUnboundError` 501 path): salvage only the zod schema (`contributions-offline.ts`) and `resolveOfflineReceiptStatus` as precedent; the legacy-`donations` persistence plan is never wired (D1.b/D1.c). All P15 tables (`gift_entry_batches`, staged-row model, `batch_templates` + per-batch frozen snapshot struct, `deposit_groups`, separate gift-grain current membership, `deposit_assignment_events`) with every identity/scope/bound constraint, composite `(tenant_id, …)` FKs, and FORCE RLS in this first migration. _Blocked by:_ Phase 3 (Minimum Permission & Role-Scoped Projection Foundation) census + projection; Phase 12 (Full Role & Permission Configuration) capability registry. _Kill/rollback:_ inert — nothing commits money yet.
...
5a. **Noncash source lifecycle + the sole D21 projection.** Ship the one generic
noncash intake mapping, exact asset-lot/disposition event and coverage facts,
source-role/legal-recipient resolver, source-family finality adapters, amount-
shape checks, exact quantity/minor-unit conservation, append-only corrections,
one source-owned repair worklist, and
`noncash_disposition_source_projection`. The UI keeps `pending / partial /
settled` plain-language and secondary; the projector separately exposes proved
`source_final`. No Phase 21 consumer is enabled until the complete D21 test grid
passes in production-shaped fixtures. _Kill/rollback:_ disable D21 consumption;
source capture/compliance history remains intact and no downstream money was
written.

6. **Deposits.** The undeposited-funds operational layer (D6): first-class `deposit_groups`, separate Phase 15-owned gift-grain current membership and append-only assignment history, plus derived deposit-state; the one-click "deposit this batch" default; audited expected-revision add/remove/reassign with at most one current membership per gift. Banking/slip-print does not freeze the current grouping; Phase 20 pins the exact consumed membership/source revision, which stays immutable while a later change creates an audited successor and any required cause-linked accounting correction. Preserve the immutable slip snapshot, NSF-retained membership and settlement-rail rejection of Stripe-rail tenders, without updating posted financial rows. Both the workbench rail and Deposits area call one service. _Kill/rollback:_ grouping moves no money; disabling it loses the operational grouping/slip convenience without changing source facts or consumed accounting evidence.

7. **Templates + acknowledgments.** Preserve D7 frozen template/default policy, seeded starters, personal column preferences and the one System Default. NF3 submits batch-origin purpose/readiness to separate Phase 14 request/source records held for `batch_gate_pending`; the explicit per-batch action releases only the exact qualified ready scope into Phase 17 preparation and Phase 6 dispatch. Keep the pre-release manifest/cancel guard, owner-labelled joined results and quick-entry default under the existing source policy; no header acknowledgment columns, P15 sender queue, copied communication outcome or post-submission recall. _Kill/rollback:_ unreleased purposes remain held; templates never subtract a live control.
...
## Dated Phase 21 D5 source-purpose provenance amendment (2026-07-30)

**New producer obligation.** Every Phase 15 offline batch or Quick entry row
must capture owner-labelled source-purpose evidence sufficient for Phase 13's
closed Accepted Source Purpose Authority Snapshot. Depending on the actual
source, that evidence may be an exact Designation, remittance line, check or
wire memo, correspondence or acceptance-authority record, or the exact
governed publication kind/reference/digest when content was genuinely
presented or captured. When no publication applied or the historical source did
not capture one, Phase 15 sends typed `not_applicable` or `not_captured` plus
the exact evidence reference/digest. It never invents a Phase 17 message or
Phase 22 giving-page publication.

`commitGiftEntryBatch` supplies this evidence to the single Phase 13
accepted-purpose resolver. Phase 15 staff and templates cannot set the legal
restriction/preference class, assert purpose compatibility, or bypass that
resolver. Missing or ambiguous authority blocks the affected row with a clear
repair action while unrelated clean rows retain the existing atomic or
...
- **Accounting Release, provider delivery, Bank Match, GL, and final bank
  reconciliation** — Phase 15 owns deposit grouping, immutable slip source content/snapshots, and
  operational deposit-state; Phase 18 owns qualified generated artifacts/access. Phase 20 consumes exact revision coverage and
...
5. **`roadmap.md` line 1145 (deposit-reports ownership) — current owner synchronization.** Phase 15 owns grouping, immutable slip source snapshot/content and operational state; Phase 18 owns qualified generated-document artifacts/access; Phase 20 owns exact Accounting Effects/delivery and bounded Bank Match; QBO/Xero owns the final GL and bank reconciliation.
...
12. **`phase-14-donor-credit-operations.md` — NF3 owner synchronization.** Preserve batch/import auto-send suppression and `batch_gate_pending`; deliberate batch release uses the separate Phase 14 request/source relations and exact ready scope. Imports remain held and never pass this batch gate; existing import/tribute activation rules are unchanged. No acknowledgment field or outcome is added to a posted financial row; Phase 17 preparation and Phase 6 delivery stay independently authoritative. The earlier header-column recipe is superseded by C-02 / ADR-0206.
