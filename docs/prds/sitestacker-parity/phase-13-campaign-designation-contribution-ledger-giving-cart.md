...
Groomed via `grill-with-docs` (2026-07-09). All decisions **D1–D25** ratified and adversarially hardened (five ruthless 16/17-category reviews across D2/D3/D5/D7/D8/D12/D13/D14/D15/D16). PRD authored from the decision log. Program posture: **groomed-not-built** — this is a design against not-yet-built Phase 3/4/5/7/9/10/11/12 contracts; no live/shipped claims. The phase is tracked by live epic **#690** and children **#691–#713**; every child is currently `status:blocked`. The original issue bodies predate later owner amendments; the September 23 reconciliation updates the affected active clauses and records exact publication hashes. Read each current body and source qualification before dispatch, and preserve its unchanged implementation state. Issues **#706–#710** additionally remain subject to the binding Phase 16 recurring-domain supersession below. Repo anchors throughout are **evidence as of authoring**, never brittle build instructions.

**Slug:** `contribution-ledger` · **Roadmap position:** Phase 13 of 45 (roadmap v3, adopted 2026-09-22) · **Status:** PRD (design ratified 2026-07-09 via grill-with-docs — decisions D1–D25 + two governing principles R-JW and R-UX, each big money decision deep-researched against Stripe/IRS/NPSP/Blackbaud/CiviCRM/Baymard/FTC-ROSCA best practice and then pressure-tested by a 16/17-category ruthless adversarial review with a verify pass. Notable reversals — D1 named-destination → actually-direct charges; D2 compat-view → delete-and-replace; D8.c free-choice → guided-override-bounded-by-method; D14.2 first-touch → last-touch; D16.5 guarded → full self-serve — are the highest-signal parts of the record. READY).
...
- **Establish three distinct Phase 13 lifecycle dimensions** (payment / ledger-posting / review). Mutable payment and review state lives in separate Phase 13-owned operational records; posting, reversal and effective money derive from append-only postings. Posted headers, designation lines and postings remain wholly immutable. Receipt/document and accounting outcomes remain with their existing owners and join through exact source-authorized projections (D7; C-02 / ADR-0206; Phases 7/18/19/20).
...
191. As a **developer**, I want mutable payment and review state in separate Phase 13 operational records, posting/reversal derived from append-only financial facts, and receipt/accounting outcomes joined from their exact owners, so that operational progress never edits a posted financial row or claims another domain’s outcome. `[D7; C-02 / ADR-0206; Phase 7/18/19/20]`
192. As a **developer**, I want payment/review operational transitions and any associated append-only money effect admitted by the same owner-qualified, revision-checked command, with exact audit/receipt/outbox facts committed atomically and downstream owners advancing their own outcomes, so that a generic status setter cannot bypass source authority or mutate frozen money. `[D7 review; C-02 / ADR-0206; Phase 7/18/19/20]`
...
2. **Strictly append-only.** The qualified initial `draft → posted` commit is the one permitted freeze transition. After its durable posting/freeze fact exists, a **`BEFORE UPDATE OR DELETE` trigger that `RAISE`s** protects every column of the header, designation lines and postings, including columns added later — **not** RLS (`service_role` has `BYPASSRLS`) or `REVOKE` (migrations run as owner). The guard reads the durable financial posting/freeze fact, never a mutable operational status: resetting operational state to `unposted` cannot make a financial row editable. No privileged writer, status-field allowlist or sequence-counter exception exists (C-02 / ADR-0206).
...
5. **The fold orders by a monotonic per-header `seq`, not `created_at`** (`NOW()` ties within one transaction are non-deterministic). The fold and revision token share that append-only posting cursor. `SELECT … FOR UPDATE` may lock the immutable header to serialize sequence allocation without changing it; the qualified locking/uniqueness protocol allocates the next posting sequence without a mutable counter, cache or other UPDATE on the posted header. Any necessary mutable allocation metadata belongs in separate Phase 13 operational storage. Preserve exact same-Tenant/header sequence uniqueness and the `(tenant_id, header_id, seq)` access path (C-02 / ADR-0206).
...
<a id="g-contribution-lifecycle--three-header-axes-plus-related-receipt-and-accounting-truth-d7"></a>

### G. Contribution lifecycle — separate operational state and append-only posting truth (D7)

**Core ruling: a contribution has no single `status`.** The three logical
Phase 13 dimensions remain payment, ledger/posting and review. Under the
[accepted C-02 ruling / ADR-0206](../../adr/0206-immutable-posted-financial-facts-and-owner-operational-state.md),
mutable payment and review state is physically separate from the financial
header, designation lines and postings. Posting, reversal and effective money
remain derived from append-only postings and exact source evidence. No mutable
operational status can create money or undo the durable financial freeze.

Payment/review operational records use closed CHECK-constrained vocabularies,
exact same-Tenant source identities and owner-qualified, expected-revision
commands. Phase 7 receipt facts; Phase 18 current artifacts; Phase 20 Source
Coverage, Accounting Releases, delivery/readback/drift, settlement and Bank
Match; and provider final books remain independently authoritative. Joined
reads retain each owner’s exact version/cursor and current source access. A
missing, stale or unavailable owner result stays unknown/blocked, never a value
reconstructed from an old header status or silently defaulted to success.

> **Real-vs-forward (as of authoring).** The parallel columns and TypeScript
> transition matrix in `staged_gifts` are historical evidence for distinct
> lifecycle concerns. They do not authorize moving mutable statuses onto posted
> financial rows. The current target uses separate Phase 13 payment/review
> records and the append-only posting fold; downstream domains keep their own
> records and transitions.

The three Phase-13-owned logical dimensions:

1. **PAYMENT** — qualified current rail state in separate Phase 13 operational records, with exact provider/offline source observations retained. States: `requires_action / processing / succeeded / failed / canceled / refunded / disputed / returned`, plus ACH `provisionally_settled`, plus the dispute and refund sub-lifecycles. **Processor-backed payment facts require exact signed provider confirmation; offline received facts require the qualified Phase 13/15 source-capture command.** Neither route permits arbitrary staff status edits, a client-derived assertion, or an unqualified provider hint to create money truth. Per-rail finality is explicit: **card `succeeded` = final; ACH `succeeded` = PROVISIONAL** (an unauthorized-consumer return can arrive up to ~60 calendar days later as `charge.dispute.created`). Terminal vs in-flight is an explicit property of each state, not a convention.
...
**Right-sized, not over-built.** Separate payment/review operational state and
the append-only posting fold preserve the same accepted lifecycle behavior.
Phase 7 receipt facts, Phase 18 artifacts and Phase 20 accounting records join
through exact source identities and coherent owner cursors. This is a physical
ownership correction, not a new lifecycle, generic workflow engine or approval
policy. A free-text financial-row status and receipt/export header stubs remain
rejected; C-01’s correction-approval question remains separate.
...
- **Reversals flow the same append-only direction**: a refund or ACH return
  ...
  original financial rows, source observations and posting remain intact;
  the separate current payment projection advances only through its owner command.
- Each operational transition is enforced inside its **locked, owner-qualified DB command** (the locked-function pattern from `[[fractional-index-collation-trap]]`), with exact expected revision, source proof and permanent semantic operation identity. No generic status setter bypasses a precondition. Unknown/illegal/stale transitions fail closed and retain durable diagnostic evidence through the qualified rejection boundary—never silent success or an escalation row lost with a rolled-back transaction.

> **Real-vs-forward (as of authoring).** The old TypeScript-only status writers are removal evidence. The target’s `SECURITY DEFINER` commands and database constraints validate transitions on separate Phase 13 payment/review records, using CHECK-constrained TEXT rather than native enums. A local accepted command commits required operational changes, append-only financial effects, command receipt, audit and outbox facts atomically under the qualified per-contribution lock/revision protocol. It never UPDATEs a posted financial row. Provider I/O stays outside the transaction and reconciles under the same operation identity. Phase 13 does not directly author another domain’s outcomes or promise a distributed transaction; explicit existing local contracts, including atomic initial Phase 7 dating facts, retain their qualified owner write boundary.
...
Imported and adopted gifts use the same separate Phase 13 payment/review
records and append-only posting fold,
...
- **Recurring recompute (Phase 16 D2/D4 owner amendment).** Preserve the accepted `cover_fees` intent and **recompute per installment through Phase 16's exact cohort, occurrence, execution-leg, and line-item mapping** — never freeze a stale dollar figure or treat a provider invoice as independent business authority. Persist the applied rate, mode, method class, base, cover, and charged total with the exact source-confirmed occurrence; a later rate increase remains grandfathered or explicitly re-consented. The checkout fee-cover line attaches to the **one-time PI header ONLY** and never bleeds into a recurring initial occurrence. Each recurring occurrence's fee-cover lands once on its source-owned header; preserve the per-line allocation so a single-line partial refund returns `line_amount + line_fee_cover_share` cleanly.
...
**Accepted physical-state ruling (C-02, 2026-09-23).** Conrad selected
**separate operational records; keep posted rows immutable**.
[ADR-0206](../../adr/0206-immutable-posted-financial-facts-and-owner-operational-state.md)
is binding: header, designation-line and posting rows are wholly frozen after
the durable initial posting/freeze fact, including future columns and privileged
writes. Mutable payment/review state and reconciliation/allocation metadata live
in separate Phase 13-owned operational records, with exact source relations and
coherent joined projections. No status, header counter, cache or future-column
exception may reopen or rewrite a posted financial row. C-01 remains a separate
pending correction-approval decision; this ruling creates no new approval rule.

- **`contribution_headers`** — one contribution (one hard-tender payment). Purpose: the gift's canonical identity and declared total. Key columns: `id` (**reuses the `donations.id` UUID**), `tenant_id`, immutable `legal_issuer_id` (server-resolved from the verified legal-issuer authority, never caller input), immutable `gift_method`, `total_minor BIGINT`, currency (ISO-4217), the **frozen legal-donor snapshot** (`donor_id` frozen at gift time per Phase 4; explicit `is_anonymous`/NULL-donor flag; survives merge re-point), the immutable initial-post/freeze provenance, with mutable payment/review state in separate Phase 13 operational records, posting/reversal derived from append-only postings, and receipt/document/accounting outcomes in their owners’ related records, `entry_method`, and Stripe link columns (`stripe_payment_intent_id`, `stripe_charge_id`, `stripe_account_id`). Invariants: `UNIQUE (tenant_id, id, legal_issuer_id)` supports exact-issuer dependent facts; header total = hard tender only; `total_minor = SUM(lines)` at COMMIT; every header column is immutable once posted, including issuer, tender, source links and fields added later; the freeze does not depend on mutable operational state. The locked commit seam creates the required initial Phase 7 `contribution_dating_facts` revision in the same transaction; the header carries no competing dating values.
...
- **`contribution_postings`** — append-only entries that fold to the effective money value. Purpose: the money ledger's event stream (initial allocation + every correction/refund/reversal). Key columns: `id`, `tenant_id`, `header_id`, `target_line_id`, `seq` (monotonic per-header), `amount_delta_minor` (signed), `kind` (TEXT+CHECK: `initial`, `refund`, `void`, `write_off`, `ach_return`, `chargeback`, `source_code_correction`, …), `reverses_posting_id` (nullable), `is_initial` replay-guard, actor/reason/provider-outcome facts, plus the **immutable D14 attribution columns copied as plain columns** (never re-read from the live registry). Invariants: **no UPDATE/DELETE once written** (BEFORE UPDATE OR DELETE trigger that RAISEs — not RLS, not REVOKE, because `service_role` has BYPASSRLS and migrations run as owner); `seq` allocated under the qualified lock/uniqueness protocol, which may `SELECT … FOR UPDATE` the immutable header without UPDATEing any header counter or column; a reversal carries inverse deltas and both rows survive as provenance (never a status flip). Date corrections carry no money delta and therefore never enter this stream; they append through Phase 7's `contribution_dating_facts` authority.
...
A contribution has **no single status**. Mutable payment and review state
lives in separate Phase 13 operational records rather than columns on
`contribution_headers`, `contribution_designation_lines` or
`contribution_postings`. Physical SQL names follow repository conventions; the
separate storage and owner boundaries are mandatory, not just a UI/view split.
Each relation preserves exact same-Tenant contribution/source identities,
qualified provider/account/mode or offline evidence where applicable, current
operational revision and last-applied source cursor, permanent command identity
and audit/receipt provenance. A header reference uses the composite same-Tenant
FK. Pre-success provider evidence remains in its admitted source lane and cannot
fabricate a contribution merely to satisfy that relationship.

Only owner-qualified, expected-revision commands may update those records. Their
closed CHECK-TEXT transitions cannot write source money; a required financial
effect appends through the one locked posting service. Co-moving local state,
append-only effect, command receipt, audit and outbox facts commit atomically;
provider calls and downstream owner execution remain outside that transaction.
Posting/reversal is derived from the immutable source posting stream, not a
mutable `posting_status`. The durable initial posting/freeze fact permanently
protects every financial column even if an operational projection is corrupt or
reset to `unposted`. Read models join exact owner versions/cursors and fail
closed on incoherent, missing or stale evidence instead of reading old header
status columns. Receipt/document and Phase 20 accounting coverage remain
separately owned related projections:

1. **PAYMENT** (separate Phase 13 operational record, advanced only by exact signed provider confirmation or the qualified Phase 13/15 offline source command): `requires_action / processing / succeeded / failed / canceled / refunded / disputed / returned`, plus ACH `provisionally_settled`. Card `succeeded` = final; **ACH `succeeded` = PROVISIONAL** (returns arrive up to ~60 days later).
...
| ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Contribution header / designation lines                    | Asym Postgres (`contribution_headers`, `contribution_designation_lines`), wholly immutable once posted                | Qualified initial source commit; later money changes append through the posting owner                         | Exact committed source facts and immutable initial-post/freeze evidence                                 | Append-only compensating postings; never header/line UPDATE or DELETE                                                                                                                    |
| Payment / finance review operational state                 | Separate Phase 13-owned records with exact same-Tenant contribution/source relations                                  | Owner-qualified, expected-revision commands; signed processor or admitted offline evidence as applicable      | Exact source/operational revision and permanent semantic command identity                               | Reconcile source evidence and rebuild coherent joined projections; never rewrite financial rows or infer success from missing state                                                      |
...
2. **No UPDATE/DELETE on a posted row.** The qualified one-time `draft → posted` commit establishes durable initial posting/freeze evidence. Thereafter the database trigger rejects every UPDATE/DELETE of the header, designation lines and postings—including future columns and privileged writes. The guard never depends on mutable payment/review state: changing a separate operational record to `unposted` cannot unlock the financial row. No status/counter/cache allowlist exists. Trigger protection remains required because RLS and REVOKE cannot provide this owner/BYPASSRLS floor (C-02 / ADR-0206).
3. **Monotonic per-header `seq`.** The append-only fold/revision token orders by `seq`, never `created_at`. Serialize allocation under the qualified contribution lock and uniqueness protocol; locking the immutable header with `SELECT … FOR UPDATE` is allowed, changing it is not. No mutable sequence counter or cache resides on any posted financial row. Any mutable allocation metadata is separately owned operational storage; concurrent append/replay preserves exact same-Tenant/header sequence uniqueness and `(tenant_id, header_id, seq)` access.
...
2. **Posted-row immutability (C-02 / ADR-0206).** After the qualified initial posting/freeze commit, every UPDATE/DELETE of a header, designation line or posting fails, including newly added columns and owner/service-role writes. Authorized payment/review transitions change only their separate operational records and leave every financial column byte-for-byte unchanged. Poison fixtures attempt a raw/privileged mutation, reset operational state to `unposted` before mutation, and introduce a financial-row status/counter exception; none bypasses the durable freeze. Concurrent append/return/replay and coherent-read tests prove unique posting sequence, same source/operational cursor, atomic receipt/audit/outbox effects and no mixed-Tenant or mixed-version success. A header row lock may serialize allocation without changing any financial byte.
...
5. The **D3 ledger core**: header + designation lines + append-only postings; monotonic append-only `seq` via the qualified lock/uniqueness protocol (a header row lock, never a header counter UPDATE); the immutability trigger; the deferrable `sum(lines)=header` constraint; the derivation-only effective-value fold + CI grep gate on base-money reads; the version-cursored read model; **delete `funds.current_amount`** (derive + reconcile); the shared largest-remainder (Hamilton) proration function used by both the UI preview and the ledger.
6. The **D7 separated lifecycle**: physically separate Phase 13 payment/review operational records with CHECK-TEXT vocabularies, exact source identities and expected-revision commands; posting/reversal stays an append-only-derived dimension. Preserve the whole-row financial freeze, atomic local operational/effect/receipt/audit/outbox commits, durable rejection evidence and coherent joined reads. Route qualified processor/offline sources through their owner commands; no generic status setter, mutable posted-header counter or downstream receipt/accounting stub. C-01’s approval policy is not selected by this storage ruling.
...
- **Roadmap position:** Phase 13 of 45 (roadmap v3, adopted 2026-09-22). Depends on Phases 1, 2, 3, 4, 5, 7 (roadmap deps) and _consumes_ the binding constraints of Phases 9, 10, 11, 12 (CRM party spine, restricted-worker safety, custom-fields posture, the capability PDP). Consumers: Phases 14 (soft-credit/DAF ops), 15 (offline batch), 16 (recurring commitments, fixed-total pledges, fulfillment, recovery, and support projection), 17 (communications), 34/35 (qualified workflow coordination and giving recipes), 20 (accounting export), 22 (public campaign pages), 25 (portal depth), 27 (appeals), 33 (reporting), 36 (P2P).
