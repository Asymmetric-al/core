...
**Pending physical-state decision (C-02, 2026-09-23).** The recorded header status
columns below and Phase 14's acknowledgment columns must change after posting,
while D3/E2 and the invariant/test register freeze the whole posted header. The
[owner decision record](../../ai/audits/2026-09-23-contract-congruence.md#c-02-posted-state)
asks whether to use separate operational records (recommended) or a narrowly
specified update exception. Until the founder rules, these physical placements
are not build-ready DDL. Keep accepted logical lifecycle/source authority,
append-only money and existing immutability intact; do not infer a trigger bypass
from the status-column shorthand or silently choose a replacement schema.
...
1. **PAYMENT** (exact signed provider confirmation for processor-backed facts, or the qualified Phase 13/15 source-capture command for offline received facts; never an arbitrary status edit): `requires_action / processing / succeeded / failed / canceled / refunded / disputed / returned`, plus ACH `provisionally_settled`. Card `succeeded` = final; **ACH `succeeded` = PROVISIONAL** (returns arrive up to ~60 days later).
