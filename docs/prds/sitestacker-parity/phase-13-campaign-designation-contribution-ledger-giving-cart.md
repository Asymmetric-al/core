...
194. As a **developer**, I want ACH treated as provisional until settled and checks treated as provisional until cleared while posting at recorded, with check individual receipts following the ordinary Phase 7 timing rail by default and hold-until-cleared only as a tenant opt-in, so that provisional money stays reversible without blocking CB-B check receipting. `[D7 review, D8]` _(Amended 2026-07-11 by Phase 15 D5 and 2026-07-24 by Phase 19 D4: the ACH clause is unchanged. For checks, any individual receipt admitted by Phase 7 follows the ordinary timing rail, with an NSF compensating reversal + void/corrected receipt if it bounces. For an active Canadian pack the frozen prospective plan controls, and `annual_cumulative_cash` creates no per-gift receipt; otherwise the applicable ordinary Phase 7 policy governs. "Gate the individual receipt on money-final" is the opt-in hold-until-cleared case.)_
...
> tender-specific §E.1 posting predicate (processor-confirmed online success/adopted gifts, or recorded CB-B offline tender) → **eligible to post** → `posted` → Phase 7 **plan/policy receipt eligibility** (checks may admit at recorded by default; ACH waits for confirmed success) → `posted` + settled/cleared where required → **exportable**.
...
- Tenant `tax_timezone` (IANA, default `America/New_York`) + optional prior-year policy live in `org_settings`.
- **CAPTURE-NOT-RECOMPUTE**: the date is resolved **once at capture** and stored, **never recomputed on read.**
- Year-boundary corrections and backdating are an **append-only, staff-attested `gift_date_correction` adjustment** by default, with stronger approval/cutoff gates only when tenant or jurisdiction policy requires them — **never a silent `UPDATE`.**
...
- **Dating (confirms Phase 7 verbatim; D8 adds only the tender→basis map):** `gift_date` = resolved date-of-delivery; `delivery_basis ∈ {postmark, received, settlement}`; a pure resolver `{method, postmark/received/settlement ts, tenant tz} → {gift_date, delivery_basis, tax_year}`; tenant `tax_timezone` (IANA, default `America/New_York`) + optional prior-year policy in `org_settings`. **Capture-not-recompute** (resolved once at capture). D8.c is a **guided override bounded by method** (smart per-tender default pre-selected, override only within the method's legally defensible set, a live tax-year preview via the same resolver, and a cross-issued-year override inheriting Phase 7 A15's staff-attestation default with optional tenant/jurisdiction strengthening). **Fail-CLOSED tenant tax config** — a missing `org_settings.tax_timezone` is resolved at an onboarding gate, never silently defaulted to US.
