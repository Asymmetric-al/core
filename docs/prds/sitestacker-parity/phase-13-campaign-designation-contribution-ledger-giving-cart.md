...
194. As a **developer**, I want ACH treated as provisional until settled and check as provisional until cleared, posting immediately with a settlement flag but gating receipts on money-final, so that we never issue a tax receipt for money that might bounce. `[D7 review, D8]` _(Amended 2026-07-11 by Phase 15 D5 and 2026-07-24 by Phase 19 D4: the **check** clause here is superseded — a check posts immediately at recorded; any individual receipt admitted by Phase 7 follows the ordinary timing rail, with an NSF compensating reversal + void/corrected receipt if it bounces. For an active Canadian pack the frozen prospective plan controls, and `annual_cumulative_cash` creates no per-gift receipt; otherwise the applicable ordinary Phase 7 policy governs. "Gate the individual receipt on money-final" is the opt-in hold-until-cleared case; the ACH clause is unchanged.)_
...
Receipt admission is orthogonal to tender. For an active exact-issuer Canadian
pack, Phase 7's frozen prospective plan controls; when that pack is not active,
the plan is structurally absent and the applicable ordinary Phase 7 policy
governs. “Plan/policy-admitted” below always means that two-lane rule.
...
| **church_remittance** | remitting-church party id (payer) · `check_number` / `postmark_date` · per-line soft-credit attributions `[{party_id, soft_credit_amount}]` | **postmark**                                                                      | offline; post at recorded; **provisional** until cleared; receipt follows exact legal donor and governing Phase 7 plan/policy                        |
