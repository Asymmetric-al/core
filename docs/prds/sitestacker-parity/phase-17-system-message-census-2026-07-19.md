...
- **Runtime baseline revision:** `b14a8a369dfc495d293a1564a16b970329075fd4`
...
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `RUN-001` | `packages/email/resend.ts:112` `createResendClientInstance`; `:815` `validateResendApiKey`; `:944` `sendEmail`; `:1091` `verifyResendWebhookSignature` (`:1114` direct `new Resend`); `:1143` `getReceivedEmail`; `:1166` `listReceivedEmailAttachments`; `:1197` `createResendClient` | Located Resend-only adapter construction, validation, send, webhook-verification, and inbound-read primitives | `not_a_message`           | Retain the adapter behind Phase 6/17 preparation, connection, transport, and verified webhook/inbound operations. Converge both direct constructors through the adapter-owned factory; it owns no product meaning, content resolution, sender choice, credential custody, or tenant authorization. Architecture CI rejects provider SDK construction/send/receive outside this boundary. |
| `RUN-002` | `packages/api/src/giving/receipts.ts:310` `deliverReceiptEmailForGift`; `:429` `sendStagedGiftReceipt`; `:468` `sendUpdatedReceiptSnapshotEmail`                                                                                                                                       | Successful and replacement/updated receipt content, send, consent skip, and send-log persistence              | `target_live_candidate`   | Migrate one source occurrence at a time to `giving_receipt_issued_v1` and `giving_receipt_replaced_v1`; Phase 7 source facts/issuance effects and Phase 18 exact artifact/currentness remain authoritative, and old/new writers never both send one occurrence.                                                                                                                          |
...
| `KEY-R05` | `recurring_payment_truth_corrected_v1`  | Recurring payment truth corrected                     | Provider-confirmed correction, Phase 7 source effects, and any Phase 18 artifact consequences |
...
| `OBL-010` | Receipt replacement, correction, void, supersession                  | Phase 7 source facts/authorization + Phase 18 artifact/currentness | Replacement/correction candidates; other meanings deferred                                                               | Distinct mutually exclusive source and exact-artifact occurrences; no history mutation                                                                                                                                                                          | Eligible key only                      |
