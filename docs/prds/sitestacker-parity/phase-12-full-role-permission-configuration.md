...
**Slug:** `permission-config` · **Roadmap position:** Phase 12 of 45 (roadmap v3, adopted 2026-09-22) ·
...
**Historical repository baseline (2026-07-08).** The following observations
...
- **Donor subject and purpose admission (IC02).** A personal, represented,
  commitment-only or document-only source grant constrains resource/subject/
  purpose inside an existing validated Tenant Authorization Context; it is not
  a fifth context or whole-Party/list permission. No donor row is manufactured
  to satisfy a snapshot. Neutral personal-first navigation, represented view
  switching and exact targeted return never change principal, Tenant, source
  grant or financial authorization. Missing membership resolution cannot fall
  back to a donor/profile role, superadmin, shared email or demo Tenant. Apply
  current subject/field/purpose admission independently to names, counts,
  options, continuation and provider references; revoke and fence late results
  rather than silently retargeting accepted work. Personal contact/security
  remains attached to the human, not the represented legal donor.
- **Shared identity-transition fence (IC08).** Before an externally completing
  shared credential change can outrun current identity/epoch reconciliation,
  the existing PDP honors its exact principal/request/revision fence at every
  protected door and writer across affected existing Tenant assignments.
  Preserve ordinary grants only where current rules permit; suspend/re-attest
  sensitive and restricted named grants without exposing other Tenants or
  copying their contact data. Shared Auth retains the exact verified finishing
  session, explicitly retires the principal's other existing sessions, and
  proves native completion/readback and delayed-call ordering. Unknown cleanup
  blocks protected use on the obsolete/unresolved basis but never claims the
  provider cannot mint credentials. After completion, stale workers cannot
  sign out later authorized sessions. Contact-only changes use neither this
  global fence nor session cleanup. No second PDP or general session product
  is introduced, and an incomplete attempt grants nothing and cannot
  permanently revoke otherwise valid ordinary access.
...
The Phase 9 personal-contact command uses independent read/edit admission for
its exact human, Tenant, field and purpose. Unsplit ordinary Name and optional
Phone are not permission to change Auth, structured/legal identity, a represented
Party, billing, released public identity or issued documents. Validate every
selected field before the owner transaction; a partially permitted request
writes nothing rather than updating a profile before rejecting another side
effect. Current contact verification and accepted historical claim evidence are
separate; neither a contact Clear nor a new matching email grants/revokes a
claim by itself. Direct REST, GraphQL/RPC, collection and database paths must
obey the same current gate (Phase 25 IC06/IC10/F04).
...
- **Crypto (D25 + R1/R3).** `decrypt.<field>` is a **distinct capability** (separate grant from read), reached only through a **resolver-gated RPC in a separate least-privilege DB role** (revoke direct `SELECT` on ciphertext and key), rate-limited + durably read-audited-before-return; **no blind index** on restricted-identity fields; no dashboard/aggregate/embedding path touches the key. On `pgcrypto` (the recorded database substrate; [PostgreSQL 17 still documents the extension](https://www.postgresql.org/docs/17/pgcrypto.html), so qualify the exact deployment/OpenSSL and key-provider boundary rather than assuming a PG17 deprecation), encryption-at-rest is DB-file-theft defense only, **not** a second gate against an app path holding the key — stated defensively in the product narrative. The `decrypt` RPC returns through a **key-provider _envelope_ interface** (wrapped-DEK in/out, key-version tag, re-wrap) with pgcrypto underneath, so v1 stores `(version, ciphertext)` and BYOK/HYOK adopts without a data migration. **`R1`:** restricted-tier DEKs are by-design unrecoverable, no escrow; break-glass (D17) and the immutable change-history hold identifiers-and-ciphertext only. **`R3` (target, never a v1 property):** BYOK/HYOK — platform-cannot-decrypt the restricted tier — so "even we cannot read your endangered workers, even if compelled" becomes true; bought at the price of "even we cannot recover them."
...
Enterprise SSO/SAML/OIDC remains an **inert principal source**; the bounded
SCIM `active=false` event bumps the grants epoch and force-kills the affected
sessions through the existing revocation owner. This enterprise reservation does
not defer Phase 25's selected Google/Apple/Facebook donor entry. Those methods
require shared Auth's unresolved G01 supported native-admission proof and each
provider's readiness before exposure; the PDP, coarse RLS, verified-email flag
or post-link application denial cannot substitute for native credential safety.

Invitation acceptance proves only the intended same-Tenant principal binding
under Phase 4's exact purpose/request/recipient proof. A matching session email,
relationship or delivered invitation alone grants no access; each grant still
uses its current owning command and this PDP. Sign-in/contact email changes
retain separately accepted claim proof and exact contact revision. Apply the
identity-transition fence and finishing-session rule in §B where global
credentials change; sensitive grants are re-evaluated/suspended independently,
and contact-only updates never trigger that global session operation.
...
**Phase 25 owner-adoption acceptance:** prove personal, representative-only,
nonfinancial and document/commitment-only contexts through the existing four
context variants without fake donor rows or profile/demo fallback. Exercise
current revocation and late-response fencing across page, API, direct data/RPC,
Storage and realtime boundaries. Name/Phone tests require all-field admission,
atomic rejected writes and no identity/billing/public effects. Shared credential
transition tests cover all affected assignments, exact finishing-session
retention, other-session cleanup/readback, unknown and delayed native calls,
no post-completion stale cleanup and contact-only isolation. Native G01 remains
a separate direct-endpoint qualification; green PDP/RLS tests cannot pass it.
...
- **Roadmap position:** Phase 12 of 45 (roadmap v3, adopted 2026-09-22). Hard-blocked on Phases 3, 9, 10, 11. Consumers: Phases 13/21/30/31/33/34/6/38/40.
