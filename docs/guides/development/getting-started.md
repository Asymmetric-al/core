...
| `E2E_AUTH_BYPASS`                     | Optional (test-only)                           | Enables deterministic E2E auth cookie mode outside production | `false`                                                   | Playwright demo auth preflight can fail without seeded demo users. |
...
| E2E auth preflight fails   | Set `E2E_AUTH_BYPASS=true` for local Playwright runs when demo fixtures are unavailable.                                  |
