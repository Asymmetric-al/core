...
| `TWENTY_API_URL` / `TWENTY_API_KEY` / `TWENTY_WEBHOOK_SECRET` / `TWENTY_WORKSPACE_ID`                                    | Optional; required for CRM sync                | Twenty CRM gateway and webhook configuration | empty                                                     | Twenty-backed CRM sync surfaces remain disabled.           |
