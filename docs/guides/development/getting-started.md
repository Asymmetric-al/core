...
| `TWENTY_API_URL` / `TWENTY_API_KEY` / `TWENTY_WEBHOOK_SECRET` / `TWENTY_WORKSPACE_ID`                                    | Optional; dormant (Twenty retired — ADR-0001)  | Twenty CRM gateway and webhook configuration (retired per ADR-0001; pending code removal) | `TWENTY_API_URL=https://api.twenty.com/rest`; others empty | Twenty-backed CRM sync surfaces remain disabled.                       |
| `TWENTY_RATE_LIMIT_RPM`                                                                                                  | Optional; dormant (Twenty retired — ADR-0001)  | Twenty API rate limit guard (retired per ADR-0001; pending code removal)                  | `100`                                                      | Callers use default rate limit.                                        |
| `CRM_SYNC_INBOUND_ENABLED` / `CRM_SYNC_OUTBOUND_ENABLED` / `CRM_SYNC_REPLAY_ENABLED` / `CRM_SYNC_RECONCILIATION_ENABLED` | Optional; dormant (Twenty retired — ADR-0001)  | CRM sync toggles (retired per ADR-0001; pending code removal)                             | `false`                                                    | CRM sync workers stay disabled.                                        |
| `CRM_SYNC_WEBHOOK_TOLERANCE_SECONDS`                                                                                     | Optional; dormant (Twenty retired — ADR-0001)  | Twenty webhook timestamp tolerance (retired per ADR-0001; pending code removal)           | `300`                                                      | Defaults to 300 seconds.                                               |
...
Twenty CRM was retired on 2026-07-06
([ADR-0001](../../adr/0001-asym-postgres-owns-crm-truth-twenty-retired.md));
the walkthrough below is dormant reference retained until the cleanup ticket
removes the code. Twenty CRM Phase 4 uses the Cloud REST base
