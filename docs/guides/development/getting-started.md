...
Runtime auth clients also accept `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` and prefer it when present, but the current setup and verification helpers still require `NEXT_PUBLIC_SUPABASE_ANON_KEY`.
...
- **Required env vars for repo onboarding today**: `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY`.
- **Optional runtime override**: `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` is supported by the shared auth/database clients and takes precedence when set, but it does not replace the anon key for `bun run setup` / `bun run verify` yet.
...
| `NEXT_PUBLIC_SUPABASE_ANON_KEY`        | Required for local setup + verification        | Legacy-compatible public Supabase key used by onboarding helpers | `your-anon-key`                                           | `bun run setup` / `bun run verify` fail; client auth may fail if no other public key is set. |
| `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` | Optional (preferred by runtime when available) | Modern Supabase publishable key                                  | `sb_publishable_...`                                      | Runtime falls back to anon key if present; otherwise onboarding helpers still fail today.    |
...
| Issue                      | Quick fix                                                                                                                                                                                             |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Supabase envs missing      | Set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` in `.env.local`. You can also add `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, but the current setup helpers still expect the anon key. |
