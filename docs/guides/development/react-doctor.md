...
- `insecure-crypto-risk` in `packages/lib/cloudinary-server.ts`: the helper SHA-256s Cloudinary's official sorted string-to-sign (not a credential hash). The live email uploader in `packages/api/src/email/assets.ts` now uses this helper. Cloudinary signed uploads use SHA-256 instead of SHA-1.
