...
that `apps/missionary/vercel.json` disabled `production`, so Vercel could not create
the intended Production deployment from the current release branch.

The latest visible `missionary` production deployment is stale and failed against an older `production` commit because the configured root directory did not exist in that historical checkout. In the current `origin/main` tree, `apps/missionary` does exist, so the latest failure should not be treated as proof that the current source still has a missing-directory problem.
...
- `apps/missionary/vercel.json` no longer disables the live Vercel Production Branch, `production`.
...
git ls-tree -d origin/production apps/missionary
