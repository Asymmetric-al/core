...
`8c2c6fda1372155a2b89f76dfb333a38c9ae6f6d`, but the intended Vercel Production
...
| Verify remote checks, not only local checks                              | GitHub CI, Nia source check, and the targeted `Sync Vercel Production Env` dry-run/write workflows succeeded for commit `8c2c6fda1372155a2b89f76dfb333a38c9ae6f6d`. Re-check the newest run after every new push before release.                                                                          | Complete |
...
| Configure all required Vercel Production env values                      | Current verifier reports missing Stripe, Sentry, `RESEND_WEBHOOK_SECRET`, and `RESEND_ENCRYPTION_KEY` values for every app. Supabase/Payload and `RESEND_API_KEY` sensitive values are present but unreadable by Vercel CLI.                                                                              | Blocked  |
...
| Prove Vercel now attempts Production builds from the release branch      | After the branch-gating fix reached `epic`, Vercel created Production builds for all three projects from commit `8c2c6fda1372155a2b89f76dfb333a38c9ae6f6d`. Those builds failed on missing provider envs rather than branch filtering or missing root directories.                                        | Complete |
| Ship successful intended Vercel Production deployments                   | Current verifier reports no READY Production deployment for the target commit for `admin`, `donor`, or `missionary`. The latest attempts are `admin-3i4y4sjhl-asymmetric-al.vercel.app`, `donor-5ntikiapz-asymmetric-al.vercel.app`, and `missionary-7zu4pru5r-asymmetric-al.vercel.app`, all `ERROR`.    | Blocked  |
...
- Shell environment: the remaining seven provider values are absent.
...
- Vercel Production env scope: `RESEND_API_KEY` is now present for all three
  projects as a sensitive value, so the CLI can confirm the name exists but
  cannot read or print the value.
...
4. Run the `Sync Vercel Production Env` workflow first as a dry-run and then as
   a write. `RESEND_API_KEY` has already been backfilled into Vercel
   Production, but that does not make Production deployable until the remaining
   provider values exist. `RESEND_WEBHOOK_SECRET` must come from the Resend
   webhook configuration, and `RESEND_ENCRYPTION_KEY` must be a managed
