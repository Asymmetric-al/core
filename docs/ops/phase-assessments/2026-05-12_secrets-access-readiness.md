...
- Sentry DSNs are present, but `SENTRY_AUTH_TOKEN` is not verified and sourcemap upload readiness is therefore not verified.
...
- Sentry sourcemap upload is not verified because `SENTRY_AUTH_TOKEN` was not found in the checked secret/env inventories.
...
| 3     | Payments / giving pipeline                | Supabase + Stripe test keys, repo checks, data boundary, Vercel app env names | Stripe webhook proof, Resend receipt proof, Twenty gift-posting proof, Sentry proof, deployment proof | ready for local/dev only | Twenty workspace ID; Stripe dashboard verification; Resend dashboard verification; migration verifier; E2E smoke | Stripe dashboard, Resend dashboard, Twenty settings, Supabase local DB, Vercel | Can start local/dev implementation planning. Cannot claim deployment completion yet.                                      |
...
| Sentry                           | `SENTRY_DSN`, `NEXT_PUBLIC_SENTRY_DSN`, `SENTRY_AUTH_TOKEN` if sourcemaps required | DSNs present in GitHub/Vercel; auth token not verified                             | DSNs production-scoped for all three apps                                                                 | partially verified                                                         | 3 deployment observability, 11                                        | Sentry org/project settings; Vercel/GitHub secrets                  | Runtime DSN readiness is present; sourcemap upload readiness is not verified.                                                   |
...
- Production deployment can proceed for current repo state. Phase 3 production completion cannot be claimed until Stripe/Resend/Twenty/Sentry provider checks are complete.
...
- Sentry sourcemap upload is not verified because `SENTRY_AUTH_TOKEN` was not found.
...
- `SENTRY_AUTH_TOKEN` was not found in the checked GitHub/Vercel/local inventories.
- Sentry org/project dashboard access and sourcemap upload were not verified.
...
- Treating env-name presence as provider readiness would be unsafe. Stripe, Resend, Twenty, and Sentry still need provider-dashboard validation before deployment completion is claimed.
...
- [ ] Provider/system: Sentry
      ...
      Required phase(s): Phase 3 deployment proof if release observability includes sourcemaps; Phase 11
