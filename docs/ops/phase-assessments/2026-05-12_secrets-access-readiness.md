...
- Twenty CRM is not complete for phase handoff because `TWENTY_WORKSPACE_ID` is missing locally and was not found in Vercel project env inventory. `TWENTY_API_URL`, `TWENTY_API_KEY`, and `TWENTY_WEBHOOK_SECRET` were not found in the project-level Vercel env inventory used for the three apps; any team-shared/custom development entries still need project-scope verification.
...
| Vercel                           | Projects, production env, custom/development env, deployment health                                                          | present                                                                            | production env present for `admin`, `donor`, `missionary`; custom env names present for many vars         | verified usable for deployments/health; env values unreadable by design    | provider endpoint proof for Phase 3 completion                        | Vercel team `asymmetric-al`, project settings, deployments          | Production verifier returned overall READY; latest deployments are development, but target commit production deployments are READY. |
...
| ----------------------------------------------- | ---------------- | --------------------- | --------------------------------- | ------------------------------------------------ | ---------------- | ------------------------ | ------------------------ | ----------------------------- | ---------------- | ---------------------------------------------------------------------------------------- |
| `NEXT_PUBLIC_SUPABASE_URL`                      | client           | Supabase              | 1-3, 8-9                          | local, custom/development, production            | present          | present                  | present                  | present                       | present          | Production verifier and build passed.                                                    |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY`                 | client           | Supabase              | 1-3, 8-9                          | local, custom/development, production            | present          | present                  | present                  | present                       | present          | Client-safe anon key; values not printed.                                                |
...
| `TWENTY_API_URL`                                | server           | Twenty CRM            | 3 if gift posting, 4-5            | server-only admin/API layer                      | missing          | missing                  | missing                  | missing                       | not verified     | User-provided development/team shared scope was not found in project-level inventory.    |
...
- Latest deployment in each project is a READY development deployment from `develop`; this does not replace the target commit production readiness proof.
...
- Twenty development/proof credentials are incomplete because `TWENTY_WORKSPACE_ID` is missing and project-scope Twenty env was not verified.
...
Production cutover is **not complete**. Do not mark Twenty production-complete until dated production domain evidence exists for development parity, rollback rehearsal, monitoring, backup/restore proof, support owner, rollback owner, and go/no-go record.
...
- Supabase CLI access listed projects, including active `production` and `development` projects.
...
- [ ] Provider/system: Twenty CRM
      ...
      Required scope: server-only admin/API runtime, preferably Vercel project custom/development and production when cutover is approved
...
- [ ] Provider/system: Twenty CRM
      ...
      Validation command/check: Vercel project env inventory shows the variables in the intended custom/development scope; data-boundary verifier still passes
...
- [ ] Provider/system: Stripe
      ...
      Required scope: test/development first; production only after go/no-go
...
- [ ] Provider/system: Resend
      ...
      Required scope: Resend production/development account aligned to Vercel app endpoint
...
- [ ] Provider/system: E2E/runtime verification
      ...
      Required scope: local or development runtime with test accounts and provider-safe env
      Where to get it: local dev servers plus Playwright setup, or development test environment
