...
- **Testing:** use the `instant()` helper from `@next/playwright` (root devDependency) to assert what must be visible immediately after a link click — see `tests/e2e/instant-navigation.spec.ts` for the pattern. Add `instant()` coverage when you build or change navigation-critical routes.
