...
- **16.3 tool set:** the MCP server is smaller and more focused. Its old knowledge-base/upgrade helper tools were removed (the bundled `node_modules/next/dist/docs/` now serve that purpose). Two compilation tools were added: `get_compilation_issues` (whole project) and `compile_route` (single route) — prefer these over running `next build` just to check whether code compiles while iterating.
...
## Instant Navigation (Next.js 16.3) — required posture for all UI work

All three apps run with `cacheComponents: true` **and** `partialPrefetching: true` (`apps/*/next.config.ts`). Navigations must stay instant; Next.js prefetches one reusable shell per route and Instant Insights raises a dev-time error when a navigation would block. Rules for all future development (human or agent):

- **Every server `await` in a route is an explicit choice — Stream, Cache, or Block:**
  - **Stream** with `<Suspense fallback={...}>` around the data access (default choice for request-time data).
  - **Cache** with `'use cache'` when the data can be reused between requests.
  - **Block** with `export const instant = false` in `page.tsx`/`layout.tsx` only when a route should deliberately wait for the server (leave a one-line comment explaining why).
- **Actionable errors:** Instant Insights errors in the dev overlay, `next dev` terminal, and `next build` output include labeled `[stream]`/`[cache]`/`[block]` fixes linking to `https://nextjs.org/docs/messages/...` pages written for agents — read the linked fix section (append `.md` for Markdown) before editing; don't improvise variations.
- **Prefetching:** the per-route shell is the baseline. Opt into deeper per-link prefetching with `<Link prefetch={true}>` plus `'use cache'` content; `export const prefetch = 'allow-runtime'` extends prefetching to request-time cached content and needs a justification in the PR.
- **Testing:** use the `instant()` helper from `@next/playwright` (root devDependency) to assert what must be visible immediately after a link click — see `tests/e2e/instant-navigation.spec.ts` for the pattern and `instant-nav.rig.md` for the production rig it requires (`instant()` verdicts from `next dev` are not trustworthy; the suite self-skips unless `INSTANT_NAV_RIG=1`). Add `instant()` coverage when you build or change navigation-critical routes.
- **Workflow skills:** `next-cache-components-adoption` (adopt Cache Components on routes), `next-cache-components-optimizer` (grow a route's static shell / make navigations instant), and `next-dev-loop` (verify runtime behavior after edits) are installed under `.agents/skills/` and mirrored to `.claude/skills/` and `.cursor/skills/`.
- **Agent browser:** `agent-browser` (root devDependency, ≥0.27 required by `next-dev-loop`) drives a real browser with React introspection — launch with `--enable react-devtools` for `react tree`, `react inspect <fiberId>`, `react renders start|stop`, and `react suspense --only-dynamic --json`.
...
- **TanStack Table v9 (beta) source verification:** this repo now **runs** Table `9.0.0-beta.9` (exact pin) behind the boundary module `packages/ui/components/shadcn/data-table/tanstack.ts`; new code must import table values/types from that boundary, not `@tanstack/react-table` directly (see `docs/guides/development/tanstack-integration.md`). Sanctioned exceptions are documented in `tanstack.ts` / `types.ts` (e.g. `packages/database` state types, `declare module` augmentation in `types.ts`, and `@tanstack/react-table-devtools` in table-creating files). tanstack.com Table docs and `tanstack search-docs --library table` currently index **v8** content; treat v8 docs as migration context only, never as forward-looking v9 API guidance. Back Table v9 decisions with v9/beta sources only: the [`TanStack/table` `beta` branch](https://github.com/TanStack/table/tree/beta) (the repo's default branch — v9 docs live under `docs/`, including `docs/framework/react/guide/migrating.md`), the published beta packages (`@tanstack/react-table@beta`, `@tanstack/table-core@beta`, which depend on `@tanstack/react-store`/`@tanstack/store`), or official TanStack announcements that clearly identify v9 behavior. A doc page counts as v9 only if it shows v9/beta APIs such as `useTable`, `tableFeatures`, `createTableHook`, `tableOptions`, `table.Subscribe`, `useLegacyTable`, `stockFeatures`, `rowModels`, or Store-backed state — otherwise treat it as v8 or ambiguous until proven otherwise.
...
- **Grow a route's static shell / make navigations instant:** `next-cache-components-optimizer` (first-party `vercel/next.js` skill; ecosystem install) — see **Instant Navigation (Next.js 16.3)** in this file
