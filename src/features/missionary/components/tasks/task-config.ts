...
import { Phone, Mail, CheckSquare, UserPlus, Heart, Users, Clock } from 'lucide-react'
import { format, isToday, isPast, isTomorrow, isThisWeek } from 'date-fns'
import type { TaskType, TaskStatus, TaskPriority } from '@/lib/missionary/types'
...
export const TASK_TYPE_CONFIG: Record<TaskType, { label: string; icon: React.ElementType; color: string; bgColor: string }> = {
...
export const PRIORITY_CONFIG: Record<TaskPriority, { label: string; color: string; badgeColor: string }> = {
