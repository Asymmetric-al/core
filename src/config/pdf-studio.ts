...
export function getPDFStudioBrandConfig(): PDFStudioBrandConfig {
  return {
    ...
    footerText: process.env.NEXT_PUBLIC_PDF_FOOTER_TEXT || process.env.NEXT_PUBLIC_EMAIL_FOOTER_TEXT
  }
}
  ...
  tax_receipt: {
    ...
    mergeTags: {
      ...
      tax_statement: {
        name: 'Tax Statement',
        value: '{{tax_statement}}',
        sample: 'No goods or services were provided in exchange for this contribution.'
      }
    }
  }
}
...
export function getPDFStudioSetupStatus(): {
  ...
  if (!config.isConfigured) {
    return {
      ...
      message: 'PDF Studio is running in free mode. Add your Unlayer project ID to unlock all features.',
  ...
  if (config.isWhiteLabel) {
    return {
      status: 'white_label',
      message: 'PDF Studio is fully configured with white-label features enabled.',
      setupUrl: 'https://dashboard.unlayer.com',
  ...
  return {
    status: 'configured',
    message: 'PDF Studio is configured. Upgrade to white-label to remove branding.',
    setupUrl: 'https://dashboard.unlayer.com',
...
export const PDF_STUDIO_SETUP_INSTRUCTIONS = {
  ...
  steps: [
    ...
    },
    {
      ...
      details: 'Go to Projects → Create New Project → Choose "Document" as the project type'
    },
    {
      step: 3,
