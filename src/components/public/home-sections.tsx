...
import { ArrowRight, Activity, Users, Globe, Sparkles, Heart, ShieldCheck, Zap } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { siteConfig } from '@/config/site';

const HERO_IMAGE = "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?q=80&w=2070&auto=format&fit=crop";
const MISSION_IMAGE = "https://images.unsplash.com/photo-1594708767771-a7502209ff51?q=80&w=2000&auto=format&fit=crop";
      ...
      <div className="flex animate-marquee gap-12 items-center" role="marquee">
        {[...activities, ...activities].map((text, i) => (
          <div key={i} className="flex items-center gap-3 text-xs font-bold uppercase tracking-widest text-zinc-900">
            <span className="h-1.5 w-1.5 rounded-full bg-zinc-900 shadow-[0_0_8px_rgba(0,0,0,0.2)]" aria-hidden="true" />
            <span>{text}</span>
          </div>
        ))}
      </div>
...
export function HomeHero() {
  return (
    ...
    >
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-6">
            ...
            >
              <Link href="/workers">
                ...
                <Zap className="ml-2 h-4 w-4 fill-current transition-transform group-hover:rotate-12" aria-hidden="true" />
              </Link>
            </Button>
            <Button 
              size="lg" 
            ...
            >
              <Link href="/about">
                Our Methodology
                <ArrowRight className="ml-2 h-4 w-4 opacity-40 transition-transform group-hover:translate-x-2 opacity-100" aria-hidden="true" />
              </Link>
            </Button>
          </div>
        </div>
      ...
      <div className="absolute bottom-24 right-6 hidden xl:flex flex-col gap-4" aria-hidden="true">
        {[
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-white/30 flex flex-col items-center gap-3 text-[10px] font-bold tracking-[0.3em] uppercase">
        ...
        <div className="w-px h-16 bg-gradient-to-b from-white/0 via-white/40 to-white/0 animate-pulse" aria-hidden="true" />
      </div>
    </section>
  );
}
...
export function HomeMission() {
  return (
    <section aria-labelledby="mission-heading" className="py-24 md:py-40 bg-white relative overflow-hidden">
      <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-zinc-50 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/2 -z-10" aria-hidden="true" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32 items-center">
          <div className="space-y-12">
            <header className="space-y-6">
              ...
              <h2 id="mission-heading" className="text-5xl sm:text-6xl md:text-8xl font-bold tracking-tighter text-slate-950 leading-[0.85] font-syne">
                Precision <br/>
            ...
            <div className="space-y-8 text-xl sm:text-2xl text-slate-600 leading-relaxed font-light tracking-tight">
              <p>
                In a world of increasing volatility, traditional charity models are too slow. <strong className="text-slate-900 font-bold">{siteConfig.name}</strong> operates on a zero-friction, direct-support model.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <article className="space-y-3 p-5 rounded-2xl bg-slate-50 border border-slate-100">
                  <ShieldCheck className="h-6 w-6 text-zinc-900" aria-hidden="true" />
                  <h3 className="font-bold text-slate-900 font-syne">100% Direct</h3>
                ...
                <article className="space-y-3 p-5 rounded-2xl bg-slate-50 border border-slate-100">
                  ...
                  <h3 className="font-bold text-slate-900 font-syne">Real-Time Data</h3>
            ...
            <div className="pt-10">
              <Link href="/about" className="group inline-flex items-center text-xs font-black text-slate-950 uppercase tracking-[0.3em]">
                ...
                <ArrowRight className="ml-5 h-5 w-5 group-hover:translate-x-3 transition-transform group-hover:text-zinc-900" aria-hidden="true" />
              </Link>
            </div>
          </div>
          
          <figure className="relative lg:ml-auto group">
            ...
            <div className="absolute -top-6 -right-6 bg-white p-6 rounded-2xl shadow-xl border border-slate-100 max-w-[200px] hidden xl:block">
              <Sparkles className="h-5 w-5 text-zinc-400 mb-3" aria-hidden="true" />
              <blockquote className="text-xs font-bold text-slate-900 leading-tight">
                &quot;Our fastest deployment yet. Resources reached the field in <span className="text-zinc-900">under 4 hours</span>.&quot;
              </blockquote>
              <footer className="mt-4 flex items-center gap-3">
                <div className="h-7 w-7 rounded-full bg-slate-200" aria-hidden="true" />
                <div>
                  <cite className="text-[9px] font-black uppercase tracking-wider text-slate-900 not-italic">Dr. Elias H.</cite>
...
export function HomeStats() {
  return (
    <section aria-labelledby="stats-heading" className="py-24 md:py-40 bg-slate-950 text-white relative overflow-hidden">
      <div className="absolute inset-0 opacity-[0.1] mix-blend-screen" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, white 1px, transparent 0)', backgroundSize: '48px 48px' }} aria-hidden="true" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <header className="flex flex-col md:flex-row md:items-end justify-between mb-16 md:mb-32 gap-12">
          <div className="space-y-6">
            ...
            <h2 id="stats-heading" className="text-5xl sm:text-6xl md:text-8xl font-bold tracking-tighter font-syne">Global <br/>Impact Score.</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          <article className="md:col-span-8 group bg-white/5 backdrop-blur-3xl border border-white/10 p-8 md:p-10 rounded-2xl hover:bg-white/10 transition-all duration-500 flex flex-col justify-between min-h-[300px] md:min-h-[400px]">
            <div>
              <Activity className="h-8 w-8 text-zinc-400 mb-8" aria-hidden="true" />
            ...
            <p className="text-6xl md:text-7xl lg:text-9xl font-bold tracking-tighter leading-none text-transparent bg-clip-text bg-gradient-to-br from-white via-white to-white/5 font-syne">$26M+</p>
          </article>
          ...
          <article className="md:col-span-4 group bg-white p-8 md:p-10 rounded-2xl hover:scale-[1.02] transition-all duration-500 flex flex-col justify-between text-slate-950">
            ...
            <div>
              <p className="text-6xl md:text-7xl font-bold tracking-tighter font-syne mb-4">42k</p>
          ...
          <article className="md:col-span-8 group bg-slate-900 border border-white/5 p-8 md:p-10 rounded-2xl hover:bg-slate-800 transition-all duration-500 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12">
            <div className="space-y-4">
              <div className="flex gap-1.5" aria-label="5 out of 5 stars">
                {[...Array(5)].map((_, i) => <Heart key={i} className="h-4 w-4 text-zinc-400 fill-current" aria-hidden="true" />)}
              </div>
            ...
            <div className="h-24 w-24 rounded-full border-4 border-white/20 flex items-center justify-center text-2xl font-bold font-syne text-white shrink-0" aria-label="A+ Rating">
              A+
            </div>
          </article>
        </div>
...
export function HomeFeatured() {
  return (
    <section aria-labelledby="featured-heading" className="py-24 md:py-40 bg-slate-50 overflow-hidden">
      <div className="container mx-auto px-6">
        <header className="flex flex-col md:flex-row justify-between items-end mb-16 md:mb-24 gap-8 md:gap-12">
          <div className="space-y-6">
            ...
            <h2 id="featured-heading" className="text-5xl sm:text-6xl md:text-8xl font-bold tracking-tighter text-slate-950 font-syne">Current Priorities.</h2>
          </div>
          <Link href="/workers" className="hidden md:flex items-center text-xs font-black text-slate-950 hover:text-zinc-600 transition-all uppercase tracking-[0.3em]">
            View Full Directory <ArrowRight className="ml-5 h-5 w-5" aria-hidden="true" />
          </Link>
        </header>
            ...
            >
              <Link href="/workers" className="block">
                <div className="relative aspect-[3/4] rounded-3xl overflow-hidden mb-6 bg-slate-200 shadow-xl group-hover:shadow-zinc-500/10 transition-all duration-700">
                  ...
                  <div className="absolute bottom-8 left-8 right-8">
                    <div className="flex items-center gap-2 text-[9px] font-black text-white/70 uppercase tracking-[0.2em] mb-3">
                      <Globe className="h-3 w-3" aria-hidden="true" /> {item.loc}
                    </div>
                    <h3 className="text-2xl sm:text-3xl font-bold text-white font-syne leading-none mb-4 group-hover:translate-x-2 transition-transform duration-500">{item.title}</h3>
                    <div className="h-1 w-full bg-white/20 rounded-full overflow-hidden" role="progressbar" aria-valuenow={parseInt(item.raised)} aria-valuemin={0} aria-valuemax={100}>
                      <div className="h-full bg-white transition-all duration-1000" style={{ width: item.raised }} />
                    </div>
                  </div>
                </div>
                <div className="flex items-center text-[10px] font-black text-zinc-900 opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500 uppercase tracking-[0.2em]">
                  Join the Mission <ArrowRight className="h-3 w-3 ml-2" aria-hidden="true" />
                </div>
              </Link>
            </article>
          ))}
        ...
        <div className="mt-16 md:mt-20 text-center md:hidden">
          <Button variant="outline" className="w-full h-16 md:h-20 rounded-full border-slate-200 text-slate-950 font-black font-syne text-base md:text-lg tracking-widest uppercase" asChild>
...
export function HomeCTA() {
  return (
    <section aria-labelledby="cta-heading" className="py-24 md:py-32 bg-slate-950 relative overflow-hidden text-center flex flex-col items-center justify-center">
      ...
      <div className="container mx-auto px-6 relative z-10 max-w-5xl">
        <h2 id="cta-heading" className="text-5xl sm:text-6xl md:text-8xl font-bold text-white tracking-tighter mb-8 leading-[0.8] font-syne">
          Be the <br/>
