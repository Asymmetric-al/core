...
import { useRef, useCallback, useState, forwardRef, useImperativeHandle, useMemo, useEffect, useId, useSyncExternalStore } from 'react'
...
export const UnlayerEditor = forwardRef<UnlayerEditorHandle, UnlayerEditorProps>(
  function UnlayerEditor(
    ...
    const [loadingState, setLoadingState] = useState<'mounting' | 'loading' | 'ready'>('mounting')
    const [loadingProgress, setLoadingProgress] = useState(0)
    const designLoadedRef = useRef(false)
    const isMounted = useSyncExternalStore(emptySubscribe, getClientSnapshot, getServerSnapshot)
    const reactId = useId()
    const stableEditorId = editorId || `unlayer-editor-${mode}-${reactId.replace(/:/g, '')}`
    ...
    const exportHtml = useCallback((options?: ExportOptions): Promise<UnlayerExportHTML> => {
    ...
    const exportPdf = useCallback((options?: PdfExportOptions): Promise<UnlayerPdfExportResult> => {
      return new Promise((resolve, reject) => {
        const unlayer = emailEditorRef.current?.editor
        if (!unlayer) {
          reject(new Error('Editor not ready'))
        ...
        const exportOptions = options?.mergeTags ? { mergeTags: options.mergeTags } : undefined
        
        unlayer.exportPdf((data: { design: UnlayerDesignJSON; url: string | null }) => {
          resolve(data)
        }, exportOptions)
      })
    }, [])
    ...
    useImperativeHandle(ref, () => ({
      ...
      redo,
      getConfig
    }), [exportHtml, exportPdf, exportDesign, loadDesign, saveDesign, setMergeTags, showPreview, undo, redo, getConfig])
    ...
    const handleEditorReady: EmailEditorProps['onReady'] = useCallback((unlayer: Editor) => {
      ...
      unlayer.addEventListener('design:updated', (data: { design: UnlayerDesignJSON }) => {
    ...
    return (
      ...
      >
        {loadingState !== 'ready' && (
          <div 
            className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-background/95 backdrop-blur-sm transition-opacity duration-300"
          >
            <div className="flex flex-col items-center gap-6 max-w-xs text-center">
              <div className="relative">
                <div className="absolute inset-0 rounded-2xl bg-primary/20 blur-xl animate-pulse" />
              ...
              <div className="space-y-2 w-full">
                <div className="flex items-center justify-center gap-2">
                  <Loader2 className="h-4 w-4 animate-spin text-primary" />
                  <span className="text-sm font-medium text-foreground">
                    {loadingState === 'mounting' ? `Preparing ${loadingLabel} editor...` : 'Loading components...'}
                  </span>
                </div>
