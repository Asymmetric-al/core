...
export interface UseColumnResizingReturn {
  ...
  getResizeHandler: (header: Header<unknown, unknown>) => (e: React.MouseEvent | React.TouchEvent) => void
  isResizing: boolean
}
  ...
  const getResizeHandler = useCallback(
    (header: Header<unknown, unknown>) => {
      ...
      return (e: React.MouseEvent | React.TouchEvent) => {
          ...
          const startX = "touches" in e && e.touches.length > 0 ? (e.touches[0]?.clientX ?? 0) : (e as React.MouseEvent).clientX
          const startSize = header.getSize()
          const columnId = header.column.id

          const handleMove = (moveEvent: MouseEvent | TouchEvent) => {
            const currentX = "touches" in moveEvent && (moveEvent as TouchEvent).touches.length > 0
...
export function ColumnResizeHandle({
  ...
  return (
    <div
      ...
      className={className ?? "absolute right-0 top-0 h-full w-1 cursor-col-resize select-none touch-none bg-transparent hover:bg-primary/50 active:bg-primary transition-colors"}
      style={{
...
export function getColumnResizingTableOptions(options: ColumnResizingOptions = {}) {
  const {
    enabled = true,
    defaultColumnSize = 150,
    minColumnSize = 50,
