...
interface OptimisticMutationConfig<TData, TVariables> {
  ...
  onSettled?: (data: TData | undefined, error: Error | null) => Promise<void> | void
  select?: string
  matchColumn?: string
}
...
export function useDataTableMutation<TData extends Record<string, unknown>, TVariables = Partial<TData>>({
  tableName,
  queryKey,
  type,
  getId,
  ...
  select = "*",
  matchColumn = "id",
}: OptimisticMutationConfig<TData, TVariables>): UseDataTableMutationReturn<TData, TVariables> {
...
export function useDataTableBulkMutation<TData extends Record<string, unknown>, TVariables = Partial<TData>>({
  tableName,
  queryKey,
  type,
  getId,
  ...
  const mutation = useMutation<number, Error, TVariables[], { previousData: TData[] | undefined }>({
