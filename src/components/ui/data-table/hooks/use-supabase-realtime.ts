...
export function useSupabaseRealtime<TData extends Record<string, unknown>>({
  ...
  const handlePayload = React.useCallback(
    (payload: RealtimePayload) => {
      ...
      const transformData = (data: Record<string, unknown> | null): TData | null => {
        if (!data) return null
          ...
          if (newRecord) {
            queryClient.setQueryData<TData[]>(queryKey, (old) => {
              ...
              const exists = old.some(
                (item) => (item as Record<string, unknown>).id === (newRecord as Record<string, unknown>).id
              )
        ...
        case "UPDATE": {
          ...
          if (newRecord) {
            queryClient.setQueryData<TData[]>(queryKey, (old) => {
              if (!old) return old
              return old.map((item) =>
                (item as Record<string, unknown>).id === (newRecord as Record<string, unknown>).id
                  ? newRecord
                  : item
              )
            })
        ...
        case "DELETE": {
          ...
          if (deletedRecord) {
            queryClient.setQueryData<TData[]>(queryKey, (old) => {
              ...
              return old.filter(
                (item) =>
                  (item as Record<string, unknown>).id !== (deletedRecord as Record<string, unknown>).id
              )
            })
  ...
  const subscribe = React.useCallback(() => {
    ...
    events.forEach((event) => {
      ...
      channel.on(
        ...
        handlePayload as unknown as (payload: { extension: string; status: string; message: string }) => void
      )
    })
...
export function useDataTableWithRealtime<TData extends Record<string, unknown>, TValue = unknown>({
  tableName,
  queryKey,
  realtimeEvents = ["*"],
    ...
    subscribe: (
      ...
      events.forEach((event) => {
        ...
        channel.on(
          ...
          }) as unknown as (payload: { extension: string; status: string; message: string }) => void
        )
      })
