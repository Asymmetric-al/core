...
export interface UseDataTableKeyboardReturn {
  ...
  getCellProps: (rowIndex: number, cellIndex: number) => {
    tabIndex: number
...
export function useDataTableKeyboard<TData>(
  ...
  const [focusedRowIndex, setFocusedRowIndexRaw] = useState<number | null>(null)
  ...
  const getCellProps = useCallback(
    (rowIndex: number, cellIndex: number) => ({
      tabIndex: validatedFocusedRowIndex === rowIndex && focusedCellIndex === cellIndex ? 0 : -1,
      "data-cell-focused": validatedFocusedRowIndex === rowIndex && focusedCellIndex === cellIndex,
    }),
...
export function getKeyboardNavigationStyles() {
  return {
    focusedRow: "outline-none ring-2 ring-ring ring-offset-1 ring-offset-background",
