  ...
  const removeCondition = useCallback(
    (conditionId: string) => {
      setFilterState((prev) => {
        const updated = {
          ...prev,
          conditions: prev.conditions.filter((c: FilterCondition) => c.id !== conditionId),
        }
        onFilterChange?.(updated)
        return updated
      })
...
export function createAdvancedFilterFn<TData>(
  filter: AdvancedFilterState
): (row: TData, getRowValue: (row: TData, columnId: string) => unknown) => boolean {
