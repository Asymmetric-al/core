...
import { uploadToCloudinary, isCloudinaryEnabled } from '@/lib/cloudinary-client'
import {
  validateImageFile,
  loadImageAsDataUrl,
  validateImageLoads,
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (file.size > 5 * 1024 * 1024) {
        toast.info(`Large file (${formatFileSize(file.size)}) will be optimized on upload`)
      }
    } catch (error) {
    ...
    const supabase = createClient()
      ...
      try {
        ...
        if (isCloudinaryEnabled) {
          ...
          const metadataResponse = await fetch('/api/upload/cloudinary/metadata', {
            method: 'POST',
            body: JSON.stringify({
              cloudinaryData,
              options: { 
            ...
            >
              {isUploading ? (
                ...
                <>
                  ...
                  <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-400">Upload</span>
                </>
              )}
            </Button>
          )}
