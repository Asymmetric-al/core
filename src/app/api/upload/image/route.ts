...
export async function POST(request: NextRequest) {
  try {
    ...
    if (displayUpload.error) {
      return NextResponse.json({ error: `Display upload failed: ${displayUpload.error.message}` }, { status: 500 })
    }
    ...
    const { data: { publicUrl: displayUrl } } = supabaseAdmin.storage.from(bucket).getPublicUrl(displayPath)
    const { data: { publicUrl: thumbnailUrl } } = supabaseAdmin.storage.from(bucket).getPublicUrl(thumbnailPath)
