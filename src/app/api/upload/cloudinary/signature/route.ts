...
import { generateCloudinarySignature, isCloudinaryEnabled } from '@/lib/cloudinary-server'

export async function POST(request: NextRequest) {
  try {
    if (!isCloudinaryEnabled) {
      return NextResponse.json({ error: 'Cloudinary pipeline is disabled' }, { status: 403 })
    }
