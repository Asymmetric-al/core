...
export async function POST(request: Request) {
  if (process.env.NODE_ENV === 'production' && process.env.ALLOW_DEMO_ACCOUNTS !== 'true') {
    return NextResponse.json({ ok: false, error: 'Demo login unavailable' }, { status: 403 })
  }
  ...
  try {
    ...
    if (!role || !DEMO_ROLES.includes(role)) {
      return NextResponse.json({ ok: false, error: 'Demo login unavailable' }, { status: 400 })
    }
    ...
    if (!supabase) {
      return NextResponse.json({ ok: false, error: 'Demo login unavailable' }, { status: 503 })
    }
