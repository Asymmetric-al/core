...
import { getDonorFeedPreferences, upsertDonorFeedPreferences } from '@/lib/donor-dashboard'
...
export async function GET() {
  try {
    ...
    const preferences = await getDonorFeedPreferences(donor.id, DEFAULT_TENANT_ID)
...
export async function POST(request: Request) {
  try {
    ...
    const updated = await upsertDonorFeedPreferences(donor.id, DEFAULT_TENANT_ID, body)
