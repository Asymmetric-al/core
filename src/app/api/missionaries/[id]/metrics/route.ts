...
const PERMISSION_ERROR_CODES = new Set(['42501', 'PGRST301', 'PGRST302', 'PGRST401', 'PGRST403'])
...
function isPermissionError(error: { code?: string; message?: string } | null) {
  ...
  return message.includes('permission') || message.includes('not authorized') || message.includes('not allowed')
}
...
export async function GET(
  ...
  try {
    ...
    if (!profile?.tenant_id || !profile?.role || !ALLOWED_ROLES.has(profile.role)) {
      return jsonWithCookies({ error: 'Forbidden' }, { status: 403 }, pendingCookies)
    }
    ...
    return jsonWithCookies({ donations: data || [] }, undefined, pendingCookies)
  } catch (e) {
