    ...
    <div className={cn(
      ...
      <div className="z-10 space-y-4 text-left">
        <div className="flex gap-3 items-center pl-1">
           {isBank ? (
             <span className="text-lg tracking-widest font-mono opacity-80 uppercase">Account</span>
           ) : (
             <>
        ...
        <div className="flex justify-between items-end">
           <div className="space-y-0.5">
              <p className="text-[8px] uppercase tracking-widest opacity-60 font-black">{isBank ? 'Account Name' : 'Card Holder'}</p>
           ...
           {!isBank && method.expiryMonth && (
             <div className="space-y-0.5 text-right">
                <p className="text-[8px] uppercase tracking-widest opacity-60 font-black">Expires</p>
        ...
        return (
          ...
          >
            <div className={cn(
              ...
              isSelected ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-300 bg-white group-hover:border-zinc-400"
            )}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {method.isDefault && <Badge variant="secondary" className="text-[9px] px-1.5 h-4 bg-zinc-900 text-white border-zinc-900 font-black uppercase tracking-widest rounded-md">Default</Badge>}
              </div>
              <p className="text-[10px] font-bold text-zinc-400 mt-0.5 uppercase tracking-widest">
                ...
                {method.expiryMonth && <span className="ml-2 opacity-50 border-l border-zinc-300 pl-2">Exp {method.expiryMonth}/{method.expiryYear?.toString().slice(-2)}</span>}
              </p>
            </div>
          </motion.div>
        );
  ...
  return (
    <div className="space-y-3 pt-4 border-t border-zinc-100 text-left">
      <div className="flex items-center gap-2 mb-1">
        ...
        <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Billing Address</Label>
      </div>
...
const CardForm = ({ formData, setFormData, isEditing }: any) => (
  <>
    <div className="space-y-2 text-left">
        <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Card Number</Label>
        <div className="relative group">
            ...
            <Input 
                ...
                className={cn(
                    ...
                    isEditing && "bg-zinc-100 text-zinc-500 border-zinc-100 cursor-not-allowed"
                )}
                value={formData.number}
                onChange={(e) => setFormData({...formData, number: e.target.value})}
                disabled={isEditing}
            />
            {isEditing && <Lock className="absolute right-4 top-3.5 h-4 w-4 text-zinc-400" />}
        </div>
    </div>
    <div className="grid grid-cols-2 gap-6 text-left">
        <div className="space-y-2">
            <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Expiration</Label>
        ...
        <div className="space-y-2">
            <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">CVC</Label>
            <Input 
                placeholder="123" 
    ...
    <div className="space-y-2 text-left">
        <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Cardholder Name</Label>
        <div className="relative">
...
const BankForm = ({ formData, setFormData, isEditing }: any) => (
  <>
    <div className="bg-emerald-50 text-emerald-800 text-xs p-4 rounded-xl flex items-start gap-3 border border-emerald-100 shadow-sm text-left uppercase font-bold tracking-tight">
        ...
        <span className="leading-relaxed">Pro Tip: Bank transfers save us ~2.5% in fees. That means more of your gift goes directly to the field!</span>
    </div>
    
    <div className="space-y-5 text-left">
        <div className="space-y-2">
            <Label htmlFor="routing" className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Routing Number</Label>
            <div className="relative group">
        ...
        <div className="space-y-2">
            <Label htmlFor="account" className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Account Number</Label>
            <div className="relative group">
        ...
        <div className="space-y-2">
            <Label htmlFor="holder" className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Account Holder Name</Label>
            <div className="relative group">
                <User className="absolute left-4 top-3.5 h-5 w-5 text-zinc-400" />
                <Input 
                    id="holder"
    ...
    <div className="flex items-center justify-center gap-2 text-[10px] font-bold uppercase tracking-widest text-zinc-400 bg-white py-3 rounded-xl border border-zinc-200 mt-4 shadow-sm">
        ...
        <span className="font-medium">Details are stored securely via <strong>Stripe</strong>.</span>
    </div>
  </>
);
  ...
  const handleSaveMethod = () => {
    if (editingMethod) {
      setMethods(prev => prev.map(m => m.id === editingMethod.id ? {
        ...
        expiryMonth: (formData.expiry && formData.expiry.length === 5) ? parseInt(formData.expiry.split('/')[0] ?? '0') : m.expiryMonth,
        expiryYear: (formData.expiry && formData.expiry.length === 5) ? 2000 + parseInt(formData.expiry.split('/')[1] ?? '0') : m.expiryYear,
      ...
      const newMethod: PaymentMethod = {
        ...
        color: isBank ? undefined : (formData.number.startsWith('5') ? 'from-zinc-900 to-zinc-800' : 'from-zinc-900 to-zinc-800'),
        billingAddress: formData.address
      };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 px-1 text-left">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-zinc-900 tracking-tight uppercase">Wallet</h1>
          <p className="text-zinc-500 text-lg font-bold uppercase tracking-widest text-[10px] mt-1.5">Manage your payment methods securely.</p>
        </div>
        <Button onClick={openAddModal} className="bg-zinc-900 hover:bg-zinc-800 text-white shadow-xl hover:shadow-2xl hover:-translate-y-0.5 transition-all h-12 px-6 font-black uppercase tracking-widest text-[10px] rounded-lg">
        ...
        {showACHNudge && (
          ...
          >
             ...
             <div className="flex gap-5 items-start relative z-10">
                ...
                <div>
                   <h3 className="font-bold text-emerald-950 text-lg uppercase tracking-tight">Maximize your impact with ACH</h3>
                   <p className="text-emerald-800/80 mt-2 max-w-2xl text-[10px] font-bold uppercase tracking-widest leading-relaxed">
                      Credit card processing fees cost nonprofits ~2.5% per donation. Switching to a direct bank transfer (ACH) lowers this to nearly zero, meaning <strong className="text-emerald-950">more of your gift goes directly to the field.</strong>
                   </p>
                   <Button variant="link" onClick={() => { openAddModal(); setActiveTab('bank'); }} className="p-0 h-auto text-emerald-700 font-black mt-3 text-[10px] uppercase tracking-widest hover:text-emerald-900 flex items-center gap-1 group/btn">
      ...
      <div className="space-y-8">
         {methods.length === 0 && (
            <div className="text-center py-20 bg-zinc-50 rounded-3xl border-2 border-dashed border-zinc-200">
               ...
               <h3 className="text-xl font-bold text-zinc-900 mb-2 uppercase tracking-tighter">No payment methods yet</h3>
               <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-6">Add a card or bank account to start giving.</p>
               <Button variant="outline" onClick={openAddModal} className="font-black uppercase tracking-widest text-[10px] h-10 px-6 rounded-lg">Add Method</Button>
            </div>
         )}
                     ...
                     <div className="flex flex-col lg:flex-row gap-8 p-6 lg:p-8">
                        ...
                        <div className="flex-1 flex flex-col min-w-0">
                           <div className="flex items-start justify-between mb-6">
                              ...
                              <DropdownMenu>
                                 <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" size="icon" className="h-10 w-10 text-zinc-300 hover:text-zinc-900 hover:bg-zinc-50 rounded-full">
                                       <MoreHorizontal className="h-5 w-5" />
                                    </Button>
                                 </DropdownMenuTrigger>
                                 <DropdownMenuContent align="end" className="w-56 p-2 rounded-xl">
                                    <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-widest text-zinc-400 px-2 py-1.5">Manage Method</DropdownMenuLabel>
                                    <DropdownMenuSeparator className="my-1" />
                                    {!method.isDefault && (
                                       <DropdownMenuItem onClick={() => handleSetDefault(method.id)} className="rounded-lg cursor-pointer font-bold uppercase tracking-widest text-[10px]">Set as Default</DropdownMenuItem>
                                    )}
                                    <DropdownMenuItem onClick={() => openEditModal(method)} className="rounded-lg cursor-pointer font-bold uppercase tracking-widest text-[10px]">
                                       <Edit2 className="mr-2 h-3.5 w-3.5" /> Edit Details
                                    </DropdownMenuItem>
                                    <DropdownMenuSeparator className="my-1" />
                                    <DropdownMenuItem onClick={() => handleDeleteRequest(method.id)} className="text-rose-600 focus:text-rose-600 focus:bg-rose-50 rounded-lg cursor-pointer font-bold uppercase tracking-widest text-[10px] group">
                                       <Trash2 className="mr-2 h-3.5 w-3.5 group-hover:scale-110 transition-transform" /> Remove
                                    </DropdownMenuItem>
                                 </DropdownMenuContent>
                              </DropdownMenu>
                           </div>
                           ...
                           <div className="mb-6 flex gap-3 items-start">
                              ...
                              <div>
                                <p className="text-[10px] font-black text-zinc-300 uppercase tracking-widest mb-0.5">Billing Address</p>
                           ...
                           <div className="mt-auto pt-6 border-t border-zinc-50">
                              <p className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                                 Connected Impact <span className="bg-zinc-100 text-zinc-900 px-1.5 py-0.5 rounded text-[9px] min-w-[20px] text-center font-black">{attachedPledges.length}</span>
                              </p>
                              
                              {attachedPledges.length > 0 ? (
                                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {attachedPledges.map(pledge => (
                                       <div key={pledge.id} className="flex items-center gap-4 p-3 rounded-xl bg-zinc-50/50 border border-zinc-100 hover:border-zinc-300 hover:bg-white transition-all group/pledge cursor-default shadow-sm hover:shadow-md">
                                           {pledge.avatar ? (
                                              <Image src={pledge.avatar} alt="" width={40} height={40} className="h-10 w-10 rounded-lg object-cover bg-white ring-2 ring-white shadow-sm" />
                                           ) : (
                                             <div className="h-10 w-10 rounded-lg bg-white flex items-center justify-center text-zinc-300 text-[10px] font-black ring-2 ring-white shadow-sm uppercase">GH</div>
                                          )}
                                          <div className="flex-1 min-w-0">
                                             <p className="text-[11px] font-bold text-zinc-900 truncate uppercase tracking-tight">{pledge.name}</p>
                                             <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">{formatCurrency(pledge.amount)} / {pledge.frequency}</p>
                                          </div>
                                 ...
                                 <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-zinc-400 italic bg-zinc-50/30 p-4 rounded-xl border border-dashed border-zinc-200">
                                    <div className="p-2 bg-white rounded-lg shadow-sm border border-zinc-100"><Wallet className="h-3.5 w-3.5 text-zinc-300" /></div>
      ...
      <Dialog open={isMethodModalOpen} onOpenChange={setIsMethodModalOpen}>
         <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden gap-0 rounded-3xl max-h-[90vh] overflow-y-auto">
               ...
               {!editingMethod ? (
                  <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
                     <TabsList className="grid w-full grid-cols-2 mb-8 bg-white p-1 rounded-xl shadow-sm border border-zinc-200 h-12">
                        <TabsTrigger value="card" className="rounded-lg font-black uppercase tracking-widest text-[10px] data-[state=active]:bg-zinc-900 data-[state=active]:text-white transition-all shadow-none">Credit Card</TabsTrigger>
                        <TabsTrigger value="bank" className="rounded-lg font-black uppercase tracking-widest text-[10px] data-[state=active]:bg-zinc-900 data-[state=active]:text-white transition-all shadow-none">Bank Account</TabsTrigger>
                     </TabsList>
            ...
            <DialogFooter className="p-6 bg-white border-t border-zinc-100 flex flex-col sm:flex-row gap-3 sticky bottom-0 z-10">
               <Button variant="ghost" onClick={() => setIsMethodModalOpen(false)} className="h-12 font-bold uppercase tracking-widest text-[10px] px-6">Cancel</Button>
               <Button onClick={handleSaveMethod} className="bg-zinc-900 hover:bg-zinc-800 text-white shadow-lg h-12 px-8 font-black uppercase tracking-widest text-[10px] rounded-xl transition-transform active:scale-95">
      ...
      <Dialog open={isSwapPledgeOpen} onOpenChange={setIsSwapPledgeOpen}>
         <DialogContent className="sm:max-w-[500px] border-none shadow-2xl rounded-3xl overflow-hidden p-0 gap-0">
            <DialogHeader className="p-8 pb-6 bg-zinc-50 border-b border-zinc-100 text-center">
                ...
                <DialogTitle className="text-xl font-bold text-zinc-900 uppercase tracking-tighter">Move Support</DialogTitle>
            ...
            <div className="p-8 space-y-8 bg-white min-h-[300px] max-h-[60vh] overflow-y-auto text-left">
                <div className="flex flex-col items-center gap-4">
                    <div className="bg-white p-4 rounded-2xl border border-zinc-200 shadow-sm flex items-center gap-4 w-full">
                        {pledgeToSwap?.avatar ? (
                            <Image src={pledgeToSwap.avatar} alt="" width={48} height={48} className="h-12 w-12 rounded-lg object-cover ring-2 ring-white shadow-sm" />
                        ) : (
                            <div className="h-12 w-12 rounded-lg bg-zinc-50 flex items-center justify-center text-zinc-300 font-black text-xs uppercase border border-zinc-100">GH</div>
                        )}
                        <div className="flex-1">
                            <p className="font-bold text-zinc-900 uppercase tracking-tight">{pledgeToSwap?.name}</p>
                            <div className="flex items-center gap-2 mt-0.5">
                                <Badge variant="secondary" className="px-1.5 py-0 text-[9px] font-black uppercase tracking-widest bg-zinc-100 text-zinc-500 rounded-md border-transparent shadow-none">{pledgeToSwap?.frequency}</Badge>
                                <span className="text-sm font-bold text-zinc-900 tabular-nums">{formatCurrency(pledgeToSwap?.amount || 0)}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-4">
                    <Label className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.2em] ml-1">Move To</Label>
                    {methods.filter(m => m.id !== pledgeToSwap?.paymentMethodId).length > 0 ? (
                        ...
                        <div className="p-6 bg-zinc-50 rounded-2xl border border-dashed border-zinc-200 text-center space-y-3">
                            ...
                            <div>
                                <p className="text-xs font-bold text-zinc-900 uppercase tracking-tight">No other payment methods</p>
                                <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mt-1">Add a new method to move this support.</p>
                            </div>
                            <Button size="sm" variant="outline" onClick={() => { setIsSwapPledgeOpen(false); openAddModal(); }} className="font-black uppercase tracking-widest text-[9px] h-8 px-4 rounded-lg">
                                + Add Method
                            </Button>
                        </div>
                    )}
            ...
            <DialogFooter className="p-6 bg-zinc-50 border-t border-zinc-100 flex justify-between">
                <Button variant="ghost" onClick={() => setIsSwapPledgeOpen(false)} className="font-bold uppercase tracking-widest text-[10px] h-10 px-4">Cancel</Button>
                <Button 
      ...
      <Dialog open={isMovePledgesOpen} onOpenChange={setIsMovePledgesOpen}>
         <DialogContent className="sm:max-w-[500px] border-none shadow-2xl rounded-3xl overflow-hidden p-0 gap-0">
            <DialogHeader className="p-8 pb-6 bg-rose-50 border-b border-rose-100 text-center">
               ...
               <DialogTitle className="text-2xl font-bold text-rose-950 uppercase tracking-tighter">Active Support Detected</DialogTitle>
            ...
            <div className="p-8 space-y-8 bg-white min-h-[300px] max-h-[60vh] overflow-y-auto text-left">
               <div className="space-y-3">
                  <p className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.2em] ml-1">Support to Transfer</p>
                  <ul className="space-y-2">
                     {pledges.filter(p => p.paymentMethodId === methodToDelete).map(p => (
                        <li key={p.id} className="text-[11px] font-bold uppercase tracking-tight flex items-center justify-between bg-zinc-50 p-3 rounded-xl border border-zinc-100">
                           <div className="flex items-center gap-3">
                              {p.avatar ? (
                                 <Image src={p.avatar} width={32} height={32} className="w-8 h-8 rounded-lg bg-white border border-zinc-200" alt="" />
                              ) : (
                                 <div className="w-8 h-8 rounded-lg bg-white border border-zinc-200 flex items-center justify-center text-[9px] font-black uppercase text-zinc-300">GH</div>
                              )}
                              <span className="text-zinc-900">{p.name}</span>
                           </div>
                           <span className="font-mono font-bold text-zinc-400 text-[10px] tabular-nums">{formatCurrency(p.amount)}</span>
                        </li>
                     ))}
                  </ul>
               </div>
               ...
               <div className="space-y-4">
                  <Label className="text-[10px] font-black text-zinc-300 uppercase tracking-[0.2em] ml-1">Move All To</Label>
                  {methods.filter(m => m.id !== methodToDelete).length > 0 ? (
                      ...
                      <div className="p-4 bg-rose-50 text-rose-800 rounded-xl border border-rose-100 text-[10px] font-bold uppercase tracking-widest flex gap-3 items-start">
                          ...
                          <div>
                              ...
                              <p className="mt-1 leading-relaxed">You must add another payment method before you can remove this one.</p>
                          </div>
                      </div>
                  )}
               </div>
            ...
            <DialogFooter className="p-6 bg-zinc-50 border-t border-zinc-100 flex justify-between">
               <Button variant="ghost" onClick={() => setIsMovePledgesOpen(false)} className="font-bold uppercase tracking-widest text-[10px] h-10 px-4">Cancel</Button>
               <Button 
