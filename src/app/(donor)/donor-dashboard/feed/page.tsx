    ...
    <div className="sticky top-[0px] z-30 bg-zinc-50/90 backdrop-blur-xl border-b border-zinc-200/50 py-4 mb-8 transition-all">
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar px-1 max-w-2xl mx-auto">
        {filters.map((type) => (
          ...
          >
            {type === 'Saved' && <BookmarkCheck className={cn("w-3.5 h-3.5", current === type ? "text-white" : "text-zinc-400")} />}
            {type}
          </button>
        ))}
      </div>
  ...
  return (
    <div className="flex items-center justify-between py-2 mt-8">
      <div className="flex items-center gap-2">
        ...
        >
          <Heart className={cn("h-5 w-5 transition-all", post.liked ? "fill-current scale-110" : "group-hover:scale-110")} strokeWidth={post.liked ? 0 : 1.5} />
        ...
        >
          <Globe className={cn("h-5 w-5 transition-all", post.prayed ? "scale-110" : "group-hover:scale-110")} strokeWidth={1.5} />
        ...
        >
          <MessageCircle className="h-5 w-5 group-hover:scale-110 transition-transform" strokeWidth={1.5} />
      ...
      <div className="flex items-center gap-1">
        <motion.button 
          ...
          className={cn(
            ...
            post.saved ? "text-zinc-900 bg-zinc-100" : "text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100"
          )}
          title={post.saved ? "Remove from bookmarks" : "Save this post"}
        >
          <Bookmark className={cn("h-4 w-4 transition-all", post.saved ? "fill-current scale-110" : "")} strokeWidth={1.5} />
        </motion.button>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Share Update</DropdownMenuLabel>
            ...
            <DropdownMenuItem onClick={() => window.open(`mailto:?subject=${encodeURIComponent(post.title || 'Update from Give Hope')}&body=${encodeURIComponent(shareText + '\n\n' + shareUrl)}`)}>
...
const CommentsSection = ({ 
  ...
  return (
    ...
    >
      <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-6">Discussion ({comments.length})</h4>
        ...
        {comments.map((comment) => (
          <div key={comment.id} className="group">
            <div className="flex gap-4">
                ...
                <AvatarFallback className="bg-white text-zinc-700 text-xs font-bold border border-zinc-100">{comment.author[0]}</AvatarFallback>
                </Avatar>
                <div className="flex-1 space-y-1 text-left">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-zinc-900 uppercase tracking-tight">{comment.author}</span>
                        {comment.authorTitle && (
                            <Badge variant="secondary" className="text-[9px] h-4 px-1.5 bg-zinc-200/50 text-zinc-600 font-bold uppercase tracking-widest">
                            {comment.authorTitle}
                            </Badge>
                        )}
                        </div>
                        <span className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest">{comment.time}</span>
                    </div>
                    ...
                    {comment.replies.map(reply => (
                        <div key={reply.id} className="flex gap-3 text-left">
                            <Avatar className="h-7 w-7 border border-white shadow-sm mt-1">
                                ...
                                <AvatarFallback className="bg-white text-zinc-700 text-[10px] font-bold border border-zinc-100">{reply.author[0]}</AvatarFallback>
                            </Avatar>
                            <div className="flex-1 space-y-1">
                                <div className="flex items-center gap-2">
                                    <span className="text-sm font-bold text-zinc-900 uppercase tracking-tight">{reply.author}</span>
                                    {reply.authorTitle && (
                                        <Badge variant="secondary" className="text-[9px] h-4 px-1.5 bg-zinc-900 text-white font-bold uppercase tracking-widest">
                                            {reply.authorTitle}
                                        </Badge>
                                    )}
                                    <span className="text-[9px] font-bold text-zinc-300 uppercase tracking-widest">• {reply.time}</span>
                                </div>
                                <p className="text-sm text-zinc-600 leading-relaxed font-medium">{reply.text}</p>
                            </div>
                        </div>
                    ))}
                </div>
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="relative group cursor-pointer">
            <Avatar className="h-10 w-10 border border-zinc-100 shadow-sm transition-transform group-hover:scale-105">
              ...
              <AvatarFallback className="bg-zinc-100 font-bold text-zinc-600 uppercase">{post.workerName[0]}</AvatarFallback>
            </Avatar>
          </div>
          <div className="text-left">
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-zinc-900 text-sm leading-none cursor-pointer hover:underline decoration-2 decoration-zinc-200 underline-offset-4 uppercase tracking-tight">{post.workerName}</h3>
              <button className="text-[10px] font-black uppercase tracking-widest text-zinc-400 hover:text-zinc-900 transition-colors">Follow</button>
            </div>
        ...
        <div className="flex items-center gap-2">
           <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="text-zinc-300 hover:text-zinc-600 hover:bg-transparent -mr-2">
                <MoreHorizontal className="h-5 w-5" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-40">
              <DropdownMenuItem className="text-[10px] font-bold uppercase tracking-widest">Mute Updates</DropdownMenuItem>
              <DropdownMenuItem className="text-[10px] font-bold uppercase tracking-widest text-rose-600">Report</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem className="text-[10px] font-bold uppercase tracking-widest">Copy Link</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
      ...
      <div className="space-y-6 text-left">
        {post.title && (
          <h2 className="text-2xl sm:text-3xl font-bold text-zinc-900 tracking-tighter leading-tight uppercase">{post.title}</h2>
        )}
        
        {/* Images - Edge to Edge look but contained */}
          {post.images && post.images.length > 0 && post.images[0] && !imageError && (
    ...
    <div className="max-w-2xl mx-auto pb-20">
      ...
      <div className="space-y-6">
        ...
        {filteredPosts.length === 0 && (
          <div className="py-32 text-center">
            ...
            <h3 className="text-xl font-bold text-zinc-900 mb-2 uppercase tracking-tighter">No posts found</h3>
            ...
            {filter === 'Saved' && (
              <Button variant="link" onClick={() => setFilter('All')} className="mt-4 text-zinc-900 font-black uppercase tracking-widest text-[10px]">
                Browse All Updates
              </Button>
            )}
          </div>
      ...
      {filteredPosts.length > 0 && (
        <div className="flex flex-col items-center py-20 space-y-4 opacity-40 hover:opacity-100 transition-opacity">
          ...
          <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest pt-2">You&apos;re all caught up</p>
        </div>
      )}
    </div>
  );
