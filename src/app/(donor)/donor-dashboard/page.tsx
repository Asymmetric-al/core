...
import { ImpactTile, DashboardSkeleton, Greeting } from '@/features/donor/components'
    ...
    <div className="space-y-8 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 sm:gap-6 pb-6 border-b border-zinc-100">
        <div className="min-w-0">
          ...
          <p className="text-zinc-400 font-bold uppercase tracking-widest text-[10px] mt-1.5">Thank you for your partnership.</p>
        </div>
        <div className="flex gap-3 w-full sm:w-auto">
          <Button variant="outline" className="flex-1 sm:flex-none h-9 rounded-lg border-zinc-100 text-zinc-500 font-bold uppercase tracking-widest text-[10px] bg-white hover:bg-zinc-50 hover:text-zinc-900 shadow-sm transition-all" asChild>
            <Link href="/donor-dashboard/history">
              <FileText className="mr-2 h-3.5 w-3.5 text-zinc-400"/> Tax Receipt
            </Link>
          </Button>
        </div>
      </div>
        ...
        <div className="lg:col-span-8">
          <div className="relative overflow-hidden rounded-xl bg-slate-900 text-white shadow-xl h-[350px] lg:h-[400px] flex flex-col justify-end group cursor-pointer border border-white/5">
            ...
            <div className="relative z-10 p-6 md:p-10 space-y-4 text-left">
              ...
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tighter text-balance leading-tight">The school year begins in Chiang Mai.</h2>
              ...
              <div className="pt-2">
                <Link href="/donor-dashboard/feed" className="inline-flex items-center px-6 py-2.5 rounded-lg bg-white text-slate-900 font-bold text-[10px] uppercase tracking-widest hover:bg-zinc-100 transition-all shadow-xl touch-target">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-zinc-50">
                {RECENT_UPDATES.map(update => (
                  <Link href="/donor-dashboard/feed" key={update.id} className="flex gap-4 p-5 hover:bg-zinc-50/50 transition-colors group touch-target">
                    <div className="shrink-0 pt-0.5">
                      {update.image ? (
                        <Image src={update.image} alt="" width={40} height={40} className="h-10 w-10 rounded-lg object-cover border border-zinc-100 shadow-sm grayscale group-hover:grayscale-0 transition-all" />
                      ) : (
              ...
              <Button variant="ghost" className="w-full h-10 text-[10px] font-bold uppercase tracking-widest text-zinc-400 bg-zinc-50/30 hover:bg-zinc-100 hover:text-zinc-900 transition-all rounded-none border-t border-zinc-50 touch-target" asChild>
                <Link href="/donor-dashboard/feed">View All Ministry Updates</Link>
              </Button>
          </Card>
        </div>
      </div>
