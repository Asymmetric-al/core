...
import { 
  CreditCard, MoreHorizontal, AlertCircle, 
  CheckCircle2, PauseCircle, PlayCircle, X, 
  ChevronRight, DollarSign, Clock, ArrowRight, 
  ShieldCheck, Wallet, ArrowRightLeft, Plus, 
  Lock, ArrowLeft, Edit2, Landmark, MapPin, Check 
} from 'lucide-react';
...
const MOCK_PLEDGES: Pledge[] = [
  { id: 'p1', recipientName: 'The Miller Family', recipientCategory: 'Missionary', recipientAvatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?fit=facearea&facepad=2&w=256&h=256&q=80', amount: 100, frequency: 'Monthly', nextChargeDate: '2024-11-01', status: 'Active', paymentMethodId: 'pm1' },
  { id: 'p2', recipientName: 'Clean Water Initiative', recipientCategory: 'Project', recipientAvatar: 'https://images.unsplash.com/photo-1509099836639-18ba1795216d?fit=crop&w=256&h=256&q=80', amount: 50, frequency: 'Monthly', nextChargeDate: '2024-11-15', status: 'Active', paymentMethodId: 'pm2' },
  { id: 'p3', recipientName: 'Dr. Sarah Smith', recipientCategory: 'Medical', recipientAvatar: 'https://images.unsplash.com/photo-1576765608535-5f04d1e3f289?fit=facearea&facepad=2&w=256&h=256&q=80', amount: 200, frequency: 'Monthly', nextChargeDate: '2024-12-01', status: 'Paused', pausedUntil: '2025-01-01', paymentMethodId: 'pm3' }
];
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 pb-24 pt-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 px-1 text-left">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-zinc-900 tracking-tight uppercase">Recurring Pledges</h1>
          <p className="text-zinc-500 mt-2 text-lg font-bold uppercase tracking-widest text-[10px]">Manage your ongoing commitments and impact.</p>
        </div>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex justify-between items-start gap-4 mb-6 text-left">
                        <div className="flex items-start gap-4">
                          <Avatar className={cn("h-14 w-14 border-2 shadow-sm", isPaused ? "border-zinc-200 grayscale" : "border-white ring-1 ring-zinc-100")}>
                            <AvatarImage src={pledge.recipientAvatar} className="object-cover" />
                            <AvatarFallback className="bg-zinc-100 text-zinc-500 font-bold">{pledge.recipientName[0]}</AvatarFallback>
                          </Avatar>
                          <div className="space-y-1">
                            <Badge variant="outline" className={cn("text-[9px] font-bold uppercase tracking-widest border-zinc-200", isPaused ? "bg-zinc-100 text-zinc-500" : "bg-zinc-900 text-white border-zinc-900")}>
                              {pledge.recipientCategory}
                            </Badge>
                            <h3 className={cn("text-lg font-bold leading-tight uppercase tracking-tight", isPaused ? "text-zinc-500" : "text-zinc-900")}>
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-zinc-400 hover:text-zinc-900 -mr-2">
                              <MoreHorizontal className="h-5 w-5" />
                            </Button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end" className="w-48">
                            {pledge.status === 'Active' ? (
                              <DropdownMenuItem onClick={() => { handleEditClick(pledge); setIsPauseOpen(true); }} className="text-[10px] font-bold uppercase tracking-widest">
                                <PauseCircle className="mr-2 h-4 w-4" /> Pause Pledge
                              </DropdownMenuItem>
                            ) : (
                              <DropdownMenuItem onClick={() => handleResumePledge(pledge.id)} className="text-emerald-600 text-[10px] font-bold uppercase tracking-widest">
                                <PlayCircle className="mr-2 h-4 w-4" /> Resume Pledge
                              </DropdownMenuItem>
                            )}
                            <DropdownMenuSeparator />
                            <DropdownMenuItem className="text-red-600 text-[10px] font-bold uppercase tracking-widest" onClick={() => handleStopPledge(pledge.id)}>
                      ...
                      <div className="flex items-end justify-between mb-6">
                        <div>
                          <div className={cn("text-3xl font-bold tabular-nums tracking-tighter", isPaused ? "text-zinc-400" : "text-zinc-900")}>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-sm">
                        ...
                        <div className="flex items-center gap-3">
                          <div className={cn("p-1.5 rounded border shadow-sm", isPaused ? "bg-zinc-50 border-zinc-200 text-zinc-400" : "bg-white border-zinc-200 text-zinc-700")}>
                            {method.type === 'card' ? <CreditCard className="h-4 w-4" /> : <Wallet className="h-4 w-4" />}
                          </div>
                          <div className="flex flex-col">
                             <span className={cn("text-[10px] font-bold uppercase tracking-tight", isPaused ? "text-zinc-400" : "text-zinc-700")}>
                        ...
                        <div className="flex items-center gap-2 text-zinc-500 bg-zinc-50 px-3 py-1.5 rounded-full border border-zinc-100 self-start sm:self-auto">
                           {isPaused ? (
                             <>
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-widest">Paused until {format(new Date(pledge.pausedUntil!), 'MMM d')}</span>
                             </>
                           ) : (
                             <>
                                <Clock className="h-3.5 w-3.5 text-emerald-500" />
                                <span className="text-[10px] font-bold uppercase tracking-widest">Next: {format(new Date(pledge.nextChargeDate), 'MMM d, yyyy')}</span>
                             </>
                           )}
                        </div>
        ...
        <DialogContent className="sm:max-w-[500px] p-0 gap-0 overflow-hidden shadow-2xl border-none rounded-2xl">
          <div className="bg-zinc-900 p-6 text-white relative overflow-hidden text-left">
             ...
             <div className="relative z-10 flex gap-4 items-center">
                ...
                <div>
                   <DialogTitle className="text-xl font-bold text-white uppercase tracking-tight">{editingPledge?.recipientName}</DialogTitle>
                   <DialogDescription className="text-zinc-400 text-[10px] font-bold uppercase tracking-[0.2em] mt-1">Update your commitment</DialogDescription>
                </div>
             </div>
             <button onClick={() => setEditingPledge(null)} className="absolute top-4 right-4 text-zinc-400 hover:text-white transition-colors">
                <X className="h-5 w-5" />
             </button>
          </div>
             ...
             {!isPauseOpen ? (
                ...
                <>
                   <div className="p-6 space-y-6">
                      <div className="grid grid-cols-2 gap-5">
                         <div className="space-y-2">
                            <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Amount</Label>
                         ...
                         <div className="space-y-2">
                            <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Frequency</Label>
                            <Select value={editForm.frequency} onValueChange={(v) => setEditForm({...editForm, frequency: v})}>
                               ...
                               <SelectContent>
                                 ...
                                 <SelectItem value="Quarterly" className="text-xs font-bold uppercase">Quarterly</SelectItem>
                                 <SelectItem value="Annually" className="text-xs font-bold uppercase">Annually</SelectItem>
                               </SelectContent>
                            </Select>
                         </div>
                      </div>
                      ...
                      <div className="space-y-2">
                         <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Payment Method</Label>
                         <div 
                         ...
                         >
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Click to change method</p>
                                </div>
                            </div>
                      ...
                      <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm flex items-center justify-between">
                         <div>
                            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-900 block">Pledge Status</span>
                         ...
                         {editingPledge?.status === 'Active' ? (
                            <Button size="sm" variant="outline" onClick={() => setIsPauseOpen(true)} className="text-amber-600 hover:bg-amber-50 hover:text-amber-700 border-amber-200 font-bold uppercase tracking-widest text-[9px] h-8 rounded-lg px-3">
                               <PauseCircle className="mr-1.5 h-3.5 w-3.5" /> Pause
                            </Button>
                         ) : (
                            <Button size="sm" onClick={() => handleResumePledge(editingPledge!.id)} className="bg-emerald-600 text-white hover:bg-emerald-700 shadow-sm font-bold uppercase tracking-widest text-[9px] h-8 rounded-lg px-3">
                   ...
                   <div className="p-6 bg-white border-t border-zinc-100 flex justify-between items-center">
                      <Button variant="ghost" className="text-red-500 hover:text-red-700 hover:bg-red-50 font-bold uppercase tracking-widest text-[9px] h-9 px-3" onClick={() => handleStopPledge(editingPledge!.id)}>Stop Support</Button>
                      <div className="flex gap-3">
                         <Button variant="outline" onClick={() => setEditingPledge(null)} className="font-bold uppercase tracking-widest text-[9px] h-9 px-4 rounded-lg">Cancel</Button>
                         <Button onClick={handleSaveChanges} className="bg-zinc-900 text-white shadow-md font-bold uppercase tracking-widest text-[9px] h-9 px-4 rounded-lg">Save Changes</Button>
                      </div>
                   </div>
                </>
             ) : (
                ...
                >
                   <div className="p-6 flex-1 space-y-6">
                      <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-xl border border-amber-100">
                         ...
                         <span className="font-bold text-xs uppercase tracking-widest">Pause Pledge</span>
                      </div>
                      
                      <div className="space-y-4">
                         <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-400">How long would you like to pause?</Label>
                         <div className="grid grid-cols-2 gap-3">
                            ...
                            >
                               ...
                               {customResumeDate && <span className="text-[9px] mt-1">{customResumeDate}</span>}
                            </div>
                         </div>

                         {pauseDuration === '' && (
                            <div className="pt-2 animate-in fade-in slide-in-from-top-2">
                               <Label className="mb-2 block text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Resume Date</Label>
                               <Input 
                                  type="date" 
                         ...
                         {!customResumeDate && (
                            <p className="text-[9px] text-zinc-400 font-bold uppercase tracking-widest text-center bg-zinc-100 p-3 rounded-lg border border-zinc-200">
                               Pledge will automatically resume on <strong className="text-zinc-900">{format(addMonths(new Date(), parseInt(pauseDuration || '0')), 'MMMM d, yyyy')}</strong>.
                            </p>
                         )}
                      </div>
                   </div>
                   ...
                   <div className="p-6 bg-white border-t border-zinc-100 flex justify-between items-center">
                      <Button variant="ghost" onClick={() => setIsPauseOpen(false)} className="font-bold uppercase tracking-widest text-[9px] h-9 px-4">Back</Button>
                      <Button onClick={handlePausePledge} disabled={pauseDuration === '' && !customResumeDate} className="bg-amber-600 hover:bg-amber-700 text-white shadow-md font-bold uppercase tracking-widest text-[9px] h-9 px-6 rounded-lg transition-transform active:scale-95">
                         Confirm Pause
                      </Button>
                   </div>
                </motion.div>
      ...
      <Dialog open={!!movingPledge} onOpenChange={(open) => !open && setMovingPledge(null)}>
         <DialogContent className="sm:max-w-[550px] p-0 gap-0 overflow-hidden shadow-2xl border-none rounded-2xl max-h-[90vh] overflow-y-auto text-left">
            <div className="bg-zinc-50 p-6 border-b border-zinc-100 flex items-center justify-between sticky top-0 z-10">
               ...
               {moveView === 'add' && (
                  <Button variant="ghost" size="sm" onClick={() => setMoveView('select')} className="h-8 text-[9px] font-black uppercase tracking-widest hover:bg-zinc-100 rounded-md">
               ...
               {moveView === 'select' ? (
                        ...
                        {wallets.map(wallet => (
                           ...
                           >
                              <div className={cn(
                                 ...
                                 selectedTargetId === wallet.id ? "border-zinc-900 bg-zinc-900" : "border-zinc-300 bg-white"
                              )}>
                              ...
                              <div className="bg-white border border-zinc-200 p-1.5 rounded-lg flex items-center justify-center w-10 h-7 shadow-sm">
                                 {wallet.type === 'card' ? <CreditCard className="h-4 w-4 text-zinc-700" /> : <Landmark className="h-4 w-4 text-emerald-600" />}
                              </div>
                              
                              <div className="flex-1">
                                 <p className="text-sm font-bold text-zinc-900 uppercase tracking-tight">{wallet.brand} ••{wallet.last4}</p>
                                 <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Expires 12/26</p>
                              </div>
                           </div>
                        ))}
                     </div>
                  ...
                  <div className="space-y-6 animate-in slide-in-from-right-4 fade-in duration-300">
                     ...
                     <div className="flex p-1 bg-zinc-100 rounded-xl border border-zinc-200 shadow-inner">
                        <button
                           ...
                           className={cn(
                              ...
                              newMethodType === 'card' ? "bg-white text-zinc-900 shadow-md" : "text-zinc-400 hover:text-zinc-600"
                           )}
                        >
                        ...
                        <button
                           ...
                           className={cn(
                              "flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all flex items-center justify-center gap-2",
                              newMethodType === 'bank' ? "bg-white text-zinc-900 shadow-md" : "text-zinc-400 hover:text-zinc-600"
                           )}
                        >
                     ...
                     {newMethodType === 'card' ? (
                        ...
                        <div className="space-y-5">
                           <div className="space-y-2 text-left">
                              <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Card Number</Label>
                              <div className="relative">
                           ...
                           <div className="grid grid-cols-2 gap-4">
                              <div className="space-y-2 text-left">
                                 <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Expiry</Label>
                                 <Input 
                                    placeholder="MM/YY" 
                              ...
                              <div className="space-y-2 text-left">
                                 <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">CVC</Label>
                                 <div className="relative">
                        ...
                        <div className="space-y-5">
                           <div className="space-y-2 text-left">
                              <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Account Holder Name</Label>
                              <Input 
                           ...
                           <div className="space-y-2 text-left">
                              <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Routing Number</Label>
                              <div className="relative">
                           ...
                           <div className="space-y-2 text-left">
                              <Label className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Account Number</Label>
                              <Input 
                           ...
                           <div className="bg-zinc-100 text-zinc-600 text-[10px] font-bold uppercase tracking-widest p-4 rounded-xl border border-zinc-200">
                              <p className="leading-relaxed">By providing your bank information, you authorize Give Hope to debit your account for this pledge.</p>
                           </div>
                        </div>
                     )}
                     ...
                     <div className="space-y-4">
                        <div className="flex items-center justify-between">
                           ...
                           <div className="flex items-center gap-2">
                              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Use profile address?</span>
                              <Switch 
                           ...
                           <div className="space-y-3 animate-in slide-in-from-top-2 fade-in">
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                 ...
                                 <Select value={billingAddress.country} onValueChange={(v) => setBillingAddress({...billingAddress, country: v})}>
                                   ...
                                   <SelectContent>
                                     <SelectItem value="USA" className="text-[10px] font-bold uppercase tracking-widest">USA</SelectItem>
                                     <SelectItem value="CAN" className="text-[10px] font-bold uppercase tracking-widest">Canada</SelectItem>
                                     <SelectItem value="UK" className="text-[10px] font-bold uppercase tracking-widest">UK</SelectItem>
                                   </SelectContent>
                                 </Select>
                              </div>
                           </div>
                     ...
                     <div className="flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-300 pt-2 pb-2">
                        <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" /> Securely encrypted by Stripe
                     </div>
                  </div>
               )}
            </div>
            ...
            <div className="p-6 bg-zinc-50 border-t border-zinc-100 flex justify-between items-center sticky bottom-0 z-10">
               <Button variant="ghost" onClick={() => setMovingPledge(null)} className="font-bold uppercase tracking-widest text-[9px] h-10 px-4 rounded-lg">Cancel</Button>
               {moveView === 'select' ? (
                  <Button onClick={handleConfirmMove} disabled={!selectedTargetId} className="bg-zinc-900 text-white shadow-lg font-bold uppercase tracking-widest text-[9px] h-10 px-6 rounded-lg transition-transform active:scale-95">
                     Confirm Move
                  </Button>
               ) : (
                  <Button onClick={handleSaveNewMethod} className="bg-zinc-900 text-white shadow-lg font-bold uppercase tracking-widest text-[9px] h-10 px-6 rounded-lg transition-transform active:scale-95">
