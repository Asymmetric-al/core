...
export function GivingWidget({ workerId, raised, goal, percentRaised }: GivingWidgetProps) {
  ...
  return (
    <Card className="border-none shadow-xl shadow-slate-200/60 overflow-hidden relative bg-white ring-1 ring-slate-100 rounded-2xl">
      <div className="p-6 sm:p-8 space-y-6">
        ...
        <div className="bg-slate-100 p-1.5 rounded-xl flex relative" role="radiogroup" aria-label="Giving frequency">
          <button 
        ...
        <div className="space-y-4">
          <div 
            className={cn(
              ...
              isInputFocused ? "border-slate-900 ring-4 ring-slate-900/5" : "border-slate-200 hover:border-slate-300"
            )}
          ...
          >
            ...
            <label htmlFor="custom-amount-input" className="sr-only">Custom donation amount</label>
            <input
            ...
            <span className="absolute right-5 text-[9px] font-bold text-slate-400 pointer-events-none uppercase tracking-wider bg-slate-50 px-2 py-1 rounded" aria-hidden="true">USD</span>
          </div>

          <div className="grid grid-cols-4 gap-2" role="radiogroup" aria-label="Preset donation amounts">
        ...
        {hasGoal && (
          <div className="space-y-3 pt-2">
            <div className="flex justify-between items-end text-sm">
              ...
              <span className="text-slate-500 font-medium">{formatCurrency(raised)} <span className="text-slate-300" aria-hidden="true">/</span> {formatCurrency(goal)}</span>
            </div>
            <Progress value={percentRaised} className="h-2.5 bg-slate-100" aria-label={`${percentRaised}% of funding goal reached`} />
          </div>
        )}

        <Button size="lg" className="w-full h-14 text-lg font-bold bg-slate-900 hover:bg-slate-800 shadow-xl shadow-slate-900/20 rounded-xl transition-all hover:scale-[1.01] active:scale-[0.99]" asChild>
          <Link href={`/checkout?workerId=${workerId}&amount=${amount}&frequency=${frequency}`}>
            {frequency === 'monthly' ? `Give ${formatCurrency(amount)} Monthly` : `Give ${formatCurrency(amount)}`}
          </Link>
        </Button>

        <div className="flex items-center justify-center gap-2 text-xs text-slate-400 font-medium">
          <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" aria-hidden="true" /> Secure Payment &bull; 100% Tax Deductible
        </div>
      </div>
    </Card>
  );
