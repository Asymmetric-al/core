...
import { createWorkerMetadata } from '@/lib/seo/metadata';
import { WorkerJsonLd, BreadcrumbJsonLd } from '@/lib/seo/json-ld';
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { 
  MapPin, ArrowLeft, ShieldCheck, Rss,
...
function UpdateCard({ update, workerTitle }: { update: typeof PUBLIC_UPDATES[0]; workerTitle: string }) {
  return (
    <article className="group relative pl-8 pb-12 last:pb-0">
      <div className="absolute left-[11px] top-3 bottom-0 w-px bg-slate-100 group-last:hidden" aria-hidden="true" />
      <div className="absolute left-0 top-3 h-6 w-6 rounded-full border-4 border-white bg-slate-100 flex items-center justify-center z-10 group-hover:bg-emerald-100 group-hover:scale-110 transition-all duration-300" aria-hidden="true">
        <div className="h-1.5 w-1.5 rounded-full bg-slate-400 group-hover:bg-emerald-600 transition-colors" />
      </div>

      <div className="space-y-3">
        <div className="flex items-center gap-3">
          <time className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{update.date}</time>
          <Badge variant="secondary" className="px-2 py-0 text-[10px] bg-slate-100 text-slate-600 border-none">
            {update.type}
          </Badge>
        </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-100">
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-rose-500 cursor-pointer transition-colors">
                  <Heart className="w-3.5 h-3.5" aria-hidden="true" /> {update.likes}
                </span>
                <span className="flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-slate-600 cursor-pointer transition-colors">
                  <MessageCircle className="w-3.5 h-3.5" aria-hidden="true" /> {update.comments}
                </span>
              </div>
            </div>
          </CardContent>
    ...
    <>
      ...
      <div className="space-y-2">
        {PUBLIC_UPDATES.map((update) => (
          <UpdateCard key={update.id} update={update} workerTitle={workerTitle} />
        ))}
      </div>

      <div className="pt-8 text-center">
        <Button variant="ghost" className="text-slate-500 hover:text-slate-900 rounded-full">
          Load older updates
        </Button>
      </div>
    </>
...
export default async function WorkerProfilePage({ params }: PageProps) {
  ...
  const percentRaised = worker.goal !== null ? Math.min(100, Math.round((worker.raised / worker.goal) * 100)) : null;
      ...
      <div className="min-h-screen bg-slate-50 font-sans pt-16">
        <div className="bg-white border-b border-slate-100">
          <div className="container mx-auto px-4 h-12 flex items-center">
            <Link href="/workers" className="inline-flex items-center text-sm font-medium text-slate-500 hover:text-slate-900 transition-colors">
              <ArrowLeft className="h-4 w-4 mr-2" aria-hidden="true" /> Back to Partners
            </Link>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8 py-8 lg:py-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-start">
              ...
              <header className="space-y-6">
                <figure className="rounded-2xl overflow-hidden shadow-sm border border-slate-200 bg-white aspect-video relative group">
                  ...
                  <figcaption className="absolute bottom-5 left-5 text-white flex items-center gap-2">
                    ...
                    <span className="font-medium tracking-wide drop-shadow-sm">{worker.location}</span>
                  </figcaption>
                </figure>
                ...
                <div className="flex flex-col sm:flex-row gap-5 items-start">
                  <Avatar className="h-20 w-20 sm:h-24 sm:w-24 border-4 border-white shadow-lg -mt-14 sm:-mt-16 bg-white relative z-10 ring-1 ring-slate-100">
                    ...
                    <AvatarFallback className="text-lg font-bold bg-slate-100 text-slate-700">{worker.title.substring(0, 2)}</AvatarFallback>
                  </Avatar>
            ...
            <aside className="lg:col-span-5 xl:col-span-4 relative mt-8 lg:mt-0" aria-label="Support options">
              <div className="sticky top-24 space-y-6">
                ...
                <div className="flex gap-4 justify-center">
                  <Button variant="ghost" className="text-slate-500 hover:text-slate-900 hover:bg-white rounded-full">
                    <Share2 className="mr-2 h-4 w-4" aria-hidden="true" /> Share Profile
                  </Button>
                </div>
              </div>
            </aside>
