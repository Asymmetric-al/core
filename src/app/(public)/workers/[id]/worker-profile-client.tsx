...
const UpdateCard = ({ update, worker }: { update: typeof PUBLIC_UPDATES[0]; worker: FieldWorker }) => (
                        ...
                        <div className="p-6 sm:p-8 space-y-8">
                            ...
                            <div className="space-y-4">
                                ...
                                >
                                    ...
                                    <span className="absolute right-5 text-[9px] font-bold text-slate-400 pointer-events-none uppercase tracking-wider bg-slate-50 px-2 py-1 rounded">USD</span>
                                </div>
                            ...
                            <Button size="lg" className="w-full h-14 text-lg font-bold bg-slate-900 hover:bg-slate-800 shadow-xl shadow-slate-900/20 rounded-2xl transition-all hover:scale-[1.02] active:scale-95" asChild>
                                <Link href={`/checkout?workerId=${worker.id}&amount=${amount}&frequency=${frequency}`}>
                                    {frequency === 'monthly' ? `Give ${formatCurrency(amount)} Monthly` : `Give ${formatCurrency(amount)}`}
                                </Link>
                            </Button>

                            <div className="flex items-center justify-center gap-2 text-xs text-slate-400 font-medium">
                                <ShieldCheck className="h-3.5 w-3.5 text-emerald-500" /> Secure Payment &bull; 100% Tax Deductible
                            </div>
