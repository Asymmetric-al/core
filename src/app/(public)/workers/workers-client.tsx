...
import { Search, MapPin, Filter, Globe, ChevronDown, X, Sparkles, Activity, ArrowRight } from 'lucide-react';
      ...
      <div className="relative aspect-[4/5] rounded-2xl overflow-hidden bg-slate-900 shadow-xl shadow-slate-200/50 ring-1 ring-black/5 transition-all duration-500 ease-out group-hover:shadow-2xl group-hover:shadow-slate-300/60 group-hover:-translate-y-1 group-hover:ring-emerald-500/20">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 z-20">
          <div className="flex items-center gap-2 text-[11px] font-medium text-white/60 mb-2 uppercase tracking-wider transition-colors duration-300 group-hover:text-white/90">
            <MapPin className="h-3 w-3 flex-shrink-0 text-emerald-400 transition-transform duration-300 group-hover:scale-110" aria-hidden="true" /> 
...
export function WorkersPageClient() {
  ...
  const regions = useMemo(() => 
    ['All', ...Array.from(new Set(workers.map(w => w.location.split(', ').pop() || 'Global')))],
    [workers]
  );
  ...
  const hasActiveFilters = categoryFilter !== 'All' || regionFilter !== 'All' || searchTerm;
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="container-responsive relative z-10">
          <div className="max-w-5xl space-y-6 sm:space-y-8">
            <div className="inline-flex items-center gap-2 sm:gap-3 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full bg-white/5 border border-white/10 text-white/60 text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.2em] sm:tracking-[0.3em] backdrop-blur-xl">
              <Globe className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-400" aria-hidden="true" /> 
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-xl sm:rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 p-3 sm:p-4">
          <div className="flex flex-col lg:flex-row gap-3 items-stretch lg:items-center">
            <div className="relative flex-1">
              ...
              <label htmlFor="worker-search" className="sr-only">Search missionaries by name, region, or mission focus</label>
              <Input 
                id="worker-search"
                type="text" 
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    ...
                    <span className="truncate">{categoryFilter === 'All' ? 'Focus' : categoryFilter}</span>
                    <ChevronDown className="h-3 w-3 opacity-50 shrink-0" aria-hidden="true" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56 p-2 rounded-xl">
                  <DropdownMenuLabel className="text-xs font-semibold text-slate-500 py-2">Category</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  {categories.map(cat => (
                    <DropdownMenuCheckboxItem 
                      key={cat} 
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    ...
                    <span className="truncate">{regionFilter === 'All' ? 'Region' : regionFilter}</span>
                    <ChevronDown className="h-3 w-3 opacity-50 shrink-0" aria-hidden="true" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56 p-2 rounded-xl">
                  <DropdownMenuLabel className="text-xs font-semibold text-slate-500 py-2">Region</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  {regions.map(reg => (
                    <DropdownMenuCheckboxItem 
                      key={reg} 
      ...
      <div className="container-responsive py-16 sm:py-20 lg:py-24">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 sm:mb-12 gap-4 sm:gap-6">
          <div className="space-y-1 sm:space-y-2">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-widest">Active Partners</span>
            <p className="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight text-slate-900">
              {filteredWorkers.length} <span className="font-normal text-slate-400">missionaries</span>
            </p>
          </div>
          <div className="flex items-center gap-2 sm:gap-3 bg-slate-50 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg sm:rounded-xl border border-slate-100">
            ...
            <span className="text-xs sm:text-sm font-medium text-slate-500">Sort: Priority</span>
          </div>
        </div>
        ...
        {filteredWorkers.length > 0 ? (
          ...
          <div className="text-center py-16 sm:py-20 bg-slate-50 rounded-2xl border border-slate-100">
            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-white rounded-xl flex items-center justify-center mb-5 sm:mb-6 shadow-lg border border-slate-100 mx-auto">
              <Search className="h-5 w-5 sm:h-6 sm:w-6 text-slate-300" aria-hidden="true" />
            </div>
            ...
            <p className="text-slate-500 max-w-sm mx-auto mb-6 sm:mb-8 text-sm px-4">
              We couldn&apos;t find any partners matching your criteria. Try adjusting your filters.
            </p>
            <Button onClick={clearFilters} variant="outline" className="h-10 sm:h-11 px-5 sm:px-6 rounded-xl">
              Reset Filters
            </Button>
          </div>
        )}
      ...
      <section className="bg-slate-950 py-20 sm:py-28 lg:py-32 relative overflow-hidden text-center">
        ...
        <div className="container-responsive relative z-10 px-4">
          <Sparkles className="h-6 w-6 sm:h-8 sm:w-8 text-amber-400 mx-auto mb-6 sm:mb-8" aria-hidden="true" />
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white tracking-tight mb-4 sm:mb-6">Undirected Impact.</h2>
          <p className="text-base sm:text-lg md:text-xl text-slate-400 max-w-2xl mx-auto mb-8 sm:mb-10 lg:mb-12 font-light leading-relaxed">
            Can&apos;t decide who to support? Donate to our <strong className="text-white">Global Resilience</strong> fund. Resources are instantly routed to the highest-priority urgent needs.
          </p>
