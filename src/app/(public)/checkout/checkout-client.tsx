...
function SummaryCard({ worker, amount, frequency, coverFees, fees, total, startDate, endDate }: SummaryCardProps) {
...
function StepIndicator({ currentStep }: { currentStep: Step }) {
  ...
  return (
    <nav className="flex items-center justify-center gap-4 mb-20" aria-label="Checkout progress">
      {steps.map((s, idx) => (
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        ...
        >
          <div className="bg-slate-950 pt-24 pb-32 px-12 text-white relative overflow-hidden">
            ...
            >
              <Check className="w-12 h-12 text-slate-950" strokeWidth={3} aria-hidden="true" />
            </motion.div>
            ...
            <div className="space-y-4">
              ...
              {frequency === 'monthly' && (
                <div className="inline-flex items-center gap-3 bg-zinc-100 text-zinc-900 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest">
                  <Activity className="h-3.5 w-3.5" aria-hidden="true" /> Ongoing Monthly Support
                </div>
              )}
            </div>
                  ...
                  <header className="space-y-4">
                    ...
                    <div className="space-y-8">
                      <fieldset className="space-y-4">
                         <legend className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Contribution Frequency</legend>
                         <div className="flex p-1.5 bg-slate-50 rounded-2xl border border-slate-100" role="radiogroup">
                            <button
                      ...
                      <fieldset className="space-y-6">
                        <legend className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Support Amount</legend>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4" role="radiogroup">
                        ...
                        <div className="relative mt-8">
                          <span className="absolute left-8 top-1/2 -translate-y-1/2 text-slate-300 font-bold font-syne text-3xl" aria-hidden="true">$</span>
                            ...
                            <div className="flex items-start justify-between mb-8">
                              <div className="space-y-1">
                                <h4 className="font-black text-zinc-900 text-[10px] uppercase tracking-[0.2em] flex items-center gap-3">
                                  <CalendarDays className="w-4 h-4" aria-hidden="true" /> Support Schedule
                                </h4>
                                ...
                                >
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-3">
                                      <Label htmlFor="start-date" className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Start Date</Label>
                                      <Input 
                                        id="start-date"
                                        type="date" 
                                        value={startDate}
                                    ...
                                    <div className="space-y-3">
                                       <div className="flex items-center justify-between">
                                          <Label htmlFor="end-date" className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ending Date</Label>
                                          <Switch checked={hasEndDate} onCheckedChange={(c) => { setHasEndDate(c); if(!c) setEndDate(""); }} />
                                       </div>
                                       {hasEndDate ? (
                                          <Input id="end-date" type="date" value={endDate} min={startDate} onChange={(e) => setEndDate(e.target.value)} className="h-14 rounded-2xl bg-white border-slate-100" />
                                       ) : (
                      ...
                      <div 
                        ...
                        tabIndex={0}
                        onKeyDown={(e) => e.key === 'Enter' && setCoverFees(!coverFees)}
                      >
                        <div className={cn("h-14 w-14 rounded-2xl flex items-center justify-center transition-colors", coverFees ? "bg-white/20" : "bg-slate-50")}>
                          <Heart className={cn("h-6 w-6", coverFees ? "text-white fill-current" : "text-zinc-900")} aria-hidden="true" />
                        </div>
                ...
                >
                  ...
                  <div className="bg-slate-50 p-12 rounded-[3rem] border border-slate-100 space-y-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <div className="space-y-4">
                        <Label htmlFor="first-name" className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">First Name</Label>
                        <Input 
                          id="first-name"
                      ...
                      <div className="space-y-4">
                        <Label htmlFor="last-name" className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Last Name</Label>
                        <Input 
                          id="last-name"
                    ...
                    <div className="space-y-4">
                      <Label htmlFor="email" className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Email Address</Label>
                      <Input 
                        id="email"
                        type="email"
              ...
              {step === 'payment' && (
                ...
                >
                  ...
                  <div className="bg-slate-50 p-12 rounded-[3.5rem] border border-slate-100 space-y-10">
                    <div className="flex p-2 bg-white rounded-[2rem] border border-slate-100" role="tablist">
                      <button role="tab" aria-selected={paymentMethod === 'card'} onClick={() => setPaymentMethod('card')} className={cn("flex-1 py-4 text-[10px] font-black uppercase tracking-widest rounded-3xl transition-all", paymentMethod === 'card' ? "bg-slate-950 text-white shadow-xl" : "text-slate-400")}>Card</button>
                      <button role="tab" aria-selected={paymentMethod === 'ach'} onClick={() => setPaymentMethod('ach')} className={cn("flex-1 py-4 text-[10px] font-black uppercase tracking-widest rounded-3xl transition-all", paymentMethod === 'ach' ? "bg-slate-950 text-white shadow-xl" : "text-slate-400")}>Bank</button>
                      <button role="tab" aria-selected={paymentMethod === 'wallet'} onClick={() => setPaymentMethod('wallet')} className={cn("flex-1 py-4 text-[10px] font-black uppercase tracking-widest rounded-3xl transition-all", paymentMethod === 'wallet' ? "bg-slate-950 text-white shadow-xl" : "text-slate-400")}>Apple/Google</button>
                    </div>

                    <div className="min-h-[300px] flex flex-col justify-center" role="tabpanel">
                      <AnimatePresence mode="wait">
                        {paymentMethod === 'card' && (
                          <motion.div key="card" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-8">
                             <div className="space-y-3">
                                <Label htmlFor="card-number" className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Card Details</Label>
                                <div className="bg-white rounded-[2rem] border border-slate-100 overflow-hidden shadow-sm">
                                   <div className="relative border-b border-slate-50">
                                      <CreditCard className="absolute left-6 top-6 h-6 w-6 text-slate-300" aria-hidden="true" />
                                      <Input id="card-number" className="h-20 border-none pl-16 text-lg font-medium bg-transparent focus-visible:ring-0" placeholder="Card Number" autoComplete="cc-number" />
                                   </div>
                                   <div className="flex">
                                      <Input className="h-20 border-none border-r border-slate-50 text-lg font-medium bg-transparent focus-visible:ring-0 px-8" placeholder="MM/YY" autoComplete="cc-exp" aria-label="Expiration date" />
                                      <Input className="h-20 border-none text-lg font-medium bg-transparent focus-visible:ring-0 px-8" placeholder="CVC" autoComplete="cc-csc" aria-label="Security code" />
                                   </div>
                                </div>
                             </div>
                             <div className="grid grid-cols-2 gap-6">
                                ...
                                <div className="space-y-3">
                                   <Label htmlFor="postal-code" className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Postal Code</Label>
                                   <Input id="postal-code" placeholder="12345" className="h-16 rounded-2xl bg-white border-none shadow-sm font-medium px-6 focus:ring-4 focus:ring-zinc-900/5" autoComplete="postal-code" />
                                </div>
                             </div>
                          </motion.div>
                        )}
                        ...
                        {paymentMethod === 'ach' && (
                          <motion.div key="ach" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-12 text-center">
                             <div className="w-24 h-24 bg-zinc-100 rounded-[2rem] flex items-center justify-center mx-auto">
                                <Landmark className="h-10 w-10 text-zinc-900" aria-hidden="true" />
                             </div>
                             <div className="space-y-4">
                        ...
                        {paymentMethod === 'wallet' && (
                          <motion.div key="wallet" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="flex items-center justify-center h-full min-h-[300px]">
                             <button className="h-20 px-12 rounded-full bg-black text-white font-bold text-2xl flex items-center gap-4 hover:scale-105 transition-all shadow-2xl">
                                <Wallet className="h-8 w-8" aria-hidden="true" /> Pay with Apple Pay
                             </button>
                          </motion.div>
                        )}
                      </AnimatePresence>
                  ...
                  <div className="flex flex-col sm:flex-row gap-6">
                    ...
                    >
                      {isProcessing ? <Loader2 className="animate-spin h-8 w-8" aria-label="Processing payment" /> : `Confirm ${formatCurrency(total)}`}
                    </Button>
                  </div>
                </motion.div>
              )}
...
export function CheckoutPageClient() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-white"><Loader2 className="animate-spin h-12 w-12 text-slate-200" aria-label="Loading checkout" /></div>}>
      <CheckoutContent />
    </Suspense>
  );
}
