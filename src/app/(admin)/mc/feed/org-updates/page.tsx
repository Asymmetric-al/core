...
const RichTextEditor = dynamic(
  ...
  {
    ssr: false,
    loading: () => <div className="h-[140px] w-full bg-muted rounded-xl animate-pulse" />,
  }
)
      ...
      <SheetContent className="w-full sm:max-w-md p-0 flex flex-col">
        <SheetHeader className="p-6 pb-4 border-b">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle className="text-base font-bold">Feed Settings</SheetTitle>
          ...
          <div className="p-6 space-y-8">
            <div className="space-y-4">
              <div className="space-y-1">
                <Label className="text-xs font-semibold text-foreground">Feed Visibility</Label>
                <p className="text-xs text-muted-foreground">Who sees organization updates by default</p>
              </div>
              ...
              >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-foreground">All Donors</span>
                      <Badge variant="secondary" className="text-[9px] h-4 px-1.5 font-semibold">Default</Badge>
                    </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-sm font-medium text-foreground">Followers Only</span>
                    <p className="text-xs text-muted-foreground mt-0.5">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <Label className="text-xs font-semibold text-foreground">Notification Defaults</Label>
                <p className="text-xs text-muted-foreground">Default settings for new donors</p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 rounded-xl border">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-foreground">Email for Org Posts</p>
              ...
              <div className="p-3 rounded-xl bg-muted/50 border border-border">
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Donors can change these settings individually. Missionary notification preferences are separate and independent.
                </p>
              </div>
            </div>
          </div>
    ...
    >
      ...
      >
            ...
            <div className="flex gap-3 sm:gap-4">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                  <h3 className="font-bold text-foreground text-base sm:text-lg tracking-tight">{brandConfig.name}</h3>
                <Badge variant="secondary" className="font-semibold text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-full">
                  {post.post_type}
                </Badge>
                {post.isPinned && (
                  <Badge variant="outline" className="text-[9px] px-2 py-0.5 gap-1 rounded-full font-semibold uppercase tracking-wider">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="rounded-xl border shadow-lg p-2 min-w-[160px]">
              <DropdownMenuItem onClick={onTogglePin} className="font-medium text-xs rounded-lg py-2.5 cursor-pointer gap-2.5">
                <Pin className="h-3.5 w-3.5 text-muted-foreground" /> {post.isPinned ? 'Unpin' : 'Pin to Top'}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={onEdit} className="font-medium text-xs rounded-lg py-2.5 cursor-pointer gap-2.5">
              ...
              <DropdownMenuItem className="font-medium text-xs rounded-lg py-2.5 cursor-pointer gap-2.5">
                <ExternalLink className="h-3.5 w-3.5 text-muted-foreground" /> View Public
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={onDelete} className="font-medium text-xs rounded-lg py-2.5 text-destructive focus:text-destructive focus:bg-destructive/10 cursor-pointer gap-2.5">
            ...
            {post.media && post.media.length > 0 && (
              ...
              >
                {post.media.slice(0, 4).map((item, idx) => (
                  <div key={idx} className="relative h-24 w-24 sm:h-32 sm:w-32 rounded-xl overflow-hidden border shadow-sm">
...
function ComposeCard({
  ...
  const handleAddMedia = async () => {
    ...
    const randomImage = demoImages[Math.floor(Math.random() * demoImages.length)] ?? demoImages[0]!
    setSelectedMedia(prev => [...prev, { url: randomImage, type: 'image' }])
    setIsUploading(false)
    toast.success('Image added')
  }

  const isDisabled = isPublishing || isUploading || 
    ((!postContent || postContent === '<p></p>' || postContent === '<p><br></p>') && selectedMedia.length === 0)
    ...
    >
      <div className="p-4 sm:p-6">
        <div className="flex gap-2 sm:gap-3 flex-wrap items-center mb-4 sm:mb-6">
          {['Announcement', 'Newsletter', 'Update', 'Prayer Request'].map((type, i) => (
            <motion.div
              key={type}
                ...
                <div className="flex flex-col gap-3 w-full">
                  ...
                  <div className="flex flex-wrap items-center gap-2 w-full">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="start" className="rounded-xl border shadow-lg p-1.5 min-w-[160px]">
                        <DropdownMenuItem onClick={() => setVisibility('public')} className="font-medium text-[9px] uppercase tracking-wider rounded-lg py-2 cursor-pointer gap-2">
                          <Globe className="h-3.5 w-3.5 text-muted-foreground" /> Public
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => setVisibility('partners')} className="font-medium text-[9px] uppercase tracking-wider rounded-lg py-2 cursor-pointer gap-2">
                          <Users className="h-3.5 w-3.5 text-muted-foreground" /> Partners Only
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => setVisibility('private')} className="font-medium text-[9px] uppercase tracking-wider rounded-lg py-2 cursor-pointer gap-2">
...
export default function OrgUpdatesPage() {
  ...
  const handleSavePost = useCallback((postData: Partial<OrgPost>) => {
    if (editingPost) {
      if (postData.status === 'published') {
        ...
        setDrafts(prev => prev.map(p => p.id === editingPost.id ? { ...p, ...postData, updated_at: new Date().toISOString() } : p))
      }
    } else {
  ...
  return (
    ...
    >
        ...
        >
        <Link href="/mc/feed">
          <Button variant="outline" size="sm" className="h-9 gap-2 rounded-xl font-medium">
