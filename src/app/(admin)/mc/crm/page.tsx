...
import { 
  Plus, List, Columns, X, User, 
  MessageSquare, Paperclip, History, FileText, MoreHorizontal
} from 'lucide-react'
...
import { DataTable, DataTableResponsive, type DataTableFilterField } from '@/components/ui/data-table'
...
function DetailDrawer({ contact, onClose }: { contact: Contact, onClose: () => void }) {
          ...
          <div className="p-6 space-y-8">
            <div className="flex gap-6 items-start">
              <Avatar className="h-20 w-20 border-4 border-background shadow-sm">
                ...
                <AvatarFallback className="bg-muted text-muted-foreground font-bold text-xl">{contact.name[0]}</AvatarFallback>
              </Avatar>
              <div className="space-y-1 pt-1">
                <h2 className="text-2xl font-bold text-foreground tracking-tight">{contact.name}</h2>
                <p className="text-sm text-muted-foreground font-medium">{contact.title} at <span className="text-foreground">{contact.company}</span></p>
                <div className="flex gap-2 pt-2">
                  ...
                  <Badge variant="secondary" className="h-5 text-[10px] font-bold uppercase tracking-wider border-none bg-muted text-muted-foreground">
              ...
              {summary && (
                ...
                >
                  <div className="flex items-center gap-2 text-foreground font-bold text-[10px] uppercase tracking-[0.2em]">
                    <FileText className="h-3.5 w-3.5 text-muted-foreground" /> Highlights
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Type</span>
                      <p className="text-sm font-bold text-foreground">{summary.category}</p>
                    </div>
                    <div>
                      <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Ministry Focus</span>
                      <p className="text-xs text-muted-foreground leading-relaxed font-medium">{summary.focus}</p>
                    </div>
                    <div>
                      <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Next Step</span>
                      <p className="text-xs text-foreground font-bold">{summary.nextMove}</p>
                    </div>
                  </div>
                </motion.div>
              )}
            ...
            <Tabs defaultValue="activity">
              <TabsList className="bg-transparent h-9 p-0 gap-6 border-b border-border w-full rounded-none justify-start">
                <TabsTrigger value="activity" className="bg-transparent border-b-2 border-transparent data-[state=active]:border-foreground data-[state=active]:text-foreground rounded-none px-0 py-2 text-[11px] font-bold uppercase tracking-wider text-muted-foreground shadow-none">Activity</TabsTrigger>
                <TabsTrigger value="properties" className="bg-transparent border-b-2 border-transparent data-[state=active]:border-foreground data-[state=active]:text-foreground rounded-none px-0 py-2 text-[11px] font-bold uppercase tracking-wider text-muted-foreground shadow-none">Properties</TabsTrigger>
              </TabsList>
                ...
                <div className="bg-card p-4 rounded-xl border border-border shadow-sm space-y-3">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-muted">
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground"><Paperclip className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground"><History className="h-3.5 w-3.5" /></Button>
                    </div>
                    ...
                    <div key={act.id} className="relative group">
                      ...
                      <div className="pb-4 space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">{new Date(act.date).toLocaleDateString()}</span>
                        </div>
                        {act.description && <p className="text-xs text-muted-foreground leading-relaxed bg-card p-3 rounded-lg border border-border shadow-sm">{act.description}</p>}
              ...
              <TabsContent value="properties" className="pt-6 grid grid-cols-2 gap-8 text-left">
                <div className="space-y-6">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-bold text-foreground truncate hover:text-primary cursor-pointer">{contact.email}</p>
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">City</label>
                    <p className="text-sm font-bold text-foreground">{contact.city}</p>
                  </div>
                </div>
                <div className="space-y-6">
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Owner</label>
                    <p className="text-sm font-bold text-foreground">{contact.owner}</p>
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Tags</label>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {contact.tags.map(t => <Badge key={t} variant="secondary" className="text-[9px] px-1.5 h-4 bg-muted text-muted-foreground border-none shadow-none">{t}</Badge>)}
                    </div>
                  </div>
                </div>
              </TabsContent>
...
function KanbanView({ contacts, onSelectContact }: { contacts: Contact[], onSelectContact: (c: Contact) => void }) {
  return (
    <div className="h-full overflow-x-auto flex p-4 md:p-6 gap-4 items-start">
      {STAGES.map(stage => (
        <div key={stage} className="flex-shrink-0 w-80 flex flex-col h-full bg-muted/30 rounded-xl border border-border/50 overflow-hidden">
          <div className="p-3 bg-muted/50 border-b border-border flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{contacts.filter(c => c.stage === stage).length}</span>
          </div>
          <div className="flex-1 overflow-y-auto p-2 space-y-2">
            {contacts.filter(c => c.stage === stage).map(c => (
              <div key={c.id} onClick={() => onSelectContact(c)} className="bg-card p-3 rounded-lg border border-border shadow-sm hover:shadow-md transition-all cursor-pointer space-y-3">
                <div className="flex justify-between items-start">
                  <span className="font-bold text-foreground text-xs truncate leading-none">{c.name}</span>
                  <MoreHorizontal className="h-3.5 w-3.5 text-muted-foreground" />
                </div>
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 rounded bg-muted flex items-center justify-center text-[8px] font-bold text-muted-foreground border border-border">{c.company[0]}</div>
                  <span className="text-[10px] text-muted-foreground font-medium truncate">{c.company}</span>
                </div>
                <div className="flex items-center justify-between pt-2 border-t border-muted">
                  <span className="text-[10px] font-bold text-foreground tabular-nums">{formatCurrency(c.value)}</span>
    ...
    <div className="flex flex-col h-[calc(100vh-10rem)] bg-background overflow-hidden border border-border rounded-xl">
      ...
      <header className="h-14 bg-card border-b border-border flex items-center justify-between px-4 shrink-0 z-20 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-sm font-bold text-foreground">
            ...
            <span className="uppercase tracking-widest text-[11px]">People CRM</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-muted p-0.5 rounded-lg border border-border">
            <button 
              ...
              className={cn("p-1.5 rounded-md transition-all", view === 'table' ? "bg-card shadow-sm text-foreground" : "text-muted-foreground")}
            >
              <List className="h-4 w-4" />
            </button>
            <button 
              onClick={() => setView('kanban')}
              className={cn("p-1.5 rounded-md transition-all", view === 'kanban' ? "bg-card shadow-sm text-foreground" : "text-muted-foreground")}
            >
      ...
      <div className="flex-1 overflow-hidden p-0 relative">
        {view === 'table' ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-bold tabular-nums">{formatCurrency(contact.value)}</span>
                      </div>
                    </div>
                  )
                }
          ...
          <KanbanView contacts={MOCK_CONTACTS} onSelectContact={setSelectedContact} />
        )}
      </div>
