...
import { 
  MoreHorizontal, Phone, Mail, Users, MessageSquare, 
  CheckSquare, FileText, Clock, Calendar, Bell,
  AlertCircle, Check
} from 'lucide-react'
...
export function getTaskColumns({ onViewTask, onEditTask, onDeleteTask, onToggleComplete }: ColumnOptions): ColumnDef<Task>[] {
  return [
    {
      id: 'complete',
      header: '',
        ...
        return (
          <div className="flex items-start gap-3 min-w-0">
            <div className={cn(
              ...
              isCompleted ? 'bg-muted text-muted-foreground' : 'bg-muted/50 text-foreground'
            )}>
              ...
              {task.linked_entity && (
                <div className="flex items-center gap-1.5 mt-1">
                  <Avatar className="size-4">
                    ...
                    <AvatarFallback className="text-[8px] bg-muted">{task.linked_entity.name[0]}</AvatarFallback>
                  </Avatar>
                  <span className="text-xs text-muted-foreground truncate">{task.linked_entity.name}</span>
                </div>
              )}
            </div>
          </div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge 
            variant="outline" 
            className={cn('text-[9px] uppercase font-semibold tracking-wide px-2 py-0.5 h-5 shadow-none rounded-lg border', config.color)}
          >
            {config.label}
          </Badge>
        )
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!formatted) return <span className="text-xs text-muted-foreground">No date</span>
        
        const isCompleted = task.status === 'completed'
        return (
          <div className={cn(
            'flex items-center gap-1.5 text-xs font-medium',
            formatted.isOverdue && !isCompleted ? 'text-red-600 dark:text-red-400' : 
            formatted.isToday && !isCompleted ? 'text-amber-600 dark:text-amber-400' : 'text-muted-foreground'
          )}>
            {formatted.isOverdue && !isCompleted && <AlertCircle className="size-3" />}
        ...
        return (
          <div className="flex items-center gap-2">
            <Avatar className="size-6 border border-border">
              ...
              <AvatarFallback className="text-[9px] bg-primary text-primary-foreground">{task.assigned_to_name[0]}</AvatarFallback>
            </Avatar>
          ...
          <div className="flex justify-end">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="size-8 p-0 text-muted-foreground hover:text-foreground rounded-xl">
                  <span className="sr-only">Open menu</span>
                  <MoreHorizontal className="size-4" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-48 rounded-2xl border-border p-2">
                <DropdownMenuItem onClick={() => onViewTask(task)} className="rounded-xl px-3 py-2 text-sm">
                  View Details
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onEditTask(task)} className="rounded-xl px-3 py-2 text-sm">
                  Edit Task
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => onToggleComplete(task)} className="rounded-xl px-3 py-2 text-sm">
