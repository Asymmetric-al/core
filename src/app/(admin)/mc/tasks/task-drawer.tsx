...
import { 
  User, Calendar, Bell, Link2, MessageSquare, 
  Send, Trash2, Phone, Mail, Users, CheckSquare, FileText,
...
import type { Task, TaskType, TaskPriority, TaskStatus, StaffMember, LinkedEntity } from './types'
import { getPriorityConfig, getStatusConfig, TASK_PRIORITIES, TASK_STATUSES } from './types'
      ...
      <SheetContent className="w-full sm:max-w-xl p-0 gap-0 border-l border-border bg-background overflow-hidden flex flex-col h-full">
          ...
          <div className="h-14 bg-card border-b border-border flex items-center px-4 pr-14 shrink-0">
            <div className="flex items-center gap-3">
              <div className={cn('p-2 rounded-xl', statusConfig.color.split(' ')[0])}>
                <TypeIcon className="size-4" />
              </div>
              <span className="text-sm font-bold text-foreground uppercase tracking-wider">Task Details</span>
            </div>
          </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm" className="h-7 gap-1.5 text-xs rounded-xl">
                      <Badge variant="outline" className={cn('text-[9px] px-1.5 py-0 h-4 border rounded-md', statusConfig.color)}>
                        {statusConfig.label}
                      </Badge>
                  ...
                  <DropdownMenuContent align="start" className="rounded-2xl p-2">
                    {TASK_STATUSES.map(s => (
                      ...
                      >
                        <Badge variant="outline" className={cn('text-[9px] px-1.5 py-0 h-4 border mr-2 rounded-md', s.color)}>
                          {s.label}
                        </Badge>
                      </DropdownMenuItem>
                    ))}
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm" className="h-7 gap-1.5 text-xs rounded-xl">
                      <Badge variant="outline" className={cn('text-[9px] px-1.5 py-0 h-4 border rounded-md', priorityConfig.color)}>
                  ...
                  <DropdownMenuContent align="start" className="rounded-2xl p-2">
                    {TASK_PRIORITIES.map(p => (
                      <DropdownMenuItem 
                        ...
                        className="rounded-xl px-3 py-2"
                      >
                        <Badge variant="outline" className={cn('text-[9px] px-1.5 py-0 h-4 border mr-2 rounded-md', p.color)}>
                          {p.label}
                        </Badge>
                      </DropdownMenuItem>
                    ))}
                ...
                {task.tags.map(tagId => {
                    ...
                    return (
                      <Badge 
                        key={tagId} 
                        className={cn('text-[9px] h-5 px-2 rounded-lg border-0', tagConfig?.color || 'bg-muted text-muted-foreground')}
                      >
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <p className={cn(
                  ...
                  {task.due_time && (
                    <span className="text-muted-foreground ml-1">at {formatTime(task.due_time)}</span>
                  )}
                </p>
              </div>
                    ...
                    {staffMembers.map(staff => (
                      ...
                      >
                        <Avatar className="size-6 mr-2 border border-border">
                          ...
                          <AvatarFallback className="text-[9px]">{staff.name[0]}</AvatarFallback>
                        </Avatar>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3 p-4 rounded-2xl border border-border bg-card shadow-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-sm truncate">{task.linked_entity.name}</p>
                      <div className="flex items-center gap-2">
                        <Badge variant="secondary" className="text-[9px] h-4 px-1.5 capitalize rounded-md">
                          {task.linked_entity.type}
                        </Badge>
                        {task.linked_entity.email && (
                          <span className="text-xs text-muted-foreground truncate">{task.linked_entity.email}</span>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    {task.reminders.map(reminder => (
                      ...
                      >
                        ...
                        {reminder.sent && (
                          <Badge variant="outline" className="text-[9px] h-4 px-1.5 text-emerald-600 border-emerald-200 bg-emerald-50 dark:bg-emerald-950/30 dark:border-emerald-800 rounded-md">
                            Sent
                          </Badge>
                        )}
                      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <Avatar className="size-8 shrink-0 border border-border">
                  <AvatarFallback className="text-[10px] bg-primary text-primary-foreground">Y</AvatarFallback>
                </Avatar>
        ...
        <div className="p-4 border-t border-border bg-card shrink-0">
          <div className="flex items-center justify-between">
            <div className="text-[10px] text-muted-foreground font-medium">
              ...
              {task.completed_at && (
                <span className="ml-3 text-emerald-600 dark:text-emerald-400">Completed {formatDate(task.completed_at)}</span>
              )}
            </div>
            <Button 
              variant="ghost" 
