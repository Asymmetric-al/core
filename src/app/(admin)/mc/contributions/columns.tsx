...
import { 
  CircleCheck, XCircle, Clock, RotateCcw, AlertTriangle,
  CreditCard, Building2, FileText, Banknote, Globe, MoreHorizontal,
  Mail, Receipt, Eye, Copy, RefreshCcw
} from 'lucide-react'
...
import type { Contribution, ContributionStatus, PaymentMethod, ContributionSource } from './types'

const statusConfig: Record<ContributionStatus, { icon: typeof CircleCheck; className: string }> = {
  Succeeded: { 
    icon: CircleCheck, 
  ...
  Refunded: { 
    icon: RotateCcw, 
    className: 'bg-slate-50 text-slate-700 border-slate-200 dark:bg-slate-950/50 dark:text-slate-400 dark:border-slate-800' 
  },
  Disputed: { 
    icon: AlertTriangle, 
...
export const columns: ColumnDef<Contribution>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col">
          <span className="text-sm font-medium text-foreground">{fundName}</span>
      ...
      return (
        <div className="flex items-center gap-1.5">
          {sent ? (
            ...
            <Badge variant="outline" className="gap-1 bg-slate-50 text-slate-500 border-slate-200 text-xs">
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            ...
            <DropdownMenuItem onClick={() => navigator.clipboard.writeText(contribution.transactionId)}>
