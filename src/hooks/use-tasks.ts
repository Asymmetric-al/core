...
interface UseTasksReturn {
  ...
  updateTask: (id: string, data: Partial<TaskFormData | { sort_key: number }>) => Promise<boolean>
  moveTask: (taskId: string, newStatus: TaskStatus, newIndex: number) => Promise<boolean>
...
export function useTasks(options: UseTasksOptions = {}): UseTasksReturn {
  ...
  const createTask = useCallback(async (data: TaskFormData): Promise<Task | null> => {
    ...
    try {
      ...
      const maxSortKey = tasks.length > 0 
      ...
      if (mountedRef.current) {
        setTasks(prev => [...prev, formattedTask].sort((a, b) => a.sort_key - b.sort_key))
      }
  ...
  const updateTask = useCallback(async (id: string, data: Partial<TaskFormData | { sort_key: number }>): Promise<boolean> => {
    try {
      ...
      if ('due_date' in data && data.due_date instanceof Date) updateData.due_date = data.due_date.toISOString()
      else if ('due_date' in data) updateData.due_date = data.due_date
      
      if ('reminder_date' in data && data.reminder_date instanceof Date) updateData.reminder_date = data.reminder_date.toISOString()
      else if ('reminder_date' in data) updateData.reminder_date = data.reminder_date
  ...
  const moveTask = useCallback(async (taskId: string, newStatus: TaskStatus, newIndex: number): Promise<boolean> => {
    ...
    if (columnTasks.length === 0) {
      ...
      newSortKey = columnTasks[0].sort_key - 100
    } else if (newIndex >= columnTasks.length) {
      newSortKey = columnTasks[columnTasks.length - 1].sort_key + 100
    } else {
      const prevTask = columnTasks[newIndex - 1]
      const nextTask = columnTasks[newIndex]
      newSortKey = (prevTask.sort_key + nextTask.sort_key) / 2
    }
