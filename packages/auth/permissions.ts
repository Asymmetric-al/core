...
const STAFF_COMPATIBILITY_ROLES = new Set<UserRole>([
  "staff",
  "admin",
  "super_admin",
]);
...
export function hasRole(snapshot: RoleSnapshot, role: UserRole) {
  ...
  if (role === "admin") {
    return (
      snapshot.profileRole === "admin" || snapshot.profileRole === "super_admin"
    );
  }
  ...
  if (role === "staff") {
    return (
      ...
      (snapshot.profileRole
        ? STAFF_COMPATIBILITY_ROLES.has(snapshot.profileRole)
        : false)
    );
  }
...
export function derivePrimaryRole(snapshot: RoleSnapshot): UserRole | null {
  ...
  if (hasRole(snapshot, "staff")) return "staff";
  if (hasRole(snapshot, "missionary")) return "missionary";
  if (hasRole(snapshot, "donor")) return "donor";
