...
export function recordSSRTiming(route: string, duration: number) {
  ...
  if (duration > SSR_TIME_THRESHOLD_MS) {
    console.warn(
      `[SSR Performance] Slow render detected: ${route} took ${duration}ms (threshold: ${SSR_TIME_THRESHOLD_MS}ms)`,
    );
  }
...
export function createTimingMiddleware() {
  return async function timingMiddleware(request: NextRequest) {
    ...
    if (duration > SSR_TIME_THRESHOLD_MS) {
      console.warn(
        `[Middleware] Slow request: ${request.nextUrl.pathname} took ${duration.toFixed(2)}ms`,
      );
    }
