...
export async function validateImageLoads(
  dataUrl: string,
...
export async function getCroppedImg(
  ...
  if (
    maxOutputDimension &&
    (canvas.width > maxOutputDimension || canvas.height > maxOutputDimension)
  ) {
    const scale = Math.min(
      maxOutputDimension / canvas.width,
      maxOutputDimension / canvas.height,
    );
