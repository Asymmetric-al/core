...
import { summarizeAdminContributions } from "./service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await summarizeAdminContributions(
      supabaseAdmin,
      ctx.tenantId,
      params,
    );
    ...
    return toErrorResponse(error, "Failed to summarize contributions.");
  }
}
