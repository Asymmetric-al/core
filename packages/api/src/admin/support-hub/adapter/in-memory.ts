...
function buildInMemoryStore(): InMemoryStore {
  return {
    conversations: clone(SUPPORT_CONVERSATIONS_SEED),
    messages: clone(SUPPORT_MESSAGES_SEED),
    labels: clone(SUPPORT_LABELS_SEED),
    macros: clone(SUPPORT_MACROS_SEED),
    cannedResponses: clone(SUPPORT_CANNED_RESPONSES_SEED),
    savedViews: clone(SUPPORT_SAVED_VIEWS_SEED),
    inboxes: clone(SUPPORT_INBOXES_SEED),
    inboxSettings: clone(SUPPORT_INBOX_SETTINGS_SEED),
    teams: clone(SUPPORT_TEAMS_SEED),
    agents: clone(SUPPORT_AGENTS_SEED),
    businessHours: clone(SUPPORT_BUSINESS_HOURS_SEED),
    slaPolicies: clone(SUPPORT_SLA_POLICIES_SEED),
    signatures: clone(SUPPORT_SIGNATURES_SEED),
    automationRules: clone(SUPPORT_AUTOMATION_RULES_SEED),
    notificationPreferences: clone(SUPPORT_NOTIFICATION_PREFERENCES_SEED),
  };
}
