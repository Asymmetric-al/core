...
export function createEveWriteFileTool(
  executeWriteFile: typeof writeFile.execute = writeFile.execute,
) {
  return defineTool({
    ...writeFile,
    approval({ toolInput }) {
    ...
    async execute(rawInput, ctx) {
      ...
      if (hasBlockingSandboxFinding(scan)) {
        throw new Error(
          "Sandbox paused: forbidden sensitive material detected.",
        );
      }
      ...
      try {
        const output = await executeWriteFile(input, ctx);
...
export default createEveWriteFileTool();
