...
const contracts = [
  ...
  {
    // Portaled popup contents expose their width through className. This follows
    // the official Dialog/Popover examples and Sheet's shared Popup API; an
    // ordinary DOM parent cannot set the width of a portaled child. Height,
    // scrolling, positioning, focus and motion remain component-owned.
    pattern: "^(DialogContent|PopoverContent|SheetContent)$",
    allow: [...placement, "w-*", "min-w-*", "max-w-*"],
  },
  {
    // PostContent's richTextClassName/htmlClassName explicitly style authored
    // text, not an interactive control. Keep backgrounds, shape and effects
    // owned while permitting the loaded typography plugin's actual classes.
    pattern: "^PostContent$",
    allow: [
      ...placement,
      "typography",
...
const structuralValues = {
  ...
  dialog: [
    // Optional shared scrollable content preserves a spacing-scale inset.
    "max-h-[calc(100dvh-var(--spacing)*8)]",
    "top-[50%]",
    "left-[50%]",
    "max-w-[calc(100%-2rem)]",
    "translate-x-[-50%]",
  ...
  return scopes.flatMap((scope) => [
    {
      ...
      settings: {
        shadcn: {
          ...
          // Lucide icons and Core's unstyled icon registry expose SVG
          // presentation APIs; they do not own control appearance. Match
          // import sources, never names such as Search or Calendar. Global
          // token, arbitrary-value, inline-style and unknown-class checks stay.
          ignoreImports: [
            "^lucide-react$",
            "^@asym/ui/components/shadcn/icons(?:/index)?$",
          ],
