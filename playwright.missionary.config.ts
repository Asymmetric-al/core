...
function normalizeBaseUrl(baseUrl: string): string {
  try {
    ...
    if (
      ...
      const port =
        parsed.port || String(parsed.protocol === "https:" ? 443 : 4000);
...
export default defineConfig({
  ...
  webServer: isLocalBaseUrl
    ? {
        command:
          "node -e \"try{require('fs').rmSync('apps/missionary/.next/dev/lock',{force:true})}catch{}\" && bun run --cwd apps/missionary dev:playwright -- --port 4000 --hostname localhost",
        url: baseURL,
