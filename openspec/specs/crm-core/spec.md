...
Define the durable contract for the platform's CRM capability: Asym Postgres
owns operational identity, relationships, and permissions-sensitive CRM records
behind Asym-owned server boundaries. Mission Control remains the native staff
CRM experience. Identity linkage, local reads and writes, tenant and Legal Entity
isolation, and command audit preserve each Asym domain's authority. External
provider identifiers remain references; Twenty is retired as a product dependency.
