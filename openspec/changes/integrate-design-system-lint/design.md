...
profiles; honest discovery and cache acceptance; retiring accepted UI debt; conditional
...
- Add cross-workspace inputs/selection through existing Turborepo and CI
  ...
  invalidation.
- Upgrade from the initial published 0.1.0 package to the latest vetted
  published release after reviewing source, integrity, exports and peers.
  Remove the Bun compatibility patch only if unpatched cross-project
  component identity through relative/mixed barrels passes its regressions.
...
### Completion phase authorized on 2026-09-22

First establish the latest analyzer's actual behavior through RED/GREEN
regressions for known discovery and Base UI render ownership gaps. Preserve
all existing rule, parser and boundary coverage during compatible dependency
updates. Research peer constraints instead of forcing an unsupported linter
major or silently dropping framework rules.

Produce a fresh raw inventory by workspace, file and rule. Repair UI in
bounded surface batches using existing variants, component composition,
semantic tokens and justified runtime APIs. Run targeted raw lint after each
batch, characterize meaningful behavior before changing it, and verify
rendered states when presentation changes. Counts alone do not prioritize
risk or establish visual correctness. Prune only after a complete scan.

Final acceptance requires an empty bulk baseline, zero raw shadcn findings
and ordinary lint warnings within the applicable scope, verified permanent
exceptions, unchanged enforcement of negative fixtures, complete local and
published-head gates, and an independent requirement-by-requirement audit.
Intermediate batches and green baseline checks are progress, not completion.
