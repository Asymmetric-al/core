...
Implement the first real Eve specialist catalog and the structured shared run context that lets those
specialists collaborate without losing provenance, risk, evidence, or disagreements. This accepted change is
the implementation contract for GitHub issue #433 and implementation-plan slice 17.
...
- This change adds the declared Eve subagents, app-owned catalog and routing policy, dynamic policy resolver,
  shared-context store and tool, Supabase governance schema, dedicated budgets, audit integration, and evals.
...
A validated implementation in which all thirteen declared specialists compile and pass deterministic evals,
model and delegation policy fail closed, and tenant-scoped shared claims preserve attribution and disagreement.
The production provider path remains disabled behind the Eve release switch until the final #437 launch gate.
