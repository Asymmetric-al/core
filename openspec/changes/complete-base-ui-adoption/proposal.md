...
- Complete a requirement-by-requirement review of the current Base UI documentation index, including each applicable component, handbook, and utility contract. Track source, test, and browser evidence separately from conditional or intentionally unused features.
- Preserve Base UI state callbacks, render composition, refs, and event cancellation throughout styled wrappers; repair actual consumer naming, grouping, and field relationships.
- Adopt Base UI Input, Number Field, and stable OTP Field where Core currently supplies equivalent shared behavior, while preserving existing product values and TanStack Form validation ownership.
