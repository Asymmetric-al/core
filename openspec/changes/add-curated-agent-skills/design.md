...
The requested source list contains seven topics. Core already has strong overlap
for two, has a conflicting package-manager standard for one, and has concrete
catalog gaps for four. Canonical skills must live under `docs/ai/skills/` and
...
| Find animation opportunities | Add `find-animation-opportunities` | Existing skills review or plan current motion, and Impeccable combines opportunity finding with implementation. A separate read-only adapter finds only justified additive motion, requires rejected candidates, reuses Core tokens/primitives, and permits a no-motion verdict.               |
