...
or shared-context value MUST NOT choose or change that scope. This change MUST remain spec-only and MUST NOT
introduce live subagents, runtime code, storage, schema, tools, or evals. [VERIFIED-REPO: AGENTS.md]
...
#### Scenario: The spec package is reviewed for scope

- **GIVEN** this change is under review
- **WHEN** its files are inspected
- **THEN** they define only the specialist, delegation, shared-context, and conflict contracts
- **AND** no live subagent, runtime, Supabase, tool, or eval implementation is included
