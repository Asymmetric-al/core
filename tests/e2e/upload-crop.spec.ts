...
test.describe('Image Upload and Crop Flow', () => {
  ...
  test('should open cropper dialog when image is selected', async ({ page }) => {
  ...
  test('should close cropper and preserve original state on cancel', async ({ page }) => {
    await page.goto('/donor-dashboard/settings')
    await expect(page.getByText('Public Avatar')).toBeVisible()

    const avatarBefore = await page.locator('img[alt="Uploaded"]').first().getAttribute('src').catch(() => null)
    ...
    if (fs.existsSync(TEST_IMAGE_PATH)) {
      ...
      const avatarAfter = await page.locator('img[alt="Uploaded"]').first().getAttribute('src').catch(() => null)
  ...
  test('should show error for invalid file type', async ({ page }) => {
    ...
    await expect(page.getByText(/unsupported|invalid|please select/i)).toBeVisible({ timeout: 5000 })
    
    fs.unlinkSync(invalidFilePath)
  })

  test('ImageUpload component should have all required controls', async ({ page }) => {
    await page.goto('/donor-dashboard/settings')
    await expect(page.getByText('Public Avatar')).toBeVisible()
  ...
  test('cropper dialog should have zoom and rotation controls', async ({ page }) => {
    await page.goto('/donor-dashboard/settings')
  ...
  test('should support drag and drop', async ({ page }) => {
    ...
    if (fs.existsSync(TEST_IMAGE_PATH)) {
      const uploadArea = page.locator('[class*="relative"]').filter({ has: page.locator('input[type="file"]') }).first()
...
test.describe('Large File Upload', () => {
  test('should allow files larger than 2MB (no client-side rejection)', async ({ page }) => {
    await page.goto('/donor-dashboard/settings')
    await expect(page.getByText('Public Avatar')).toBeVisible()
  ...
  test('should only reject files over 50MB safeguard limit', async ({ page }) => {
    await page.goto('/donor-dashboard/settings')
...
test.describe('Backend Image Processing', () => {
  test('API endpoint should exist for image processing', async ({ request }) => {
