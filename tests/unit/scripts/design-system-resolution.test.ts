...
describe("Core component resolution through actual ESLint configurations", () => {
  ...
  it("documents the pinned upstream render-prop attribution limit without opening trigger contracts", async () => {
    // https://github.com/shadcn-ui/lint/issues/20: 0.1.0 attributes this styling
    // to DialogTrigger, not the rendered Button. This is a known analysis limit,
    // not evidence that component polymorphism has been correctly analyzed.
    const result = await lintFixture(
      'import { Button } from "@asym/ui/components/shadcn/button"; import { DialogTrigger } from "@asym/ui/components/shadcn/dialog"; export const Probe=()=> <DialogTrigger render={<Button/>} className="p-8"/>;',
    );
    expect(messagesFor(result)).toHaveLength(1);
