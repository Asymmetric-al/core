...
const guardProbe = hasBash ? runGuard() : null;
const canRunBashGuard = guardProbe !== null && guardProbe.status !== 127;
const itWithRunnableBashGuard = canRunBashGuard ? it : it.skip;

describe("bun version guard", () => {
  itWithRunnableBashGuard(
