...
describe("duplicate JSX extraction contracts", () => {
  it("keeps Email Studio and PDF Studio chrome on the shared studio components", () => {
    ...
    for (const source of [email, pdf]) {
      expect(source).toContain('from "@/components/studio/studio-chrome"');
      expect(source).toContain("<StudioTemplateBreadcrumb");
      expect(source).toContain("<StudioPreviewDeviceToggle");
      expect(source).toContain("<StudioSaveButton");
      expect(source).toContain("<StudioExportedHtmlPreview");
    }
  });
