...
describe("admin/contribution-shared/filters", () => {
  ...
  it("evaluates the parameterized state filters against shared fields", () => {
    const row = {
      shared: sharedFields({
  ...
  it("flags designation issues from the designation set context", () => {
    const withIssue = {
      shared: sharedFields(),
  ...
  it("returns the same gifts for Hub and CRM rows built from the same data", () => {
    ...
    for (const filterSet of filters) {
      ...
      // The same gift matches (or not) identically on both surfaces.
