...
describe("supportBusinessHoursSchema HTML clock values", () => {
  ...
  it("accepts legacy date-only holiday values", () => {
    const parsed = supportBusinessHoursSchema.parse({
      id: "hours_1",
      tenantId: "tenant_1",
      name: "Office",
    ...
    expect(parsed.holidays[0]?.date).toBe("2026-11-26");
  });
...
describe("shared holiday date schema ownership", () => {
  it("uses the same date validator for collection and save inputs", () => {
    const schemaSource = readFileSync(schemaPath, "utf8");
    const apiSchemasPath = fileURLToPath(
      new URL(
        "../../../../packages/api/src/admin/support-hub/schemas.ts",
    ...
    expect(schemaSource).toContain("export const supportHolidayDateSchema");
