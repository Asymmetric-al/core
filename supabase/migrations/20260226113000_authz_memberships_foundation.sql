...
BEGIN
  ...
  ELSIF role_from_metadata IN ('staff', 'admin', 'super_admin') THEN
    membership_role := 'staff';
    membership_staff_role := 'member_care';
  ELSE
...
CREATE POLICY "authz staff tenant update audit_logs"
  ON public.audit_logs
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY "authz staff tenant delete audit_logs"
  ON public.audit_logs
  FOR DELETE
  TO authenticated
  USING (
