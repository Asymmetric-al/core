...
export type OperationCategory = "correction" | "receipt" | "crm" | "provider";
...
export const OPERATION_DEFINITIONS: Record<string, OperationDefinition> = {
  ...
  resend_receipt: {
    ...
    fields: [],
    buildPayload: ({ stagedGiftId }) => ({ stagedGiftId }),
  },
  approve_staged_gift: {
...
export function ContributionOperationShell({
  ...
  const validationMessage = (() => {
    ...
    if (operation.fields.includes("fundId") && !values.fundId?.trim()) {
      return "Enter the destination fund.";
    }
    if (operation.requiresReason && !values.reason.trim()) {
      return "A reason is required for this operation.";
    }
    if (operation.requiresConfirmation && !values.confirmed) {
      return "Confirm the change to continue.";
    }
    return null;
  })();

  const handleSubmit = async () => {
    if (!donationId || validationMessage) {
      return;
    }
        ...
        {!blocked &&
          ...
          phase.name !== "submitting" && (
            <div className="space-y-4">
              ...
              {operation.riskCopy && (
                <p className="rounded-lg border border-border bg-amber-50/50 p-3 text-xs leading-relaxed text-muted-foreground dark:bg-amber-950/20">
                  {operation.riskCopy}
                </p>
              )}
              ...
              {operation.fields.includes("fundId") && (
                <div className="space-y-1.5">
                  <Label htmlFor={fundId}>Destination fund ID</Label>
                  <Input
                    id={fundId}
                    value={values.fundId ?? ""}
                    onChange={(event) =>
