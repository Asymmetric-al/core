import { ConditionalRuleSchema, DocumentAssetImageAttributesSchema, DocumentAssetReferenceSchema, DocumentHeaderFooterSettingsSchema, DocumentPageBreakAttributesSchema, DocumentPageBreakNodeSchema, DocumentPlaceholderSchema, RepeaterBindingSchema, TableBindingSchema, coreVariableRegistry, createVariableResolver, evaluateConditionalRule, resolveRepeaterItems, resolveTableRows } from "@asym/pdf-template-schema";
...
function normalizeAssetImageAttributes(input) {
	const parseResult = DocumentAssetImageAttributesSchema.safeParse(stripNullValues(input));
...
function readAsset(value) {
	...
	try {
		...
		const parseResult = DocumentAssetReferenceSchema.safeParse(parsed);
		return parseResult.success ? parseResult.data : void 0;
	} catch {
		return;
	}
...
function getHeaderFooterPreview(input = {}) {
	const result = DocumentHeaderFooterSettingsSchema.safeParse(input);
...
function isValidDocumentHeaderFooterSettings(value) {
	return DocumentHeaderFooterSettingsSchema.safeParse(value).success;
}
//#endregion
...
function getPageBreakPreview(input = {}) {
	const result = DocumentPageBreakAttributesSchema.safeParse(input);
	if (!result.success) return {
		diagnostics: [{
...
function isValidDocumentPageBreakNode(value) {
	return DocumentPageBreakNodeSchema.safeParse(value).success;
}
function normalizePageBreakAttributes(input) {
	...
	const result = DocumentPageBreakAttributesSchema.safeParse(source);
	return result.success ? result.data : void 0;
}
...
function getDocumentPlaceholderPreview(input) {
	...
	if (input.placeholder !== void 0 && input.placeholder !== null) {
		const parseResult = DocumentPlaceholderSchema.safeParse(input.placeholder);
		return {
			diagnostics: [{
...
function isValidDocumentPlaceholder(value) {
	return DocumentPlaceholderSchema.safeParse(value).success;
}
...
function readPlaceholder(value) {
	...
	const result = DocumentPlaceholderSchema.safeParse(parsedValue);
	return result.success ? result.data : void 0;
}
...
function getVariableChipPreview(input) {
	...
	const resolvedValue = createVariableResolver({
		...input,
		registry
	}).resolve({
...
function isKnownVariableChipKey(key, registry = coreVariableRegistry) {
