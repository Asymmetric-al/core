...
const DocumentAssetRoleSchema = z.enum([
	"logo",
	"image",
	"signature",
	"font",
...
const DocumentAssetUrlClassificationSchema = z.enum([
	"render_safe_public",
	"signed_render_url",
	"browser_blob",
	"data_url",
...
const DocumentAssetAlignmentSchema = z.enum([
	"left",
	"center",
	"right",
	"full_width"
]);
const DocumentAssetSourceMetadataSchema = z.object({
...
const DocumentAssetReferenceSchema = z.object({
	id: IdentifierSchema,
...
const DocumentAssetImageAttributesSchema = z.object({
...
const DocumentAssetImageNodeSchema = z.object({
	type: z.literal("assetImage"),
...
const VariableGroupSchema = z.enum([
	"organization",
	"recipient",
	"donation",
	"document",
...
const FallbackBehaviorSchema = z.discriminatedUnion("mode", [
...
const VariableDefinitionBaseSchema = z.object({
...
const RepeaterBindingSchema = z.object({
	id: IdentifierSchema,
	sourcePath: DataPathSchema,
...
const TableColumnWidthSchema = z.string().regex(/^(?:auto|(?:0|[1-9]\d*)(?:\.\d+)?(?:%|px|pt|in|cm|mm))$/, "Table column width must be auto or a positive CSS length/percentage.");
const TableColumnBindingSchema = z.object({
...
const TableGroupingBindingSchema = z.object({
	fieldPath: DataPathSchema,
	label: NonEmptyStringSchema.optional()
}).strict();
const TableTotalBindingSchema = z.object({
...
const TableBindingSchema = z.object({
	...
	avoidRowSplit: z.boolean().default(true),
...
const HeaderFooterPlacementSchema = z.enum(["header", "footer"]);
const HeaderFooterScopeSchema = z.enum(["first_page", "repeating"]);
const HeaderFooterAlignmentSchema = z.enum([
	"left",
	"center",
	"right"
]);
const HeaderFooterTextTokenSchema = z.object({
	kind: z.literal("text"),
	text: z.string().min(1)
}).strict();
const HeaderFooterDocumentTitleTokenSchema = z.object({ kind: z.literal("document_title") }).strict();
const HeaderFooterOrganizationFooterTokenSchema = z.object({
	kind: z.literal("organization_footer"),
	fallbackText: z.string().optional()
}).strict();
const HeaderFooterPageNumberTokenSchema = z.object({ kind: z.literal("page_number") }).strict();
const HeaderFooterTotalPagesTokenSchema = z.object({ kind: z.literal("total_pages") }).strict();
const HeaderFooterContentTokenSchema = z.discriminatedUnion("kind", [
...
const HeaderFooterRegionSchema = z.object({
	id: IdentifierSchema.optional(),
	...
	content: z.array(HeaderFooterContentTokenSchema).default([])
...
const DocumentHeaderFooterSettingsSchema = z.object({ regions: z.array(HeaderFooterRegionSchema).default([]) }).strict().default(defaultDocumentHeaderFooterSettings).superRefine((settings, context) => {
...
const PdfDocumentMetadataSchema = z.object({
...
const PdfDocumentProfileSchema = z.enum([
	"PDF/A-1a",
	"PDF/A-1a+PDF/UA-1",
	"PDF/A-1b",
	"PDF/A-2a",
...
const PdfDocumentProfileOptionsSchema = z.object({ profile: PdfDocumentProfileSchema.optional() }).strict().default({});
const PdfDocumentOutputSettingsSchema = z.object({
...
const DocumentPlaceholderKindSchema = z.enum([
	"text_field",
	"checkbox",
	"signature",
	"initials",
...
const DocumentPlaceholderSignerRoleSchema = z.enum([
	"organization",
	"recipient",
	"witness",
	"staff",
...
const QrPlaceholderPayloadSchema = z.discriminatedUnion("type", [
	z.object({
		type: z.literal("url"),
...
const BaseDocumentPlaceholderSchema = z.object({
	id: IdentifierSchema,
...
const TextFieldPlaceholderSchema = BaseDocumentPlaceholderSchema.extend({
	...
	placeholderText: z.string().trim().min(1).optional(),
...
const DocumentPlaceholderSchema = z.discriminatedUnion("kind", [
...
const SummaryCalculationPrecisionSchema = z.object({
	scale: z.number().int().nonnegative().optional(),
	roundingMode: z.literal("half_away_from_zero").optional()
}).strict();
const SummaryDisplayLabelsSchema = z.record(z.string(), NonEmptyStringSchema).optional();
const TotalContributionsCalculationReferenceSchema = z.object({
	type: z.literal("total_contributions"),
	sourcePath: DataPathSchema,
	amountPath: DataPathSchema,
	label: NonEmptyStringSchema.optional()
}).strict();
const TableTotalCalculationReferenceSchema = z.object({
	type: z.literal("table_total"),
	tableBindingId: IdentifierSchema,
	columnKey: IdentifierSchema,
	operation: z.enum(["sum", "count"]).optional(),
	label: NonEmptyStringSchema.optional()
}).strict();
const InvoiceTotalsCalculationReferenceSchema = z.object({
	type: z.literal("invoice_totals"),
...
const FinancialReportTotalsCalculationReferenceSchema = z.object({
	type: z.literal("financial_report_totals"),
	sourcePath: DataPathSchema,
	amountPath: DataPathSchema,
	categoryPath: DataPathSchema,
	incomeCategories: z.array(NonEmptyStringSchema).min(1),
	expenseCategories: z.array(NonEmptyStringSchema).min(1),
	fields: z.array(z.enum([
		"income",
		"expense",
		"net"
...
const GroupedSubtotalsCalculationReferenceSchema = z.object({
	type: z.literal("grouped_subtotals"),
	sourcePath: DataPathSchema,
...
const GrandTotalCalculationReferenceSchema = z.object({
	type: z.literal("grand_total"),
	sourcePath: DataPathSchema,
	groupPath: DataPathSchema,
	valuePath: DataPathSchema,
	label: NonEmptyStringSchema.optional()
}).strict();
const SummaryCalculationReferenceSchema = z.discriminatedUnion("type", [
...
const SummaryBlockBindingSchema = z.object({
	id: IdentifierSchema,
	title: NonEmptyStringSchema.optional(),
...
const DocumentBrandSourceSchema = z.enum([
	"system_default",
	"tenant_default",
	"template_override"
]);
const DocumentBrandFieldSchema = z.enum([
	"organization_name",
	"logo_asset",
	"primary_color",
	"accent_color",
...
const DocumentBrandingMetadataSchema = z.object({
	source: DocumentBrandSourceSchema.default("system_default"),
	tenantBrandId: IdentifierSchema.optional(),
	overriddenFields: z.array(DocumentBrandFieldSchema).default([])
}).strict().default({
...
const DocumentThemeColorsSchema = z.object({
...
const DocumentThemeFontsSchema = z.object({
	...
	fallback: z.array(DocumentThemeFontFamilySchema).default([...defaultThemeFonts.fallback])
}).strict().default(defaultThemeFonts);
const DocumentThemeOrganizationSchema = z.object({
	name: NonEmptyStringSchema.optional(),
	legalName: NonEmptyStringSchema.optional(),
	websiteUrl: UrlSchema.optional()
}).strict().default({});
const DocumentReceiptDefaultsSchema = z.object({
...
const DocumentEngineSchema = z.enum(["asym_pdf_document_builder", "unlayer"]);
...
const CustomPageSizeSchema = z.object({
	width: PositiveIntegerSchema,
	height: PositiveIntegerSchema,
	unit: PageUnitSchema.default("in")
}).strict();
const DocumentPageSettingsSchema = z.object({
...
const DocumentContentNodeSchema = z.lazy(() => z.object({
	type: NonEmptyStringSchema,
	attrs: z.record(z.string(), JsonValueSchema).optional(),
...
const DocumentTemplateV1Schema = z.object({
	version: z.literal(1),
	id: IdentifierSchema,
	...
	summaryBlockBindings: z.array(SummaryBlockBindingSchema).default([]),
	placeholderBindings: z.array(DocumentPlaceholderSchema).default([]),
...
const TemplateLifecycleStatusSchema = z.enum([
	"draft",
	"published",
	"archived"
]);
const TemplateLifecycleEventSchema = z.enum([
	"created",
	"updated",
	"published",
	"archived",
...
const TemplateLifecycleCheckStatusSchema = z.enum([
	"not_run",
	"passed",
	"warning",
	"failed"
]);
const TemplateLifecycleCheckSchema = z.object({
...
const TemplateLifecycleEngineMetadataSchema = z.object({
	engine: DocumentEngineSchema.default("asym_pdf_document_builder"),
...
const TemplateLifecycleChangelogEntrySchema = z.object({
	id: IdentifierSchema,
	event: TemplateLifecycleEventSchema,
	at: IsoDateTimeSchema,
	actorId: IdentifierSchema.optional(),
	message: NonEmptyStringSchema.optional(),
	changes: z.array(NonEmptyStringSchema).default([]),
	metadata: JsonObjectSchema.default({})
}).strict();
const TemplateLifecycleRecordV1Schema = z.object({
	version: z.literal(1),
	...
	changelog: z.array(TemplateLifecycleChangelogEntrySchema).default([])
...
const PublishedTemplateSnapshotV1Schema = z.object({
	version: z.literal(1),
	id: IdentifierSchema,
	templateId: IdentifierSchema,
...
const RendererSchema = z.enum([
	"docraptor",
	"browser",
	"local"
]);
const RenderWarningSchema = z.object({
...
const DocumentArtifactLocationSchema = z.discriminatedUnion("type", [
	z.object({
	...
	z.object({
		type: z.literal("adapter_reference"),
...
const RenderTimingSchema = z.object({
	queuedAt: IsoDateTimeSchema.optional(),
	startedAt: IsoDateTimeSchema.optional(),
	completedAt: IsoDateTimeSchema.optional(),
	durationMs: NonNegativeNumberSchema.optional()
}).strict().default({});
const DocRaptorRenderMetadataSchema = z.object({
	mode: z.enum(["test", "production"]),
	media: z.enum(["print", "screen"]).default("print"),
	requestUrl: UrlSchema.optional(),
	requestMethod: z.enum(["POST", "GET"]).optional(),
...
const RenderMetadataV1Schema = z.object({
	version: z.literal(1),
	id: IdentifierSchema,
...
const RenderJobV1Schema = z.object({
	version: z.literal(1),
	id: IdentifierSchema,
	tenantId: IdentifierSchema.optional(),
	actorId: IdentifierSchema.optional(),
...
const BatchRunV1Schema = z.object({
	version: z.literal(1),
	id: IdentifierSchema,
	templateId: IdentifierSchema,
	templateVersion: z.number().int().positive(),
...
//#endregion
//#region src/batch.ts
const BatchGenerationStatusSchema = z.enum([
	"draft",
	"queued",
	"running",
	"completed",
...
const BatchDocumentJobStatusSchema = z.enum([
	"queued",
	"running",
	"succeeded",
	"failed",
...
const BatchDatasetReferenceSchema = z.object({
	type: z.enum([
		"sample_fixture",
		"query",
		"static_records",
...
const BatchRecipientReferenceSchema = z.object({
	id: IdentifierSchema,
	label: NonEmptyStringSchema.optional(),
	dataSnapshotHash: DataSnapshotHashSchema.optional(),
	metadata: JsonObjectSchema.default({})
}).strict();
const BatchFailureReasonSchema = z.object({
	code: NonEmptyStringSchema,
	message: NonEmptyStringSchema,
	retryable: z.boolean().default(false),
	path: z.array(NonEmptyStringSchema).default([]),
...
const BatchSafetyPreflightDiagnosticSchema = z.object({
	code: NonEmptyStringSchema,
	severity: z.enum([
		"info",
		"warning",
...
const BatchSafetyPreflightResultSchema = z.object({
	ok: z.boolean(),
	diagnostics: z.array(BatchSafetyPreflightDiagnosticSchema).default([])
}).strict().superRefine((result, context) => {
...
const BatchGenerationDefinitionV1Schema = z.object({
	version: z.literal(1),
	id: IdentifierSchema,
	...
	recipients: z.array(BatchRecipientReferenceSchema).min(1),
	dataSnapshotHash: DataSnapshotHashSchema,
	mode: z.literal("batch").default("batch"),
	renderer: RendererSchema.default("docraptor"),
...
const BatchDocumentJobV1Schema = z.object({
	version: z.literal(1),
	id: IdentifierSchema,
...
const BatchProgressSummarySchema = z.object({
	total: NonNegativeIntegerSchema,
...
const BatchGenerationRunV1Schema = z.object({
	version: z.literal(1),
	id: IdentifierSchema,
	templateId: IdentifierSchema,
	templateVersion: PositiveIntegerSchema,
...
const BatchResultManifestJobSchema = z.object({
	id: IdentifierSchema,
...
const BatchResultManifestV1Schema = z.object({
	version: z.literal(1),
	batchId: IdentifierSchema,
	templateId: IdentifierSchema,
	templateVersion: PositiveIntegerSchema,
	...
	progress: BatchProgressSummarySchema,
	jobs: z.array(BatchResultManifestJobSchema).default([]),
	failures: z.array(BatchFailureReasonSchema).default([]),
	artifacts: z.array(DocumentArtifactSchema).default([])
}).strict();
const BatchDownloadManifestV1Schema = z.object({
	version: z.literal(1),
	batchId: IdentifierSchema,
	kind: z.literal("zip"),
	generatedAt: IsoDateTimeSchema,
...
function createBatchGenerationDefinition(input) {
	...
	const recipients = z.array(BatchRecipientReferenceSchema).min(1).parse(input.recipients);
...
function createBatchGenerationRun(input) {
	const definition = BatchGenerationDefinitionV1Schema.parse(input.definition);
	const jobs = z.array(BatchDocumentJobV1Schema).parse(input.jobs);
...
function cancelBatchGenerationRun(input) {
	...
	const canceledJobs = z.array(BatchDocumentJobV1Schema).parse(input.jobs).map((job) => {
...
const LegacyPdfTemplateArtifactSchema = z.object({
	artifactId: IdentifierSchema,
	location: DocumentArtifactLocationSchema.optional(),
	createdAt: IsoDateTimeSchema.optional(),
	metadata: JsonObjectSchema.default({})
}).strict();
const LegacyPdfTemplateReferenceSchema = z.object({
	engine: z.literal("unlayer"),
	...
	pdfArtifacts: z.array(LegacyPdfTemplateArtifactSchema).default([]),
	metadata: JsonObjectSchema.default({})
}).strict();
const UnlayerUnsupportedFeatureCodeSchema = z.enum([
	"custom_javascript",
	"custom_css",
	"dynamic_data_source",
	"ecommerce_block",
...
const UnlayerMigrationSeveritySchema = z.enum([
	"info",
	"warning",
	"error"
]);
const UnlayerUnsupportedFeatureSchema = z.object({
...
const UnlayerMigrationStrategySchema = z.enum([
	"manual_rebuild_with_report",
	"html_import_report_only",
	"side_by_side_review"
]);
const UnlayerMigrationStatusSchema = z.enum([
	"ready_for_manual_rebuild",
	"needs_manual_rebuild",
	"blocked",
	"completed"
]);
const UnlayerMigrationReportV1Schema = z.object({
	version: z.literal(1).default(1),
	id: IdentifierSchema,
	legacyTemplate: LegacyPdfTemplateReferenceSchema,
	targetEngine: z.literal("asym_pdf_document_builder").default("asym_pdf_document_builder"),
	strategy: UnlayerMigrationStrategySchema.default("manual_rebuild_with_report"),
	status: UnlayerMigrationStatusSchema,
	unsupportedFeatures: z.array(UnlayerUnsupportedFeatureSchema).default([]),
...
function createUnlayerMigrationReport(input) {
	...
	const unsupportedFeatures = z.array(UnlayerUnsupportedFeatureSchema).parse(input.unsupportedFeatures ?? []);
...
const UnlayerHtmlImportRequestSchema = z.object({
	legacyTemplate: LegacyPdfTemplateReferenceSchema,
	html: NonEmptyStringSchema,
	mode: z.literal("report_only").default("report_only"),
	claimsLosslessConversion: z.literal(false).default(false),
	metadata: JsonObjectSchema.default({})
}).strict();
const PdfBuilderFeatureFlagRolloutModeSchema = z.enum([
	"legacy_only",
	"native_preview",
...
const PdfBuilderFeatureFlagContractSchema = z.object({
	flagName: NonEmptyStringSchema,
	enabled: z.boolean(),
	tenantId: IdentifierSchema.optional(),
	rolloutMode: PdfBuilderFeatureFlagRolloutModeSchema.default("legacy_only"),
	fallbackEngine: z.literal("unlayer").default("unlayer"),
	metadata: JsonObjectSchema.default({})
}).strict();
const PdfTemplateEngineSelectionReasonSchema = z.enum([
	"feature_flag_disabled",
	"legacy_template_pass_through",
	"native_feature_enabled"
]);
const PdfTemplateEditorSelectionSchema = z.enum(["legacy_unlayer", "native_pdf_builder"]);
const PdfTemplateEngineSelectionResultSchema = z.object({
...
const UnlayerComparisonDifferenceCodeSchema = z.enum([
	"asset_mismatch",
	"content_mismatch",
	"layout_mismatch",
	"metadata_mismatch",
...
const UnlayerComparisonDifferenceSchema = z.object({
...
const UnlayerSideBySideComparisonStatusSchema = z.enum([
	"matches",
	"differences_found",
	"blocked"
]);
const UnlayerSideBySideComparisonRequestSchema = z.object({
	legacyTemplate: LegacyPdfTemplateReferenceSchema,
	nativeTemplateId: IdentifierSchema,
	metadata: JsonObjectSchema.default({})
}).strict();
const UnlayerSideBySideComparisonResultSchema = z.object({
	legacyTemplateId: IdentifierSchema,
	nativeTemplateId: IdentifierSchema,
	status: UnlayerSideBySideComparisonStatusSchema,
	differences: z.array(UnlayerComparisonDifferenceSchema).default([]),
	metadata: JsonObjectSchema.default({})
}).strict();
//#endregion
//#region src/page-flow.ts
const DocumentPageBreakAttributesSchema = z.object({
	id: IdentifierSchema.optional(),
	label: NonEmptyStringSchema.optional()
}).strict();
const DocumentPageBreakNodeSchema = z.object({
	type: z.literal("pageBreak"),
	attrs: DocumentPageBreakAttributesSchema.default({})
}).strict();
const PageFlowControlAttributesSchema = z.object({
	keepTogether: z.boolean().optional(),
	startOnNewPage: z.boolean().optional(),
	avoidBreakAfter: z.boolean().optional(),
	avoidRowSplit: z.boolean().optional()
}).strict();
//#endregion
...
const PdfSecurityActorSchema = z.object({
	type: z.enum(["user", "system"]),
	id: IdentifierSchema.optional()
...
const PdfSecurityPermissionSchema = z.enum([
	"template.edit",
	"template.publish",
	"render.preview",
	"render.production",
...
const PdfSecurityActionSchema = z.enum([
	"edit_template",
	"publish_template",
	"preview_render",
	"production_render",
...
const PdfSecurityContextSchema = z.object({
	tenantId: IdentifierSchema,
	actor: PdfSecurityActorSchema,
	permissions: z.array(PdfSecurityPermissionSchema).default([])
}).strict();
const PdfSecurityResourceTypeSchema = z.enum([
	"template",
	"asset",
	"render_artifact",
	"batch",
...
const PdfSecurityResourceSchema = z.object({
	type: PdfSecurityResourceTypeSchema,
	id: IdentifierSchema.optional(),
	tenantId: IdentifierSchema
}).strict();
const PdfAuthorizationReasonCodeSchema = z.enum([
	"allowed",
	"missing_permission",
	"tenant_mismatch"
]);
const PdfAuthorizationDecisionSchema = z.object({
	ok: z.boolean(),
...
const PdfAssetAccessRequestSchema = z.object({
	tenantId: IdentifierSchema,
	actorId: IdentifierSchema.optional(),
...
const PdfSignedRenderUrlRequestSchema = z.object({
	tenantId: IdentifierSchema,
	actorId: IdentifierSchema.optional(),
	assetId: IdentifierSchema,
	purpose: z.enum(["browser_preview", "production_render"]),
	expiresInSeconds: NonNegativeIntegerSchema.optional()
}).strict();
const PdfSignedRenderUrlResultSchema = z.object({
	tenantId: IdentifierSchema,
	assetId: IdentifierSchema,
	url: UrlSchema,
...
const PdfDataClassificationSchema = z.enum([
	"public",
	"internal",
	"confidential",
	"restricted",
...
const PdfClassifiedDataPathSchema = z.object({
	path: z.array(NonEmptyStringSchema),
	classification: PdfDataClassificationSchema,
	reason: NonEmptyStringSchema
}).strict();
const PdfSecretLikeTemplateDiagnosticSchema = z.object({
	code: z.literal("secret_like_template_value"),
...
//#endregion
export { AssetReferenceSchema, AssetRoleSchema, AuditEventSchema, BatchDatasetReferenceSchema, BatchDocumentJobStatusSchema, BatchDocumentJobV1Schema, BatchDownloadManifestV1Schema, BatchFailureReasonSchema, BatchGenerationDefinitionV1Schema, BatchGenerationRunV1Schema, BatchGenerationStatusSchema, BatchProgressSummarySchema, BatchRecipientReferenceSchema, BatchResultManifestJobSchema, BatchResultManifestV1Schema, BatchRunV1Schema, BatchSafetyPreflightDiagnosticSchema, BatchSafetyPreflightResultSchema, CheckboxPlaceholderSchema, ConditionalOperatorSchema, ConditionalRuleSchema, CustomPageSizeSchema, DataBindingSchema, DataSnapshotHashSchema, DatePlaceholderSchema, DocRaptorRenderMetadataSchema, DocumentArtifactLocationSchema, DocumentArtifactSchema, DocumentAssetAlignmentSchema, DocumentAssetImageAttributesSchema, DocumentAssetImageNodeSchema, DocumentAssetReferenceSchema, DocumentAssetRoleSchema, DocumentAssetSourceMetadataSchema, DocumentAssetUrlClassificationSchema, DocumentBrandFieldSchema, DocumentBrandSourceSchema, DocumentBrandingMetadataSchema, DocumentContentNodeSchema, DocumentContentSchema, DocumentEngineSchema, DocumentHeaderFooterSettingsSchema, DocumentPageBreakAttributesSchema, DocumentPageBreakNodeSchema, DocumentPageSettingsSchema, DocumentPlaceholderKindSchema, DocumentPlaceholderSchema, DocumentPlaceholderSignerRoleSchema, DocumentReceiptDefaultsSchema, DocumentTemplateV1Schema, DocumentThemeColorSchema, DocumentThemeColorsSchema, DocumentThemeFontFamilySchema, DocumentThemeFontsSchema, DocumentThemeOrganizationSchema, DocumentThemeSchema, FallbackBehaviorSchema, FinancialReportTotalsCalculationReferenceSchema, GrandTotalCalculationReferenceSchema, GroupedSubtotalsCalculationReferenceSchema, HeaderFooterAlignmentSchema, HeaderFooterContentTokenSchema, HeaderFooterDocumentTitleTokenSchema, HeaderFooterOrganizationFooterTokenSchema, HeaderFooterPageNumberTokenSchema, HeaderFooterPlacementSchema, HeaderFooterRegionSchema, HeaderFooterScopeSchema, HeaderFooterTextTokenSchema, HeaderFooterTotalPagesTokenSchema, InitialsPlaceholderSchema, InvoiceTotalsCalculationReferenceSchema, LegacyPdfTemplateArtifactSchema, LegacyPdfTemplateReferenceSchema, PageFlowControlAttributesSchema, PageMarginsSchema, PageOrientationSchema, PageSizeSchema, PageUnitSchema, PdfAssetAccessRequestSchema, PdfAuthorizationDecisionSchema, PdfAuthorizationReasonCodeSchema, PdfBuilderFeatureFlagContractSchema, PdfBuilderFeatureFlagRolloutModeSchema, PdfClassifiedDataPathSchema, PdfDataClassificationSchema, PdfDocumentMetadataSchema, PdfDocumentOutputSettingsSchema, PdfDocumentProfileOptionsSchema, PdfDocumentProfileSchema, PdfSecretLikeTemplateDiagnosticSchema, PdfSecurityActionSchema, PdfSecurityActorSchema, PdfSecurityContextSchema, PdfSecurityPermissionSchema, PdfSecurityResourceSchema, PdfSecurityResourceTypeSchema, PdfSignedRenderUrlRequestSchema, PdfSignedRenderUrlResultSchema, PdfTemplateEditorSelectionSchema, PdfTemplateEngineSelectionReasonSchema, PdfTemplateEngineSelectionResultSchema, PrivacyClassificationSchema, PublishedTemplateSnapshotV1Schema, QrPlaceholderPayloadSchema, QrPlaceholderSchema, RegistryVariableDefinitionSchema, RenderErrorSchema, RenderJobV1Schema, RenderMetadataV1Schema, RenderModeSchema, RenderRequestSchema, RenderResultSchema, RenderTimingSchema, RenderWarningSchema, RendererSchema, RepeaterBindingSchema, SignaturePlaceholderSchema, SummaryBlockBindingSchema, SummaryCalculationPrecisionSchema, SummaryCalculationReferenceSchema, TableBindingSchema, TableColumnBindingSchema, TableGroupingBindingSchema, TableTotalBindingSchema, TableTotalCalculationReferenceSchema, TemplateCategorySchema, TemplateLifecycleChangelogEntrySchema, TemplateLifecycleCheckSchema, TemplateLifecycleCheckStatusSchema, TemplateLifecycleEngineMetadataSchema, TemplateLifecycleEventSchema, TemplateLifecycleRecordV1Schema, TemplateLifecycleStatusSchema, TextFieldPlaceholderSchema, TotalContributionsCalculationReferenceSchema, UnlayerComparisonDifferenceCodeSchema, UnlayerComparisonDifferenceSchema, UnlayerHtmlImportRequestSchema, UnlayerMigrationReportV1Schema, UnlayerMigrationSeveritySchema, UnlayerMigrationStatusSchema, UnlayerMigrationStrategySchema, UnlayerSideBySideComparisonRequestSchema, UnlayerSideBySideComparisonResultSchema, UnlayerSideBySideComparisonStatusSchema, UnlayerUnsupportedFeatureCodeSchema, UnlayerUnsupportedFeatureSchema, VariableDefinitionSchema, VariableGroupSchema, VariableReferenceSchema, VariableRegistryError, VariableValueTypeSchema, archiveTemplateLifecycle, assertProductionRenderableTemplateSnapshot, authorizePdfSecurityAction, calculateFinancialTotals, calculateGroupedTableTotals, calculateInvoiceTotals, calculateNumericAggregate, calculateTableTotals, calculateTaxDeductibleAmount, cancelBatchGenerationRun, classifyDocumentAssetUrl, classifyPdfDataPath, coreVariableDefinitions, coreVariableRegistry, createBatchDocumentJobs, createBatchDownloadManifest, createBatchGenerationDefinition, createBatchGenerationRun, createBatchResultManifest, createDataSnapshotHash, createFakePdfPermissionAdapter, createRetryBatchDocumentJob, createScopedRepeaterContext, createTemplateLifecycle, createUnlayerMigrationReport, createVariableRegistry, createVariableResolver, defaultDocumentHeaderFooterSettings, defaultVariableFormatters, duplicateTemplateLifecycle, evaluateConditionalRule, evaluateConditionalRules, findSecretLikeTemplateValues, formatVariableValue, getValueAtDataPath, isProductionRenderableTemplateSnapshot, normalizeVariableFormatterOptions, pdfTemplateSchemaBoundary, publishTemplateVersion, redactPdfSecurityLogValue, redactRenderMetadataForClient, resolveDocumentTheme, resolveRepeaterItems, resolveTableRows, resolveVariableValue, resolveVariableValues, restoreTemplateLifecycle, selectPdfTemplateEngine, stableStringifyJsonValue, starterPdfTemplateCategories, starterPdfTemplateFixtureByCategory, starterPdfTemplateFixtures, summarizeBatchProgress, transitionBatchDocumentJob, updateTemplateDraft };
