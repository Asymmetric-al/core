...
function createPdfStudioAdapter(dependencies) {
	return {
		...
		async createSignedAssetUrl(input) {
			...
			return _asym_pdf_template_schema.PdfSignedRenderUrlResultSchema.parse(signedUrl);
		},
		...
		async loadTemplate(input) {
			...
			const featureFlag = _asym_pdf_template_schema.PdfBuilderFeatureFlagContractSchema.parse(await dependencies.featureFlags.resolveNativeBuilderFlag({
				engine: normalizedRecord.engine,
				templateId: input.templateId,
				tenantId: input.tenantId
			}));
			return {
				engineSelection: (0, _asym_pdf_template_schema.selectPdfTemplateEngine)({
					featureFlag,
		...
		async previewTemplate(input) {
			const template = _asym_pdf_template_schema.DocumentTemplateV1Schema.parse(input.template);
			return {
		...
		selectTemplateEngine(input) {
			return (0, _asym_pdf_template_schema.selectPdfTemplateEngine)(input);
		},
...
function normalizeTemplateRecord(record) {
	if (record.engine === "unlayer") return {
		engine: "unlayer",
		legacyTemplate: _asym_pdf_template_schema.LegacyPdfTemplateReferenceSchema.parse(record.legacyTemplate)
	};
	return {
		engine: "asym_pdf_document_builder",
		lifecycle: record.lifecycle,
		publishedSnapshot: record.publishedSnapshot,
		template: _asym_pdf_template_schema.DocumentTemplateV1Schema.parse(record.template)
	};
}
...
//#endregion
exports.PdfStudioAdapterError = PdfStudioAdapterError;
Object.defineProperty(exports, "createFakePdfPermissionAdapter", {
	enumerable: true,
	get: function() {
		return _asym_pdf_template_schema.createFakePdfPermissionAdapter;
	}
});
exports.createPdfStudioAdapter = createPdfStudioAdapter;
exports.pdfStudioAdapterBoundary = pdfStudioAdapterBoundary;
Object.defineProperty(exports, "starterPdfTemplateFixtureByCategory", {
	enumerable: true,
	get: function() {
		return _asym_pdf_template_schema.starterPdfTemplateFixtureByCategory;
	}
});
