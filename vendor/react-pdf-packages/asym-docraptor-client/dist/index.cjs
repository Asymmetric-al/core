...
function parsePdfMetadata(metadata) {
	try {
		return _asym_pdf_template_schema.PdfDocumentMetadataSchema.parse(metadata ?? {});
	} catch (error) {
...
function parsePdfProfile(profile) {
	try {
		return _asym_pdf_template_schema.PdfDocumentProfileOptionsSchema.parse(profile ?? {});
	} catch (error) {
...
function createJsonHeaders(apiKey, accept, hasBody = true) {
	return {
		accept,
		authorization: `Basic ${node_buffer.Buffer.from(`${apiKey}:`).toString("base64")}`,
...
//#endregion
exports.DocRaptorClientError = DocRaptorClientError;
exports.calculateDocRaptorRetryDelay = calculateDocRaptorRetryDelay;
exports.classifyDocRaptorRenderError = classifyDocRaptorRenderError;
exports.createDocRaptorClient = createDocRaptorClient;
exports.docraptorClientBoundary = docraptorClientBoundary;
exports.executeDocRaptorAsyncRender = executeDocRaptorAsyncRender;
exports.normalizeDocRaptorAsyncCallbackPayload = normalizeDocRaptorAsyncCallbackPayload;
exports.summarizeDocRaptorAsyncRenderExecutions = summarizeDocRaptorAsyncRenderExecutions;
