import { c as preflightPdfDocumentAssets, d as evaluatePdfDocumentCondition, l as resolvePdfDocumentRepeaterItems, o as composePrintDocumentHtml, s as composePdfDocumentHtml, t as createBrowserPdfPreview, u as resolvePdfDocumentTableRows } from "./preview-BmcBZiCw.mjs";
import { DocumentTemplateV1Schema, PdfDocumentMetadataSchema, calculateFinancialTotals, calculateGroupedTableTotals, calculateInvoiceTotals, calculateNumericAggregate, calculateTableTotals, coreVariableRegistry, createScopedRepeaterContext, createVariableResolver, defaultVariableFormatters, evaluateConditionalRule, findSecretLikeTemplateValues, getValueAtDataPath, resolveRepeaterItems, resolveTableRows } from "@asym/pdf-template-schema";
...
function preflightPdfTemplate(input) {
	...
	const diagnostics = findSecretLikeTemplateValues(input.template).map((diagnostic) => ({
...
function collectVariableDefinitionDiagnostics(template) {
	...
	for (const [index, variable] of template.variables.entries()) {
		if (!coreVariableRegistry.get(variable.key)) diagnostics.push({
...
function collectUnknownFormatterDiagnostics(input) {
	if (!input.formatter || defaultVariableFormatters[input.formatter]) return [];
	return [{
...
function collectDataBindingDiagnostics(template, dataContext) {
	...
	for (const [index, binding] of template.dataBindings.entries()) {
		if (!templateVariableKeys.has(binding.variableKey) && !coreVariableRegistry.get(binding.variableKey)) diagnostics.push({
			code: "unknown_variable",
			details: {
...
function validateSummaryBinding(input) {
	...
	return (calculation.type === "total_contributions" ? calculateNumericAggregate({
	...
	}).diagnostics : calculation.type === "invoice_totals" ? calculateInvoiceTotals({
	...
	}).diagnostics : calculation.type === "financial_report_totals" ? calculateFinancialTotals({
		amountPath: calculation.amountPath,
		...
		precision: input.binding.precision,
		sourcePath: calculation.sourcePath
	}).diagnostics : calculation.type === "grouped_subtotals" || calculation.type === "grand_total" ? calculateGroupedTableTotals({
		context: input.dataContext,
...
//#endregion
export { composePdfDocumentHtml, composePrintDocumentHtml, createBrowserPdfPreview, evaluatePdfDocumentCondition, pdfRendererBoundary, preflightPdfDocumentAccessibility, preflightPdfDocumentAssets, preflightPdfTemplate, resolvePdfDocumentRepeaterItems, resolvePdfDocumentTableRows, resolvePdfDocumentVariables };
