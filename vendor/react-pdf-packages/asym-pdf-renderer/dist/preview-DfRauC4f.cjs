...
function evaluatePdfDocumentCondition(input) {
	...
	const evaluation = (0, _asym_pdf_template_schema.evaluateConditionalRule)({
...
function resolvePdfDocumentTableRows(input) {
	const result = (0, _asym_pdf_template_schema.resolveTableRows)(input);
	return {
...
function createAssetReferenceRegistry(inputs) {
	...
	for (const [index, input] of (inputs ?? []).entries()) {
		const parseResult = _asym_pdf_template_schema.DocumentAssetReferenceSchema.safeParse(input);
...
function preflightPdfDocumentAssets(input) {
	...
	for (const [index, assetInput] of (input.assets ?? []).entries()) {
		...
		const parseResult = _asym_pdf_template_schema.DocumentAssetReferenceSchema.safeParse(assetInput);
		if (!parseResult.success) {
			diagnostics.push({
				code: "invalid_asset_reference",
...
function createRepeaterBindingRegistry(bindings) {
	...
	for (const binding of bindings ?? []) {
		const result = _asym_pdf_template_schema.RepeaterBindingSchema.safeParse(binding);
		if (result.success) {
...
function createTableBindingRegistry(bindings) {
	...
	const invalidBindings = /* @__PURE__ */ new Map();
	for (const binding of bindings ?? []) {
		const result = _asym_pdf_template_schema.TableBindingSchema.safeParse(binding);
		if (result.success) {
			bindingMap.set(result.data.id, result.data);
			continue;
		}
...
function createSummaryBlockBindingRegistry(bindings) {
	...
	const invalidBindings = /* @__PURE__ */ new Map();
	for (const binding of bindings ?? []) {
		const result = _asym_pdf_template_schema.SummaryBlockBindingSchema.safeParse(binding);
		if (result.success) {
			bindingMap.set(result.data.id, result.data);
			continue;
		}
...
function createPlaceholderRegistry(placeholders) {
	...
	for (const placeholder of placeholders ?? []) {
		const result = _asym_pdf_template_schema.DocumentPlaceholderSchema.safeParse(placeholder);
		if (result.success) {
			bindingMap.set(result.data.id, result.data);
			continue;
		}
...
function resolveRenderedTableTotals(input) {
	...
	const result = (0, _asym_pdf_template_schema.calculateTableTotals)({
		context: input.context,
...
function createGroupedSubtotalMap(input) {
	...
	for (const renderedTotal of input.totals) {
		const result = (0, _asym_pdf_template_schema.calculateGroupedTableTotals)({
			context: input.context,
...
function groupRows(rows, groupPath) {
	...
	for (const row of rows) {
		const lookup = (0, _asym_pdf_template_schema.getValueAtDataPath)(isRecord(row.value) ? row.value : {}, groupPath);
...
function renderPageBreak(node, path, state) {
	const result = _asym_pdf_template_schema.DocumentPageBreakAttributesSchema.safeParse(node.attrs ?? {});
	if (!result.success) {
		state.warnings.push({
...
function resolveTotalContributionsSummary(input, calculation) {
	const result = (0, _asym_pdf_template_schema.calculateNumericAggregate)({
		context: input.context,
		operation: "sum",
...
function resolveInvoiceSummary(input, calculation) {
	const result = (0, _asym_pdf_template_schema.calculateInvoiceTotals)({
...
function resolveFinancialReportSummary(input, calculation) {
	const result = (0, _asym_pdf_template_schema.calculateFinancialTotals)({
		amountPath: calculation.amountPath,
...
function formatCalculationValue(input) {
	const formatted = (0, _asym_pdf_template_schema.formatVariableValue)({
...
function readInlineAssetReference(context) {
	...
	const parseResult = _asym_pdf_template_schema.DocumentAssetReferenceSchema.safeParse(asset);
...
function readPageFlowControls(attributes) {
	const result = _asym_pdf_template_schema.PageFlowControlAttributesSchema.safeParse({
...
function readConditionalRule(attributes) {
	...
	const result = _asym_pdf_template_schema.ConditionalRuleSchema.safeParse(parsedRule);
...
function readRepeaterBinding(attributes, registry) {
	...
	if (inlineBinding !== void 0) {
		const inlineResult = _asym_pdf_template_schema.RepeaterBindingSchema.safeParse(inlineBinding);
...
function readTableBinding(attributes, registry) {
	const inlineBinding = readStructuredAttribute(attributes, "binding");
	if (inlineBinding !== void 0) {
		const inlineResult = _asym_pdf_template_schema.TableBindingSchema.safeParse(inlineBinding);
		if (inlineResult.success) return {
			binding: inlineResult.data,
			status: "valid"
		};
...
function readSummaryBlockBinding(attributes, registry) {
	const inlineBinding = readStructuredAttribute(attributes, "binding");
	if (inlineBinding !== void 0) {
		const inlineResult = _asym_pdf_template_schema.SummaryBlockBindingSchema.safeParse(inlineBinding);
		if (inlineResult.success) return {
			binding: inlineResult.data,
			status: "valid"
		};
...
function readDocumentPlaceholder(attributes, registry) {
	...
	if (inlinePlaceholder !== void 0) {
		const inlineResult = _asym_pdf_template_schema.DocumentPlaceholderSchema.safeParse(inlinePlaceholder);
		if (inlineResult.success) return {
...
function composePrintDocumentHtml(input) {
	...
	const themeResult = _asym_pdf_template_schema.DocumentThemeSchema.safeParse(input.theme ?? {});
	const theme = themeResult.success ? themeResult.data : _asym_pdf_template_schema.DocumentThemeSchema.parse({});
...
function resolvePrintMetadata(title, metadata) {
	const parsedMetadata = _asym_pdf_template_schema.PdfDocumentMetadataSchema.parse(metadata ?? {});
	return {
		...parsedMetadata,
...
async function preparePdfPreviewDocument(request, mode) {
	...
	const startedAt = now();
	const templateParseResult = _asym_pdf_template_schema.DocumentTemplateV1Schema.safeParse(request.template);
...
//#endregion
Object.defineProperty(exports, "composePdfDocumentHtml", {
	enumerable: true,
	get: function() {
		return composePdfDocumentHtml;
	}
});
Object.defineProperty(exports, "composePrintDocumentHtml", {
	enumerable: true,
	get: function() {
		return composePrintDocumentHtml;
	}
});
Object.defineProperty(exports, "createBrowserPdfPreview", {
	enumerable: true,
	get: function() {
		return createBrowserPdfPreview;
	}
});
Object.defineProperty(exports, "createDocRaptorTestPreviewMetadata", {
	enumerable: true,
	get: function() {
		return createDocRaptorTestPreviewMetadata;
	}
});
Object.defineProperty(exports, "createPdfPreviewResult", {
	enumerable: true,
	get: function() {
		return createPdfPreviewResult;
	}
});
Object.defineProperty(exports, "evaluatePdfDocumentCondition", {
	enumerable: true,
	get: function() {
		return evaluatePdfDocumentCondition;
	}
});
Object.defineProperty(exports, "normalizeDiagnosticInput", {
	enumerable: true,
	get: function() {
		return normalizeDiagnosticInput;
	}
});
Object.defineProperty(exports, "preflightPdfDocumentAssets", {
	enumerable: true,
	get: function() {
		return preflightPdfDocumentAssets;
	}
});
Object.defineProperty(exports, "preparePdfPreviewDocument", {
	enumerable: true,
	get: function() {
		return preparePdfPreviewDocument;
	}
});
Object.defineProperty(exports, "resolvePdfDocumentRepeaterItems", {
	enumerable: true,
	get: function() {
		return resolvePdfDocumentRepeaterItems;
	}
});
Object.defineProperty(exports, "resolvePdfDocumentTableRows", {
	enumerable: true,
	get: function() {
		return resolvePdfDocumentTableRows;
	}
});
